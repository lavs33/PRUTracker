...
const handleSideNav = (key) => {
  ...
  // Re-Open Logic
  const onReopenClick = () => {
    const previousStatus = ["Active", "Wrong Contact"].includes(String(prospect?.statusBeforeDrop || ""))
      ? String(prospect.statusBeforeDrop)
      : "Active";

    openDeleteModal({
      type: "reopen",
      title: "Re-open Prospect?",
      message: `This will restore the prospect status to ${previousStatus}. Lead records will not be changed. Do you want to continue?`,
      leadsPreview: null,
      policyholder: null,
    });
  };
  ...
  const attemptReopen = async () => {
    try {
      ...
      // Re-opening is status-only; the backend restores the previous prospect status.
