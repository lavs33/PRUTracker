...
function AgentTasksProgress() {
  ...
  const generatePdfReport = () => {
    ...
    reportWindow.document.write(`
      <html>
        <head>
          <title>Agent Task Performance Report - ${escapeHtml(agentCode)}</title>
          <style>
            @page { size: A4 portrait; margin: 12mm 12mm 16mm 12mm; }
            * { box-sizing: border-box; }
            body { font-family: Verdana, Geneva, sans-serif; color: #1f2937; margin: 0; font-size: 11px; line-height: 1.3; }
            .report-shell { padding-bottom: 18mm; }
            .header-band { height: 8px; background: linear-gradient(90deg, #da291c, #ffb81c, #00539b); border-radius: 6px; margin-bottom: 8px; }
            .top-grid { display: grid; grid-template-columns: 1.6fr 1fr; gap: 10px; align-items: start; }
            .title-block { padding: 6px 0 4px; }
            .report-title { margin: 0; color: #991b1b; font-size: 24px; font-weight: 700; letter-spacing: .2px; }
            .report-period { margin-top: 8px; color: #374151; font-size: 13px; font-weight: 700; }
            .details-card { border: 1px solid #f3c4c0; background: #fff7f6; border-radius: 10px; padding: 8px 10px; }
            .details-card h3 { margin: 0 0 6px; color: #991b1b; font-size: 12px; text-transform: uppercase; letter-spacing: .4px; }
            .details-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 4px 10px; }
            ...
            .report-footer { position: fixed; left: 0; right: 0; bottom: 0; padding: 4mm 12mm 0; font-size: 9px; color: #6b7280; display:flex; justify-content:space-between; align-items:center; }
        ...
        <body>
          <div class="report-shell">
            ...
            <section class="section">
              <div class="meta-row">
                ...
                <div class="meta-chip"><div class="label">Rows Included</div><div class="value">${Number(Array.isArray(data.reportTasks) ? data.reportTasks.length : 0)}</div></div>
              </div>
            </section>
          ...
          <div class="report-footer">
            <div>Generated by PRUTracker • ${escapeHtml(formatDateTime(now))}</div>
