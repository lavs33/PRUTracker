...
function AgentTasksProgress() {
  ...
  const [leadWorkloadRows, setLeadWorkloadRows] = useState([]);
  ...
  const fetchProgress = useCallback(async (signal) => {
    ...
    const [progressRes, tasksRes] = await Promise.all([
      fetch(`${API_BASE}/api/tasks/progress?userId=${user.id}`, signal ? { signal } : undefined),
    ...
    if (!tasksRes.ok) throw new Error(tasksPayload?.message || "Failed to fetch task workload.");
    ...
    for (const t of tasks) {
      ...
      if (status === "Open" && Number.isFinite(dueMs) && dueMs < now) row.overdue += 1;
      byLead.set(key, row);
    }
