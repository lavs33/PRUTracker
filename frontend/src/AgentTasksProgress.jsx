...
function AgentTasksProgress() {
  ...
  const fetchProgress = useCallback(async (signal) => {
    if (!user?.id) return;
    const progressRes = await fetch(`${API_BASE}/api/tasks/progress?userId=${user.id}`, signal ? { signal } : undefined);

    const data = await progressRes.json();

    if (!progressRes.ok) throw new Error(data?.message || "Failed to fetch task progress.");
    ...
    setMetrics({
      ...
      lateCompletionRate: Number(data?.doneTasks ? Math.round(((data?.delayedDoneTasks || 0) / data.doneTasks) * 100) : 0),
      typeCounts: Array.isArray(data?.typeCounts) ? data.typeCounts : [],
      dueSoon: Array.isArray(data?.dueSoon) ? data.dueSoon : [],
      leadWorkloadRows: Array.isArray(data?.leadWorkloadRows) ? data.leadWorkloadRows : [],
    });
  }, [API_BASE, user?.id]);
        ...
        <main className="tp-content">
          ...
          <div className="tp-grid">
            <section className="tp-card">
              ...
              <div className="tp-progressRow"><label>Overdue Open Tasks</label><div className="tp-track warn"><span style={{ width: `${overdueOpenRate}%` }} /></div><b>{overdueOpenRate}%</b></div>
            ...
            <section className="tp-card tp-wide">
              ...
              {metrics.leadWorkloadRows.length === 0 ? (
                <p className="tp-muted">No lead engagement workload data yet.</p>
              ) : (
