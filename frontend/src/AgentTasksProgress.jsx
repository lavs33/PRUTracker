...
function AgentTasksProgress() {
  ...
  const generatePdfReport = () => {
    ...
    reportWindow.document.write(`
      <html>
        <head>
          <title>Task Progress Report - ${username}</title>
          <style>
        ...
        <body>
          ...
          <div class="meta">
            ...
            <div><strong>Date Range:</strong> ${datePreset}</div>
            <div><strong>Status Filter:</strong> ${statusFilter}</div>
            <div><strong>Type Filter:</strong> ${typeFilter}</div>
            <div><strong>Total/Open/Done/Overdue:</strong> ${metrics.totalTasks} / ${metrics.openTasks} / ${metrics.doneTasks} / ${metrics.overdueTasks}</div>
            <div><strong>Completion/On-Time/Late:</strong> ${metrics.completionRate}% / ${metrics.onTimeRate}% / ${metrics.lateCompletionRate}%</div>
          </div>
            ...
            <section className="tp-card">
              ...
              <div className="tp-statusChart">
                {metrics.statusChart.map((s) => {
                  const pct = metrics.totalTasks ? Math.round((s.value / metrics.totalTasks) * 100) : 0;
                  return (
