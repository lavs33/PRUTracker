...
function AgentPolicyholderDetails() {
  ...
  const formatTermValue = (type, years, untilAge) => {
    ...
    if (["RANGE_TO_AGE", "MIXED"].includes(normalized) && Number.isFinite(y) && y > 0 && Number.isFinite(age) && age > 0) {
      return `${y} ${y === 1 ? "year" : "years"} / until age ${age}`;
    }
    return "";
  };

  const formatPaymentTerm = (label, type, years, untilAge) => {
    const labelText = String(label || "").trim();
    const valueText = formatTermValue(type, years, untilAge);
    if (labelText && valueText && labelText.toLowerCase() !== valueText.toLowerCase()) {
  ...
  const detailRows = [
    ...
    [
      ...
      formatPaymentTerm(
        coverage.selectedPaymentTermLabel,
