...
function createPrintableReport({
  ...
  const getReportPageSize = (section = {}) => {
    ...
    const requestedSize = Math.max(Number(section.pageSize || fallbackSize), minimumSize);
    if ((section.columns || []).length >= 10) return Math.min(requestedSize, isLandscape ? 12 : 7);
  ...
  reportTableSections.forEach((section) => {
    const rowChunks = chunkRows(section.rows || [], getReportPageSize(section));
  ...
  tablePages.forEach((page) => {
    ...
    if (page.compactWithPrevious && currentGroup && currentGroup.estimatedRows + page.estimatedRows <= compactLimit) {
  ...
  const firstInlineTables = [];
  const continuationTablePageGroups = tablePageGroups;
      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          ...
          .pdf-page { height: ${pageHeight}; margin: 0; padding: 12mm 8mm 18mm; position: relative; page-break-after: always; overflow: hidden; }
          ...
          .pdf-footer { position: absolute; left: 8mm; right: 8mm; bottom: 2mm; padding-top: 6px; border-top: 1px solid #d6dee8; display: flex; justify-content: space-between; font-size: 9px; break-inside: avoid; page-break-inside: avoid; }
        </style>
      </head>
      <body>
        ${firstPage}
