...
function ManagerPortal({ roleType }) {
  ...
  const selectedUnitRows = useMemo(
    () => (portalData?.unitPerformanceRows || portalData?.agents || []).filter((agent) => (selectedUnit?.name ? String(agent?.unit || "") === selectedUnit.name : true)),
    [portalData?.agents, portalData?.unitPerformanceRows, selectedUnit?.name],
  );
  ...
  const generateUnitPerformancePdfReport = () => {
    ...
    createPrintableReport({
      filename: `${user?.username || normalizedRole} - ${unitName} ${unitPerformanceTab === "clients" ? "Clients Relationship Report" : unitPerformanceTab === "tasks" ? "Tasks Performance Report" : `${tabLabel} Performance Report`}`,
      title: unitPerformanceTab === "clients" ? "Unit Clients Relationship Report" : unitPerformanceTab === "tasks" ? "Unit Tasks Performance Report" : `${unitName} ${tabLabel} Performance Report`,
