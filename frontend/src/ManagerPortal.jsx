...
function ManagerPortal({ roleType }) {
  ...
  const generateAgentDetailsPdfReport = () => {
    ...
    const agentReportLabel = unitPerformanceTab === "clients" ? "Clients Relationship" : tabLabel;
    ...
    createPrintableReport({
      filename: `${selectedAgentSummary.username || "Agent"} - Agent ${agentReportLabel} Report`,
      title: `Agent ${agentReportLabel} Report`,
      ...
      details: [
        ...
        { label: "Agent Name", value: selectedAgentSummary.name || "—" },
        { label: "Agent Type", value: selectedAgentSummary.agentType || "—" },
        { label: "Date Employed", value: selectedAgentSummary.dateEmployed ? formatDate(selectedAgentSummary.dateEmployed) : "—" },
      ],
      filters: [
          ...
          {!isLoading && !loadError && activeView === "agents" && (
            selectedAgent ? (
              <section className="manager-panel">
                ...
                <div className="manager-panel__head manager-panel__head--split">
                  <div>
                    <h2>{selectedAgent.name}</h2>
                    <p>Agent details and performance metrics for {selectedAgent.username || selectedAgent.name}.</p>
                  </div>
                  <div className="manager-agent-profile-card" aria-label="Agent details">
                    <div className="manager-metric-pair"><span>Agent Code</span><strong>{selectedAgent.username || "—"}</strong></div>
                    <div className="manager-metric-pair"><span>Agent Type</span><strong>{selectedAgent.agentType || "—"}</strong></div>
                    <div className="manager-metric-pair"><span>First Name</span><strong>{selectedAgent.firstName || "—"}</strong></div>
                    <div className="manager-metric-pair"><span>Middle Name</span><strong>{selectedAgent.middleName || "—"}</strong></div>
                    <div className="manager-metric-pair"><span>Last Name</span><strong>{selectedAgent.lastName || "—"}</strong></div>
                    <div className="manager-metric-pair"><span>Unit</span><strong>{selectedAgent.unit || "—"}</strong></div>
                    <div className="manager-metric-pair"><span>Branch</span><strong>{scope.branchName || selectedAgent.branch || "—"}</strong></div>
                    <div className="manager-metric-pair"><span>Area</span><strong>{scope.areaName || selectedAgent.area || "—"}</strong></div>
                    <div className="manager-metric-pair"><span>Date Employed</span><strong>{selectedAgent.dateEmployed ? formatDate(selectedAgent.dateEmployed) : "—"}</strong></div>
                  </div>
                </div>
