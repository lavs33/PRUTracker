                      ...
                      <div className="manager-retirement-client-cards">
                        ...
                        <article>
                          ...
                          {(selectedUmAffectedClient.policies || []).length ? (selectedUmAffectedClient.policies || []).map((policy) => (
                            <p key={policy.policyholderCode || policy.id || policy._id}><strong>{policy.policyholderCode || "—"}</strong><span>{policy.policyNumber || "—"}</span><span>{policy.productName || policy.policyName || "—"}</span><span>{policy.status || "—"}</span></p>
                                  ...
                                  <tbody>
                                    {retirementAffectedProspects.map((prospect) => (
                                      <tr key={prospect.id || prospect.prospectId || prospect.prospectCode}>
                                        ...
                                        <td>{(prospect.policies || []).length ? prospect.policies.map((policy) => `${policy.policyholderCode || "—"} • ${policy.policyNumber || "—"} • ${policy.productName || policy.policyName || "—"} • ${policy.status || "—"}`).join("; ") : "—"}</td>
                                      </tr>
                                    ))}
                                  </tbody>
                                </table>
