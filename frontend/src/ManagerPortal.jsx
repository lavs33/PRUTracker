...
function ManagerPortal({ roleType }) {
  ...
  }, [isAllUnitsSelected, kpiData?.assignments, selectedUnitSummary]);
  ...
  const branchSalesKpiProgressRows = useMemo(
    () => branchKpiProgressRows.filter(({ kpi }) => ["monthly_sales_production", "monthly_target_achievement_index"].includes(kpi.key)),
          ...
          {!isLoading && !loadError && activeView === "agents" && (
            selectedAgent ? (
              ...
              <section className="manager-panel">
                ...
                <div className="manager-agent-detail-grid manager-agent-detail-grid--single">
                  ...
                  {unitPerformanceTab === "sales" && (
                    <article>
                      ...
                      <div className="manager-metric-pair"><span>Total Frequency Premium Breakdown</span><strong>{formatMoney(selectedUnitSummary.frequencyPremium)}</strong></div>
                ...
                {unitPerformanceTab === "sales" && !isAllUnitsSelected && (
                  <div className="manager-agent-kpi-section">
                        ...
                        <div className="manager-kpi-unit-drilldown">
                          ...
                          <div className="manager-kpi-unit-grid">
                            {topUnitSalesAgents.map((agent) => (
                              <article key={`top-agent:${agent.id}`}>
                                <strong>{agent.username}</strong>
                                <span>{agent.name}</span>
                                <span>Sales Production: {formatMoney(agent.annualPremium)}</span>
                              </article>
                            ))}
                          </div>
                        </div>
                ...
                {unitPerformanceTab === "sales" && isAllUnitsSelected && (
                  <div className="manager-agent-kpi-section">
                    ...
                    {branchSalesKpiProgressRows.length ? (
                      <>
                        <div className="manager-kpi-progress-grid">
                          {branchSalesKpiProgressRows.map(({ assignment, kpi, actual, dateRangeLabel }) => {
                            const comparison = getKpiComparison(actual, kpi);
                            return (
                          ...
                          <div className="manager-kpi-unit-grid">
                            {branchKpiUnitRows.map((unit) => (
                              <article key={`branch-unit:${unit.unit}`}>
                                ...
                                <small>Top Agents: {unit.topAgents.length ? unit.topAgents.map((agent) => `${agent.username} (${formatMoney(agent.annualPremium)})`).join(", ") : "No production yet"}</small>
                              </article>
                            ))}
                          </div>
                        </div>
