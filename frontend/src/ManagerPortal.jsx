...
function createPrintableReport({
  ...
  tablePages.forEach((page) => {
    ...
    if (page.compactWithPrevious && currentGroup && currentGroup.estimatedRows + page.estimatedRows <= page.compactMaxRows) {
      currentGroup.pages.push(page);
      currentGroup.estimatedRows += page.estimatedRows;
      return;
    }
    tablePageGroups.push({ pages: [page], estimatedRows: page.estimatedRows });
  });
  const firstInlineTables = tablePageGroups[0]?.pages || [];
  const continuationTablePageGroups = tablePageGroups.slice(1);
      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          ...
          .pdf-page { height: ${pageHeight}; margin: 10mm 8mm 12mm; padding: 4mm 2mm 16mm; position: relative; page-break-after: always; overflow: hidden; }
...
function ManagerPortal({ roleType }) {
  ...
  const topUnitKpiSalesAgents = useMemo(
  ...
  const generateUnitKpiPdfReport = () => {
    ...
    createPrintableReport({
      ...
      filters: [
        ...
        { label: "Top Contributing Agents", value: String(topUnitKpiSalesAgents.length) },
      ],
      ...
      tableSections: [
        ...
        },
        {
          ...
          rows: topUnitKpiSalesAgents.map((agent) => ({
            username: agent.username,
            name: agent.name || agent.username,
            salesProduction: formatMoney(agent.annualPremium),
          })),
                ...
                <div className="manager-panel__head">
                  <div>
                    <h2 className="manager-agent-title">
                      ...
                      <span className={`manager-agent-status-pill manager-agent-status-pill--${getAgentStatusClass(selectedAgent.status)}`} title={getLongLeaveStatusHint(selectedAgent)}>{selectedAgent.status || "Active"}</span>
                    </h2>
                    <p>
              ...
              <section className="manager-panel">
                ...
                <div className="manager-panel__head manager-agent-profile-head">
                  ...
                  <div className="manager-orphan-agent-actions">
                    <button type="button" className="manager-refresh-btn" onClick={() => openOrphanAgentAction("long_leave")} disabled={selectedAgent.status === "On Long Leave" || selectedAgent.status === "Retired" || hasPendingLongLeaveRecord}>Mark as On Long Leave</button>
                    ...
                    <tbody>
                      {orphanAgentRows.map((agent) => (
                        <tr key={agent.id} onClick={() => openAgentDetails(agent.id)} tabIndex={0} role="button" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") openAgentDetails(agent.id); }}>
                          ...
                          <td><span className={`manager-agent-status-pill manager-agent-status-pill--table manager-agent-status-pill--${getAgentStatusClass(agent.status)}`} title={getLongLeaveStatusHint(agent)}>{agent.status || "Active"}</span></td>
                          <td>{agent.unit || "—"}</td>
          ...
          {!isLoading && !loadError && activeView === "dashboard" && normalizedRole !== "BM" && (
            <section className="manager-panel">
              ...
              {!dashboardUnitKpiCards.length ? (
                ...
                <div className="manager-kpi-unit-drilldown">
                  <h3>Top 5 Contributing Agents</h3>
                  {topUnitKpiSalesAgents.length ? (
                    <ul className="manager-kpi-agent-list">
                      {topUnitKpiSalesAgents.map((agent) => (
