  ...
  const notifyAgentSalesRecommendation = async (agent) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/manager/agent-sales-recommendations/notify`, {
        ...
        body: JSON.stringify({ userId: user.id, agentUserId: agent.userId, recommendationKey: key, periodLabel: getPresetLabel(unitKpiDatePreset), agentContribution: formatMoney(agent.annualPremium), unitProduction: formatMoney(unitProduction), unitTarget: formatKpiTarget(dashboardUnitKpiCards.find(({ kpi }) => kpi.key === "monthly_sales_production")?.kpi || {}), contributionShare: unitProduction ? `${((Number(agent.annualPremium || 0) / unitProduction) * 100).toFixed(1)}%` : "0.0%", fairShare: formatMoney(fairShare) }),
      });
      const payload = await response.json();
