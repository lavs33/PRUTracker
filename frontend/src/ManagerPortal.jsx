...
function getKpiComparison(actual, kpi) {
  ...
  if (delta > 0) {
    const deltaValue = kpi?.valueType === "Percent" ? `${Number(delta).toFixed(0)} percentage points` : formatActualKpiValue(delta, kpi.valueType);
...
function ManagerPortal({ roleType }) {
  ...
  }, [branchKpiDatePreset, branchKpiProgressRows, branchKpiUnitRows, unitOptions]);
                    ...
                    <article className={`manager-kpi-progress-card manager-kpi-progress-card--branch ${comparison.className}`} key={`${assignment.scopeType}:${assignment.scopeId}:${kpi.key}`}>
                      ...
                      <div className="manager-branch-kpi-units">
                        ...
                        {branchKpiUnitRows.map((unit) => {
                          ...
                          const recommendation = branchKpiRecommendations.find((item) => item.key === `${kpi.key}:${unit.unit}`);
                          return (
                              ...
                              {recommendation ? (
                                <div className="manager-branch-kpi-unit__recommendation">
                                  ...
                                  <button type="button" onClick={() => { setSelectedUnitName(recommendation.unitName); setSelectedAgentId(""); setActiveView("agents"); window.scrollTo({ top: 0, behavior: "smooth" }); }}>
                                    Open unit details <FaArrowRight aria-hidden="true" />
                                  </button>
                                </div>
                              ) : null}
                            </div>
