      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          ...
          .pdf-page { height: 274mm; padding: 4mm 2mm 16mm; position: relative; page-break-after: always; overflow: hidden; }
        ...
        ? [
            {
              ...
              rows: branchSalesKpiProgressRows.map(({ kpi, actual, targetBasis }) => ({
                ...
                value: kpi.key === "monthly_target_achievement_index" && targetBasis
                  ? `${formatMoney(targetBasis)} sales target • ${formatActualKpiValue(actual, kpi.valueType)} achievement`
                  : `${formatActualKpiValue(actual, kpi.valueType)} actual • ${formatKpiTarget(kpi)} target`,
              })),
            },
            {
              title: "Unit Drilldown",
              rows: branchKpiUnitRows.map((unit) => ({
                label: unit.unit,
                value: `${formatMoney(unit.annualPremium)}${unit.topAgents.length ? ` • Top: ${unit.topAgents.map((agent) => `${agent.username} ${agent.name || ""}`.trim()).join(", ")}` : ""}`,
              })),
            },
          ]
        : [],
