...
function ManagerPortal({ roleType }) {
  ...
  const branchKpiDateOptions = MANAGER_REPORT_DATE_OPTIONS;
  ...
  const branchKpiRecommendations = useMemo(() => {
    ...
    const guidanceByKpi = {
      ...
      monthly_active_agents: {
        title: "Activate more agents",
        action: "schedule an activation huddle and practical field coaching for inactive agents",
        detail: "Use activity planning, joint-field-work, and prospecting training to help more agents contribute this period.",
      },
    ...
    return branchKpiProgressRows.flatMap(({ kpi, actual }) => {
      ...
      return [...branchKpiUnitRows]
        ...
        .map((unit) => {
          ...
          return {
            key: `${kpi.key}:${unit.unit}`,
            kpiLabel: formatKpiLabel(kpi, "BRANCH"),
            title: guidance.title,
            unitName: unit.unit,
            unitManager: unitProfile?.manager?.name || unitProfile?.manager?.code || "the Unit Manager",
  ...
  }, [branchKpiProgressRows, branchKpiUnitRows, unitOptions]);
        ...
        <main className="manager-portal__content">
          ...
          {!isLoading && !loadError && activeView === "kpi_progress" && normalizedRole === "BM" && (
            <section className="manager-panel">
              ...
              {branchKpiRecommendations.length ? (
                <section className="manager-branch-recommendations" aria-labelledby="branch-kpi-recommendations-title">
                  <div className="manager-branch-recommendations__head">
                    <div>
                      <span>Recommended unit actions</span>
                      <h3 id="branch-kpi-recommendations-title">Where Branch Management can intervene</h3>
                      <p>Unit-level recommendations are prioritized from the lowest contributors to KPIs that remain below target.</p>
                    </div>
                    <b>{branchKpiRecommendations.length}</b>
                  </div>
                  <div className="manager-branch-recommendations__grid">
                    {branchKpiRecommendations.map((recommendation) => (
                      <article key={recommendation.key}>
                        <div className="manager-branch-recommendations__meta">
                          <span>{recommendation.kpiLabel}</span>
                          <b>{recommendation.metricValue}</b>
                        </div>
                        <h4>{recommendation.title} in {recommendation.unitName}</h4>
                        <p>Contact <strong>{recommendation.unitManager}</strong> to {recommendation.action}.</p>
                        <small>{recommendation.detail}</small>
                        <button type="button" onClick={() => { setSelectedUnitName(recommendation.unitName); setSelectedAgentId(""); setActiveView("agents"); window.scrollTo({ top: 0, behavior: "smooth" }); }}>
                          Open {recommendation.unitName} details <FaArrowRight aria-hidden="true" />
                        </button>
                      </article>
                    ))}
                  </div>
