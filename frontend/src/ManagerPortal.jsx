...
const BM_URGENT_KPI_NOTIFICATION_TYPES = new Set([
...
const BmRecommendationConcernMessage = ({ notification }) => {
  ...
  const capitalizedRecommendation = recommendationText
    ...
    : "";
  return (
    <div className="manager-urgent-recommendation">
      <div className="manager-urgent-recommendation__heading">
      ...
      <div className="manager-urgent-recommendation__context">
        {isSalesProduction ? (
          <>
            ...
            <div><small>Branch Sales Production target</small><b>{metadata.branchTargetLabel || "—"}</b></div>
            <div><small>Unit contribution to branch target</small><b>{metadata.branchContributionLabel || "—"}</b></div>
          </>
        ) : (
          <>
            <div><small>Branch KPI progress</small><b>{metadata.progressLabel || "—"}</b></div>
            <div><small>Unit contribution to branch</small><b>{metadata.unitContributionLabel || "—"}</b></div>
            <div><small>Branch target to achieve</small><b>{metadata.branchTargetLabel || metadata.targetLabel || "—"}</b></div>
          </>
        )}
        <div><small>Reporting period</small><b>{metadata.periodLabel || "Current month"}</b></div>
      </div>
      <p>{metadata.contributionReminder || notification?.message}</p>
      <div className="manager-urgent-recommendation__action">
...
function ManagerPortal({ roleType }) {
  ...
  const [reassignmentMonth, setReassignmentMonth] = useState(currentKpiMonth);
  ...
  const hasAssignedUnitKpi = useMemo(() => {
    if (normalizedRole === "BM") return true;
    const unitAssignment = (kpiData?.assignments || []).find((assignment) => assignment.scopeType === "UNIT");
    return (unitAssignment?.kpis || []).some((kpi) => kpi.assigned !== false && kpi.key === "monthly_sales_production");
  }, [kpiData?.assignments, normalizedRole]);
  ...
  useEffect(() => {
    if (normalizedRole !== "UM" || location.state?.activeView !== "orphan_endorsements") return;
    const endorsementRecordId = String(location.state?.endorsementRecordId || "");
    if (!endorsementRecordId) return;
    const endorsementType = String(location.state?.endorsementType || "").toLowerCase();
    setSelectedUmAffectedClient(null);
    setSelectedReassignmentAgentId("");
    setOrphanEndorsementTab(endorsementType === "resignation" ? "resignations" : "long_leaves");
    setSelectedUmLongLeaveRecordId(endorsementRecordId);
    window.scrollTo({ top: 0, behavior: "auto" });
  }, [location.state, normalizedRole]);
  ...
  useEffect(() => {
    ...
    if (normalizedRole !== "BM" && activeView === "kpi_progress" && !hasAssignedUnitKpi) setActiveView("dashboard");
  }, [activeView, hasAssignedUnitKpi, normalizedRole]);
  ...
  useEffect(() => {
    ...
    const fetchUrgentNotifications = async () => {
      setUrgentNotificationsLoading(true);
      setUrgentNotificationsError("");
      try {
        const params = new URLSearchParams({ userId: user.id, includeRefs: "true" });
  ...
  const openUrgentKpiNotification = async (notification) => {
    setUrgentNotificationsError("");
    try {
      if (notification?.status === "Unread") {
        const response = await fetch(`${API_BASE}/api/notifications/${notification._id}/read?userId=${user.id}`, {
          method: "PATCH",
        });
        const payload = await response.json();
        if (!response.ok) throw new Error(payload?.message || "Failed to mark notification as read.");
        setUrgentNotifications((current) => current.map((item) => (
          String(item?._id) === String(notification?._id) ? { ...item, status: "Read" } : item
        )));
  ...
  const notifyAgentKpiRecommendation = async ({ kpi, actual, dateRangeLabel }) => {
    ...
    const recommendationAction = `Improve ${formatKpiLabel(kpi, "AGENT")}`;
    const recommendationDetail = "Focus on consistent daily activity, review current opportunities, and close the remaining KPI gap before the end of the reporting period.";
    setRecommendationNotifyingKey(key);
    try {
      const response = await fetch(`${API_BASE}/api/manager/agent-sales-recommendations/notify`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const reassignmentAgentRows = useMemo(() =>
    (portalData?.agents || [])
      ...
      .filter((agent) => agent.reassignmentMetrics.activePolicies <= 9)
  ...
  const openAffectedClientDetail = (client) => {
    ...
    const reassignedAt = new Date(client?.reassignedAt || 0);
    setReassignmentMonth(client?.reassigned === true && !Number.isNaN(reassignedAt.getTime())
      ? monthKey(reassignedAt)
      : currentKpiMonth);
    ...
    return branchKpiProgressRows.flatMap(({ kpi, actual, dateRangeLabel }) => {
      ...
      return [...branchKpiUnitRows]
        ...
        .map((unit) => {
          ...
          const recommendationValueType = kpi.key === "monthly_sales_production" ? "Currency" : kpi.valueType;
          return {
            ...
            metricValue: formatActualKpiValue(kpi.key === "monthly_sales_production" ? unit.annualPremium : resolveMetric(unit), recommendationValueType),
            unitContributionLabel: formatActualKpiValue(kpi.key === "monthly_sales_production" ? unit.annualPremium : unitMetric, recommendationValueType),
            ...
            requiredTargetLabel: kpi.key === "monthly_sales_production"
              ? branchUnitSalesTargetDetails.label
    ...
    <div className="manager-portal">
      ...
      <div className="manager-portal__body">
        <ManagerSideNav
          ...
          showUnitKpiProgress={hasAssignedUnitKpi}
        />
        ...
        <main className="manager-portal__content">
          ...
          {!isLoading && !loadError && activeView === "dashboard" && ["BM", "UM"].includes(normalizedRole) && (
            <section className="manager-urgent-actions" aria-labelledby="manager-urgent-actions-title">
              ...
              {urgentNotificationsError ? (
                <div className="manager-urgent-actions__feedback manager-urgent-actions__feedback--error">
                  {urgentNotificationsError}
                </div>
                      ...
                      <div className="manager-urgent-card__body">
                        ...
                        {String(notification.message || "").trim() ? (
                          normalizedRole === "UM" && String(notification?.type || "").toUpperCase() === "ORPHANS_ENDORSEMENTS"
                            ...
                            : normalizedRole === "UM" && String(notification?.type || "").toUpperCase() === "BM_RECOMMENDATION"
                              ? <BmRecommendationConcernMessage notification={notification} />
          ...
          {!isLoading && !loadError && activeView === "agents" && (
            selectedAgent ? (
                    ...
                    <div className="manager-kpi-progress-grid">
                      {selectedAgentKpiCards.map(({ assignment, kpi, actual, comparison, dateRangeLabel }) => {
                        ...
                        return <article className={`manager-kpi-progress-card ${comparison.className}`} key={`${assignment.scopeType}:${assignment.scopeId}:${kpi.key}`}>
                          ...
                          {belowTarget && ["UM", "AUM"].includes(normalizedRole) ? <div className="manager-agent-sales-recommendation manager-agent-sales-recommendation--underperforming">
                            <strong>Recommended action: Improve {formatKpiLabel(kpi, "AGENT")}</strong>
                            <small>Focus on consistent daily activity, review current opportunities, and close the remaining KPI gap before the end of the reporting period.</small>
              ...
              <section className="manager-panel">
                ...
                {unitPerformanceTab === "sales" && !isAllUnitsSelected && isUnitPerformanceMonthSelected && selectedUnitKpiCards.length > 0 && (
                  <div className="manager-agent-kpi-section">
          ...
          {!isLoading && !loadError && activeView === "kpi_progress" && normalizedRole !== "BM" && hasAssignedUnitKpi && (
            <section className="manager-panel manager-panel--unit-kpi-progress">
              <div className="manager-panel__head">
                <div>
              ...
              <div className="manager-kpi-progress-grid manager-kpi-progress-grid--branch manager-kpi-progress-grid--unit-focus">
                {dashboardUnitKpiCards.map(({ kpi, actual, comparison, dateRangeLabel }) => (
                  <article className={`manager-kpi-progress-card manager-kpi-progress-card--branch manager-kpi-progress-card--unit-focus ${comparison.className}`} key={`dashboard-unit:${kpi.key}`}>
                        ...
                        {branchKpiUnitRows.map((unit) => {
                          ...
                          return (
                            <div className="manager-branch-kpi-unit" key={`${kpi.key}:${unit.unit}`}>
                              ...
                              {unitMetSalesTarget ? (
                                <div className="manager-branch-kpi-unit__achieved">
                                  Reached or exceeded the Unit Sales Production KPI target - {branchUnitSalesTargetDetails.label}.
                                </div>
                              ) : null}
                              {recommendation ? (
