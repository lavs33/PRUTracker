...
function ManagerPortal({ roleType }) {
  ...
  const validateKpiDraft = (kpi) => {
    if (!KPI_FREQUENCIES.includes(String(kpi.period || ""))) return "Frequency is required.";
    const hasMin = String(kpi.targetMin ?? "").trim() !== "";
    const hasMax = String(kpi.targetMax ?? "").trim() !== "";
    const hasTarget = String(kpi.targetValue ?? "").trim() !== "";
    const min = Number(kpi.targetMin);
    const max = Number(kpi.targetMax);
    const target = Number(kpi.targetValue);

    if ((hasMin && !Number.isFinite(min)) || (hasMax && !Number.isFinite(max)) || (hasTarget && !Number.isFinite(target))) {
      return "Targets must be valid numbers.";
    }
    if ((hasMin && !hasMax) || (!hasMin && hasMax)) return "Min and max must both be filled or both be blank.";
    if (hasMin && hasMax && min >= max) return "Min must be less than max.";
    return "";
  };
  ...
  const saveKpi = async (assignment, kpiKey) => {
    ...
    const validationMessage = validateKpiDraft(kpi || {});
    if (validationMessage) {
      setKpiMessage(`${kpi?.label || "KPI"}: ${validationMessage}`);
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/manager/kpi-assignments/${assignment.scopeType}/${assignment.scopeId}`, {
        ...
        body: JSON.stringify({ userId: user.id, kpis: draftList }),
      });
      const data = await res.json();
                  ...
                  <div className="manager-kpi-assignment-card" key={assignmentKey}>
                    ...
                    <div className="manager-kpi-edit-grid">
                      {draftKpis.map((kpi) => {
                        ...
                        return (
                          <div className={`manager-kpi-edit-row ${isEditing ? "editing" : ""}`} key={kpi.key}>
                            ...
                            <label>
                              <span>Frequency</span>
                              <select className="manager-kpi-input" value={kpi.period || ""} disabled={!kpiData?.canEdit || !isEditing} onChange={(e) => updateKpiDraft(assignment, kpi.key, "period", e.target.value)} required>
                            ...
                            <label>
                              <span>Min</span>
                              <input className="manager-kpi-input" type="number" value={kpi.targetMin ?? ""} disabled={!kpiData?.canEdit || !isEditing} onChange={(e) => updateKpiDraft(assignment, kpi.key, "targetMin", e.target.value)} />
                            </label>
                            <label>
                              <span>Max</span>
                              <input className="manager-kpi-input" type="number" value={kpi.targetMax ?? ""} disabled={!kpiData?.canEdit || !isEditing} onChange={(e) => updateKpiDraft(assignment, kpi.key, "targetMax", e.target.value)} />
                            </label>
                            <label>
                              <span>Target</span>
                              <input className="manager-kpi-input" type="number" value={kpi.targetValue ?? ""} disabled={!kpiData?.canEdit || !isEditing} onChange={(e) => updateKpiDraft(assignment, kpi.key, "targetValue", e.target.value)} />
                            </label>
