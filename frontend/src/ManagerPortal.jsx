...
function ManagerPortal({ roleType }) {
  ...
  const validateKpiDraft = (kpi) => {
    ...
    getKpiTargets(kpi).forEach((target) => {
      ...
      if (hasMin && !hasMax) errors[`${prefix}.targetMax`] = "Max is required when min is entered.";
      if (!hasMin && hasMax) errors[`${prefix}.targetMin`] = "Min is required when max is entered.";
  ...
  const saveKpi = async (assignment, kpiKey) => {
    ...
    if (Object.keys(validationErrors).length) {
      ...
      setKpiMessage(`${kpi?.label || "KPI"} needs required target values for every frequency.`);
      return;
    }
                      ...
                      {draftKpis.map((kpi) => {
                        ...
                        return (
                          <div className={`manager-kpi-edit-row ${isEditing ? "editing" : ""}`} key={kpi.key}>
                            ...
                            <div className="manager-kpi-frequency-grid">
