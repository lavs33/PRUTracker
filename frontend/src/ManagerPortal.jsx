...
function buildScopeData(roleType) {
  ...
  return {
    ...
    agents,
    alerts: [
      { title: "Overdue follow-ups", detail: `${totalOverdueTasks} overdue tasks need coaching attention.` },
      { title: "Conversion focus", detail: `${totalAgents - 2} agents are below the target conversion band this month.` },
      { title: "Premium watch", detail: "One advisor has no issued policy in the current reporting window." },
    ],
    salesRows: agents
      .map((agent) => ({
        ...agent,
...
function ManagerPortal({ roleType }) {
  ...
  useEffect(() => {
    if (!user || user.role !== normalizedRole) {
      navigate(`/${normalizedRole.toLowerCase()}/login`, { replace: true });
      return;
    }
        ...
        <main className="manager-portal__content">
          ...
          {activeView === "dashboard" && (
            <section className="manager-grid">
              ...
              <article className="manager-panel">
                <div className="manager-panel__head">
                  <h2>Manager Action Queue</h2>
                  <p>Suggested coaching and follow-up priorities.</p>
                </div>
                <div className="manager-alert-list">
                  {scopeData.alerts.map((alert) => (
                    <div key={alert.title}>
          ...
          {activeView === "notifications" && (
            <section className="manager-panel">
              <div className="manager-panel__head">
                <h2>Notifications</h2>
                <p>Preview manager-focused alerts and follow-up prompts.</p>
              </div>
              <div className="manager-alert-list">
                {scopeData.alerts.map((alert) => (
                  <div key={alert.title}>
          ...
          {activeView === "profile" && (
            <section className="manager-panel">
              <div className="manager-panel__head">
                ...
                <p>Current signed-in manager identity and scope summary.</p>
              </div>
              <div className="manager-kpi-grid">
                <div>
                  <span>Role</span>
                  <strong>{user?.role || normalizedRole}</strong>
                </div>
                <div>
                  <span>Username</span>
