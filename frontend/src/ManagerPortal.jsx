...
function ManagerPortal({ roleType }) {
  ...
  const renderReassignmentProgressCell = (value, target, suffix = "") => {
    const status = getReassignmentProgressStatus(value, target);
    return (
      <td className={`manager-reassignment-metric-cell manager-reassignment-metric-cell--progress manager-reassignment-metric-cell--${status.key}`}>
        <span className="manager-reassignment-metric-value">{value}{suffix}</span>
        <span className="manager-reassignment-status-indicator">{status.label}</span>
      </td>
    );
  };

  const renderReassignmentTargetCell = (value, suffix = "") => (
    <td className="manager-reassignment-metric-cell manager-reassignment-metric-cell--target">
      <span className="manager-reassignment-metric-value">{value}{suffix}</span>
      <span className="manager-reassignment-status-indicator">Target</span>
    </td>
  );
                  ...
                  <>
                    ...
                    <div className="manager-table-wrap">
                      <table className="manager-table manager-table--promotion-history">
                        <thead>
                          <tr><th>Agent Code</th><th>Agent Name</th><th>Agent Type</th><th>Status</th><th>{reassignmentMonthLabel} Done Approaches</th><th>{reassignmentMonthLabel} Approach Target</th><th>Open/Overdue Approach Tasks as of {reassignmentMonthLabel}</th><th>Projected {reassignmentMonthLabel} Approaches</th><th>{reassignmentMonthLabel} Closing Ratio</th><th>{reassignmentMonthLabel} Closing Ratio Target</th><th>{reassignmentMonthLabel} Active Policies</th><th>{reassignmentMonthLabel} Active Policy Target</th><th>Select</th>{selectedUmAffectedClient.reassigned === true && <th>Reassignment Date and Time</th>}</tr>
                        </thead>
                        <tbody>
                          {displayedReassignmentAgentRows.map((agent) => (
                            <tr key={agent.id}>
                              ...
                              {renderReassignmentProgressCell(agent.reassignmentMetrics.completedApproaches, agent.reassignmentMetrics.monthlyDoneApproachesTarget)}
                              {renderReassignmentTargetCell(agent.reassignmentMetrics.monthlyDoneApproachesTarget)}
                              <td className="manager-reassignment-metric-cell manager-reassignment-metric-cell--open">
                                <span className="manager-reassignment-metric-value">{agent.reassignmentMetrics.openApproachTasks}</span>
                                <span className="manager-reassignment-status-indicator">Open/overdue now</span>
                              </td>
                              {renderReassignmentProgressCell(agent.reassignmentMetrics.projectedApproaches, agent.reassignmentMetrics.monthlyDoneApproachesTarget)}
                              {renderReassignmentProgressCell(agent.reassignmentMetrics.closingRatio, agent.reassignmentMetrics.monthlyClosingRatioTarget, "%")}
                              {renderReassignmentTargetCell(agent.reassignmentMetrics.monthlyClosingRatioTarget, "%")}
                              {renderReassignmentProgressCell(agent.reassignmentMetrics.activePolicies, agent.reassignmentMetrics.monthlyActivePoliciesTarget)}
                              {renderReassignmentTargetCell(agent.reassignmentMetrics.monthlyActivePoliciesTarget)}
