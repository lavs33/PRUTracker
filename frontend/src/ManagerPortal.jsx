...
function selectKpiTargetForPeriod(kpi = {}, period = "") {
  ...
  const periodTarget = getKpiTargets(kpi).find((target) => target.period === period) || {};
  return {
    ...kpi,
    period,
...
function ManagerPortal({ roleType }) {
  ...
  const branchSalesKpiProgressRows = useMemo(() => {
    ...
    const productionTarget = Number(salesProductionKpiForPeriod.targetValue ?? salesProductionKpiForPeriod.targetMin ?? 0);
    const targetAchievementIndex = productionTarget ? Math.round((Number(rowSummary.totalAnnualPremium || 0) / productionTarget) * 100) : 0;
                          ...
                          <div className="manager-kpi-unit-grid">
                            {branchKpiUnitRows.map((unit) => (
                              <article key={`branch-unit:${unit.unit}`}>
                                ...
                                <small>Top Agents: {unit.topAgents.length ? unit.topAgents.map((agent) => `${agent.name || agent.username} (${formatMoney(agent.annualPremium)})`).join(", ") : "No production yet"}</small>
                              </article>
                            ))}
                          </div>
                        </div>
