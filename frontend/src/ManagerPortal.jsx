...
function ManagerPortal({ roleType }) {
  ...
  const branchUnitSalesTarget = useMemo(() => {
    ...
    return Number([targetKpi.targetValue, targetKpi.targetMin, targetKpi.targetMax].find((value) => Number(value) > 0) || 0);
  ...
  }, [branchKpiDatePreset, branchKpiProgressRows, branchKpiUnitRows, branchUnitSalesTarget, unitOptions]);
                          ...
                          return (
                            <div className="manager-branch-kpi-unit" key={`${kpi.key}:${unit.unit}`}>
                              <div className="manager-branch-kpi-unit__metric">
                                <button type="button" className="manager-branch-kpi-unit__link" onClick={() => { setSelectedUnitName(unit.unit); setSelectedAgentId(""); setActiveView("agents"); window.scrollTo({ top: 0, behavior: "smooth" }); }}>{unit.unit}</button>
                              ...
                              {kpi.key === "monthly_sales_production" && unit.topAgents.length ? (
                                <div className="manager-branch-kpi-agents">
                                  ...
                                  {unit.topAgents.map((agent) => (
                                    <button type="button" key={`${unit.unit}:${agent.id}`} onClick={() => openAgentDetails(agent.id)}>
