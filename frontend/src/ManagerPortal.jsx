...
function ManagerPortal({ roleType }) {
  ...
  const endorseRetirementOrphans = async () => {
    if (!orphanRetirementId) return;
    setOrphanLongLeaveSaving(true);
    try {
      const res = await fetch(`${API_BASE}/api/manager/retirement/${orphanRetirementId}/status`, {
                    ...
                    {orphanAgentAction === "retired" && (
                      <>
                        ...
                        {orphanRetirementStep === 3 && (
                          <div className="manager-orphan-confirm-step">
                            <div className="manager-section-subhead"><h3>Endorse Orphan Clients</h3><p>Endorse confirmed retirement orphan clients to the unit manager.</p></div>
                            <p className="manager-filter-note">Status: {orphanRetirementStatus}</p>
                            {retirementAffectedProspects.length ? (
                              <div className="manager-promotion-table-wrap manager-promotion-table-wrap--retirement">
                                <table className="manager-table manager-table--promotion-history manager-table--retirement-orphans">
                                  <thead><tr><th>Prospect Code</th><th>Prospect Name</th><th>Market Type</th><th>Prospect Type</th><th>Leads</th><th>Policies</th></tr></thead>
        ...
        <div className="manager-modal-backdrop" role="presentation">
          <div className="manager-endorse-modal" role="dialog" aria-modal="true" aria-labelledby="endorse-orphans-title">
            ...
            <div className="manager-endorse-modal__header">
              ...
              <p>Review the long leave details and selected orphan clients before endorsement.</p>
            </div>
            ...
            <div className="manager-endorse-modal__actions">
              ...
              <button type="button" className="manager-refresh-btn manager-long-leave-next-btn" onClick={confirmEndorseLongLeaveOrphans} disabled={orphanLongLeaveSaving}>
