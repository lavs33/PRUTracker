...
function ManagerPortal({ roleType }) {
  ...
  const generateUnitKpiPdfReport = () => {
    if (!selectedUnitKpiCards.length) return;
    ...
    createPrintableReport({
      ...
      filters: [
        { label: "Assigned KPI Cards", value: String(selectedUnitKpiCards.length) },
      ...
      statCards: selectedUnitKpiCards.map(({ kpi, actual }) => ({
      ...
      tableSections: [
        {
          ...
          rows: selectedUnitKpiCards.map(({ kpi, actual, comparison, dateRangeLabel }) => ({
              ...
              <section className="manager-panel">
                <nav className="manager-breadcrumb manager-breadcrumb--agent" aria-label="Agent detail breadcrumb">
                  {normalizedRole === "BM" ? (
                    ...
                    <button type="button" onClick={() => { setSelectedAgentId(""); setActiveView("dashboard"); window.scrollTo({ top: 0, behavior: "smooth" }); }}>{selectedAgent.unit || scope.unitName || "Unit"}</button>
                  )}
                  <span>&gt;</span>
