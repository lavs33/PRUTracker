...
function ManagerPortal({ roleType }) {
  ...
  const filteredSalesRows = useMemo(
    () => buildFilter(portalData?.salesRows || [], salesSearch, ["username", "name", "latestPolicyStatus"]),
  ...
  const taskReportColumns = [
    ...
    { key: "lastCompletedAt", label: "Last Completed", render: (row) => formatDateTime(row.lastCompletedAt) },
  ...
  const salesReportColumns = [
    ...
    { key: "latestPolicyStatus", label: "Latest Policy Status" },
  ];

  const unitDetails = [
    { label: normalizedRole === "BM" ? "Branch Manager Code" : "Unit Manager Code", value: user?.username || "—" },
    { label: "Role", value: normalizedRole || "—" },
    { label: normalizedRole === "BM" ? "Branch" : "Unit", value: scope.branchName || scope.unitName || "—" },
    ...
    { label: "Prepared By", value: [user?.firstName, user?.lastName].filter(Boolean).join(" ") || user?.username || "—" },
    { label: "Generated At", value: generatedAtLabel || formatDateTime(new Date()) },
  ];
          ...
          {!isLoading && !loadError && activeView === "sales_performance" && (
            <section className="manager-panel">
              ...
              <Toolbar
                ...
                searchPlaceholder="Search username, name, or policy status"
