...
function scaleKpiTargetValue(value, valueType, fromPeriod, toPeriod) {
  ...
  return Math.round(numericValue * (toWeight / fromWeight) * 100) / 100;
}
  ...
  const prefillKpiTargetsFromDefault = (assignment, kpiKey, defaultPeriodOverride = "") => {
    ...
    setKpiDrafts((current) => ({
      ...
      [assignmentKey]: (current[assignmentKey] || assignment.kpis || []).map((kpi) => {
        ...
        if (!hasDefaultTarget && !hasDefaultMin && !hasDefaultMax) return { ...kpi, period: defaultPeriod };
                            ...
                            <div className="manager-kpi-edit-row__head">
                              ...
                              <div className="manager-kpi-row-actions">
                                ...
                                {kpiData?.canEdit ? (
                                  isEditing ? (
                                    <>
                                      <button type="button" className="manager-refresh-btn manager-kpi-autofill-btn" onClick={() => prefillKpiTargetsFromDefault(assignment, kpi.key)} disabled={isSaving}>
                                        Auto-fill
                                      </button>
