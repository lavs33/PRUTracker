...
function ManagerPortal({ roleType }) {
  ...
  const [orphanSupportingFiles, setOrphanSupportingFiles] = useState([]);
                        ...
                        {orphanLongLeaveStep === 1 && (
                          <div className="manager-orphan-action-form">
                            <label>
                              ...
                              <input type="date" min={todayDateInputValue} value={orphanLeaveStartDate} onChange={(e) => { setOrphanLeaveStartDate(e.target.value); setConfirmOrphanTransfer(false); }} required />
                            </label>
                            <label>
                              <span>Leave End Date</span>
                              <input type="date" min={orphanLeaveStartDate || todayDateInputValue} value={orphanLeaveEndDate} onChange={(e) => { setOrphanLeaveEndDate(e.target.value); setConfirmOrphanTransfer(false); }} required />
                              {orphanLeaveEndDateError && <small className="manager-field-error">{orphanLeaveEndDateError}</small>}
                            </label>
                            <label className="manager-file-upload">
                              <span>Supporting Documents (Optional)</span>
                              <input type="file" multiple onChange={(e) => setOrphanSupportingFiles(Array.from(e.target.files || []))} />
                            </label>
                            {orphanSupportingFiles.length > 0 && (
                              <div className="manager-file-preview-grid">
                                {orphanSupportingFiles.map((file, index) => (
                                  <div className="manager-file-preview" key={`${file.name}-${index}`}>
                    ...
                    <div className="manager-orphan-action-buttons">
                      ...
                      {orphanAgentAction === "long_leave" && orphanLongLeaveStep < 3 ? (
                        <button
                          type="button"
                          className="manager-refresh-btn"
                          disabled={orphanLongLeaveStep === 1 ? !isLongLeaveDetailsValid : !confirmOrphanTransfer}
                          onClick={() => setOrphanLongLeaveStep((current) => Math.min(3, current + 1))}
                        >
                          Next
                        </button>
                      ) : (
