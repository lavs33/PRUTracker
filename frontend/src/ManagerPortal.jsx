...
function ManagerPortal({ roleType }) {
  ...
  const displayedConfirmedProspects = orphanConfirmedProspects.length ? orphanConfirmedProspects : orphanProspectsWithActiveLeads;
              ...
              <section className="manager-panel">
                ...
                {orphanAgentAction ? (
                  <div className="manager-orphan-action-page" ref={orphanActionTopRef}>
                    ...
                    {orphanAgentAction === "retired" && (
                      <>
                        ...
                        {orphanRetirementStep === 2 && (
                          <div className="manager-orphan-confirm-step">
                            <div className="manager-section-subhead"><h3>Confirm Orphan Clients</h3><p>Prospects and selected policyholders affected by this retirement.</p></div>
                            <label className="manager-checkbox-row"><input type="checkbox" checked={includeOngoingPolicyholders} onChange={(e) => { setIncludeOngoingPolicyholders(e.target.checked); setConfirmOrphanTransfer(false); }} /> Include affected policyholders with ongoing policies</label>
                            <div className="manager-orphan-confirm-list"><h4>Affected Prospects with Active Leads</h4>{step2Prospects.length ? step2Prospects.map((prospect) => <article key={prospect.id || prospect.prospectId || prospect.code}><strong>{prospect.name || prospect.code || "Prospect"}</strong><span>{prospect.code || prospect.leadCode || "—"}</span></article>) : <p>No active-lead prospects found.</p>}</div>
                            {includeOngoingPolicyholders && <div className="manager-orphan-confirm-list"><h4>Affected Policyholders with Ongoing Policies</h4>{step2Policyholders.length ? step2Policyholders.map((policyholder) => <article key={policyholder.id || policyholder.policyholderId || policyholder.code}><strong>{policyholder.name || policyholder.code || "Policyholder"}</strong><span>{policyholder.code || "—"}</span></article>) : <p>No ongoing policyholders found.</p>}</div>}
                            <label className="manager-checkbox-row"><input type="checkbox" checked={confirmOrphanTransfer} onChange={(e) => setConfirmOrphanTransfer(e.target.checked)} /> I confirm these orphan clients are ready for endorsement.</label>
                            {orphanLongLeaveFieldErrors.form && <small className="manager-field-error">{orphanLongLeaveFieldErrors.form}</small>}
                          </div>
                        )}
                    ...
                    <div className="manager-orphan-action-buttons">
                      {!(orphanAgentAction === "long_leave" && (orphanLongLeaveStep === 2 || orphanLongLeaveStep === 3)) && (
                        <button
                          type="button"
                          className="manager-refresh-btn"
                      ...
                      {(orphanAgentAction === "long_leave" && orphanLongLeaveStep < 3) || (orphanAgentAction === "retired" && orphanRetirementStep < 3) ? (
                        <button
                          ...
                          disabled={orphanLongLeaveSaving || (orphanAgentAction === "long_leave" ? (isLongLeaveReadOnly || (orphanLongLeaveStep === 1 ? (Boolean(orphanLeaveEndDateError) || (Boolean(orphanLongLeaveId) && !orphanLongLeaveDetailsDirty)) : !confirmOrphanTransfer)) : (orphanRetirementStep === 1 ? (!orphanRetirementDate || !orphanRetirementLetter || !orphanApprovedRetirementProof || Boolean(orphanRetirementDateError)) : !confirmOrphanTransfer))}
                          onClick={orphanAgentAction === "long_leave" ? goToNextLongLeaveStep : goToNextRetirementStep}
                        >
                          {orphanLongLeaveSaving ? "Saving..." : (orphanAgentAction === "long_leave" ? (orphanLongLeaveStep === 1 && orphanLongLeaveId ? "Save Details" : (orphanLongLeaveStep === 2 ? "Confirm Orphans" : "Next")) : (orphanRetirementStep === 2 ? "Confirm Orphans" : "Next"))}
                        </button>
                      ) : (
                        <button
                          type="button"
