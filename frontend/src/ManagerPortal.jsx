...
const KPI_DATE_PRESETS = [
  { value: "DEFAULT", label: "Default by KPI Assignment" },
  { value: "ALL", label: "All Time" },
...
function getKpiPresetLabel(value) {
  return (
    KPI_DATE_PRESETS.find((option) => option.value === value)?.label || "Default by KPI Assignment"
  );
}
...
function ManagerPortal({ roleType }) {
  ...
  const [kpiDatePreset, setKpiDatePreset] = useState("DEFAULT");
  ...
  const branchKpiProgressRows = useMemo(() => {
    ...
    const resolveRowsForKpi = (kpi) => (
      kpiDatePreset === "DEFAULT"
        ? portalData?.kpiSalesRowsByFrequency?.[kpi.period] || []
        : portalData?.kpiSalesRows || []
    );
    ...
    return (branchAssignment.kpis || [])
      ...
      .map((kpi) => {
        ...
        return { assignment: branchAssignment, kpi, actual: actualByKey[kpi.key] || 0 };
      });
  }, [kpiData?.assignments, kpiDatePreset, portalData?.kpiSalesRows, portalData?.kpiSalesRowsByFrequency]);

  const branchKpiUnitRows = useMemo(() => {
    const byUnit = new Map();
    const drilldownRows = kpiDatePreset === "DEFAULT" ? (portalData?.kpiSalesRowsByFrequency?.Monthly || []) : (portalData?.kpiSalesRows || []);
  ...
  }, [kpiDatePreset, portalData?.kpiSalesRows, portalData?.kpiSalesRowsByFrequency]);
  ...
  const kpiPeriodLabel =
    portalData?.reportContext?.kpiPeriodLabel ||
    getKpiPresetLabel(kpiDatePreset);
  ...
  const generateKpiPdfReport = () => {
    const kpiTableRows = branchKpiProgressRows.map(({ assignment, kpi, actual }) => {
    ...
    createPrintableReport({
      ...
      details: reportDetails,
      filters: [
        { label: "Date Range Filter", value: kpiPeriodLabel },
        { label: "Report Scope", value: scopeLabel },
      ...
      statCards: [
        ...
        { label: "KPI Date Range", value: kpiPeriodLabel, tone: "gold" },
      ],
      analyticsSections: [
        {
          title: "Unit Drilldown",
          rows: branchKpiUnitRows.length
            ? branchKpiUnitRows.map((unit) => ({
                label: unit.unit,
                value: `${formatMoney(unit.annualPremium)} • ${unit.activeAgents} active agents • ${unit.persistencyRate}% persistency`,
              }))
            : [{ label: "No unit rows", value: "No unit KPI activity in this date range." }],
        },
      ],
      tableSections: [
        {
    ...
    <div className="manager-portal">
      ...
      <div className="manager-portal__body">
        ...
        <main className="manager-portal__content">
          {!activeView.startsWith("kpi_") && <section className="manager-hero">
            <div>
          ...
          {!isLoading && !loadError && activeView === "kpi_progress" && (
            <section className="manager-panel">
              ...
              <div className="manager-toolbar manager-kpi-toolbar">
                <div className="manager-toolbar__filters">
                  <label className="manager-select" htmlFor="manager-kpi-date-preset">
                    <span>Date Range</span>
                    <select id="manager-kpi-date-preset" value={kpiDatePreset} onChange={(e) => setKpiDatePreset(e.target.value)}>
                      {KPI_DATE_PRESETS.map((option) => (
              ...
              <div className="manager-filter-note">
                Showing <strong>{kpiPeriodLabel}</strong> KPI progress data. This filter only changes dashboard/report results and does not change KPI assignment frequencies.
              </div>
              {kpiLoading && <div className="manager-empty-state">Loading KPI progress...</div>}
              {kpiMessage && <div className="manager-filter-note">{kpiMessage}</div>}
              <div className="manager-kpi-progress-grid">
                {branchKpiProgressRows.map(({ assignment, kpi, actual }) => {
                  const comparison = getKpiComparison(actual, kpi);
