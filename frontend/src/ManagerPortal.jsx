...
function ManagerPortal({ roleType }) {
  ...
  useEffect(() => {
    if (!user?.id || user.role !== normalizedRole) return;
    if (!["dashboard", "agents", "kpi_assignment", "kpi_progress"].includes(activeView)) return;
  ...
  const branchKpiPeriodLabel = useMemo(() => {
  ...
  const topBranchSalesAgents = useMemo(() => {
    const rows = portalData?.kpiSalesRowsByFrequency?.[branchKpiPeriod] || [];
    return [...rows]
      .filter((row) => Number(row?.annualPremium || 0) > 0)
      .sort((left, right) => Number(right?.annualPremium || 0) - Number(left?.annualPremium || 0) || String(left?.username || "").localeCompare(String(right?.username || ""), undefined, { numeric: true, sensitivity: "base" }))
      .slice(0, 5);
  }, [branchKpiPeriod, portalData?.kpiSalesRowsByFrequency]);
  ...
  const generateKpiPdfReport = () => {
    ...
    createPrintableReport({
      filename: `${user?.username || normalizedRole} - ${scope.branchName || "Branch"} KPI Progress Report`,
      title: "Branch KPI Progress Report",
      periodLabel: branchKpiPeriodLabel,
      ...
      filters: [
        ...
        { label: "Top Contributing Agents", value: String(topBranchSalesAgents.length) },
      ],
      statCards: [
        { label: "Sales Production", value: `${formatMoney(salesProductionActual)} • Target Achievement: ${targetAchievementActual}%`, tone: "red" },
      ...
      tableSections: [
        ...
        },
        {
          ...
          rows: branchKpiUnitRows.map((unit) => ({
            ...
            targetAchievement: `${unit.targetAchievementIndex}%`,
        ...
        {
          title: "Top 5 Contributing Agents for Sales Production",
          columns: [
          ...
          rows: topBranchSalesAgents.map((agent) => ({
        ...
        <main className="manager-portal__content">
          {activeView === "dashboard" && <section className="manager-hero">
            <div>
              ...
              <h1>{normalizedRole === "BM" ? `${scope.branchName || "Branch"} Branch Overview` : getPortalHeading(normalizedRole)}</h1>
              <p>
                {normalizedRole === "BM"
                  ...
                  : `Monitor ${scopeLabel} with live backend metrics, unit-wide agent coverage, auto-updating date-filtered tables, and printable reports.`}
              </p>
            ...
            <section className="manager-panel">
              ...
              <div className="manager-toolbar manager-kpi-toolbar manager-kpi-toolbar--end">
                ...
                <button type="button" className="manager-report-btn" onClick={generateKpiPdfReport}>
                  <FaFilePdf size={15} />
                  <span>Generate Branch KPI Report (PDF)</span>
                </button>
              </div>
                ...
                <div className="manager-kpi-unit-grid">
                  {branchKpiUnitRows.map((unit) => (
                    <article key={unit.unit}>
                      <strong>{unit.unit}</strong>
                      <span>Sales Production: {formatMoney(unit.annualPremium)} • Target Achievement: {unit.targetAchievementIndex}%</span>
              ...
              <div className="manager-kpi-unit-drilldown">
                <h3>Top 5 Contributing Agents for Sales Production</h3>
                {topBranchSalesAgents.length ? (
                  <ul className="manager-kpi-agent-list">
                    {topBranchSalesAgents.map((agent) => (
                      <li key={`branch-top-agent:${agent.id}`}>
                        <button type="button" className="manager-kpi-agent-link" onClick={() => openAgentDetails(agent.id)}>
                          <strong>{agent.username}</strong>
                          <span>{agent.name || agent.username} • {agent.unit || "—"}</span>
                        </button>
                        <b>{formatMoney(agent.annualPremium)}</b>
                      </li>
                    ))}
                  ...
                  <div className="manager-empty-state">No contributing agents for this date range.</div>
                )}
              </div>
                </>
              ) : null}
