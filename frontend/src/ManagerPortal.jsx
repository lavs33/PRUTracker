...
function ManagerPortal({ roleType }) {
  ...
  }, [kpiData?.assignments, portalData?.kpiSalesRowsByFrequency, selectedAgent, selectedKpiPeriod]);
  ...
  const generateAgentDetailsPdfReport = () => {
    ...
    createPrintableReport({
      ...
      analyticsSections: selectedKpiPeriod
        ? [
            {
              title: "Agent KPI Progress",
              rows: selectedAgentKpiCards.length
                ? selectedAgentKpiCards.map(({ kpi, actual }) => ({
                    label: formatKpiLabel(kpi, "AGENT"),
                    value: `Actual ${formatActualKpiValue(actual, kpi.valueType)} • Target ${formatKpiTarget(kpi)}`,
                  }))
                : [{ label: "Agent KPIs", value: "No agent KPIs assigned." }],
            },
          ]
        : [],
      tableSections: [],
                ...
                {selectedKpiPeriod && (
                  <div className="manager-agent-kpi-section">
                    ...
                    {selectedAgentKpiCards.length ? (
                      ...
                      <div className="manager-empty-state">No agent KPIs assigned.</div>
                    )}
                  </div>
                )}
              </section>
