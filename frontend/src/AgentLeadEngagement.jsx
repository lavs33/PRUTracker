...
function AgentLeadEngagement() {
  ...
  const onSaveNeedsAssessment = async () => {
    ...
    if (currentPriority === "Investment") {
      ...
      const allocations = np?.investment?.fundChoice?.allocations && typeof np.investment.fundChoice.allocations === "object"
        ...
        : {};
      const allowedRatings = SUITABLE_RISK_RATINGS_BY_CATEGORY[riskProfileCategory] || [];
      ...
      if (selectedFunds.length === 0) {
        setNeedsAssessmentError("Fund Choice: select at least one fund.");
        return;
      }
      if (Math.abs(totalAllocation - 100) > 0.0001) {
        setNeedsAssessmentError("Fund Choice: allocation in percentage must equal 100%.");
        return;
      }
      if (fundMatch === "No" && !mismatchReason) {
        setNeedsAssessmentError("Fund Choice: reason for mismatch is required when Fund Match is No.");
        return;
      }
    }
  ...
  const attendanceProofErrorMessages = [
  ...
  const attendanceProofInlineError = attendanceProofErrorMessages.includes(String(needsAssessmentError || ""))
    ? String(needsAssessmentError || "")
    : "";
  const showTopNeedsAssessmentError = Boolean(needsAssessmentError) && !attendanceProofInlineError;
