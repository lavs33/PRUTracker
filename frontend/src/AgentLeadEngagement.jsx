...
function AgentLeadEngagement() {
  ...
  const hasSavedApplicationPremiumPaymentTransfer = useMemo(() => {
    const hasAnnual = toNonNegativeNumber(applicationPremiumPaymentForm.totalAnnualPremiumPhp) !== null;
    const hasFrequency = toNonNegativeNumber(applicationPremiumPaymentForm.totalFrequencyPremiumPhp) !== null;
  ...
  const onApplicationPaymentProofPicked = (file) => {
    ...
    const looksImage = /^image\/(jpeg|png)$/i.test(String(file.type || "")) || /\.(jpe?g|png)$/i.test(String(file.name || ""));
    if (!looksImage) {
      setApplicationPremiumPaymentError("Proof of payment must be a JPG, JPEG, or PNG image.");
      return;
    }
    ...
    reader.onerror = () => {
      setApplicationPremiumPaymentError("Failed to read proof of payment image.");
    };
    reader.readAsDataURL(file);
  };
  ...
  const submitApplicationPremiumPaymentTransfer = async () => {
    try {
      ...
      const totalAnnualPremiumPhp = toNonNegativeNumber(applicationPremiumPaymentForm.totalAnnualPremiumPhp);
      const totalFrequencyPremiumPhp = toNonNegativeNumber(applicationPremiumPaymentForm.totalFrequencyPremiumPhp);
      ...
      if (totalAnnualPremiumPhp === null) {
        setApplicationPremiumPaymentError("Total annual premium is required.");
        return;
      }
      if (totalFrequencyPremiumPhp === null) {
        setApplicationPremiumPaymentError("Total requested-frequency premium is required.");
        return;
      }
      if (!allowedPaymentMethods.includes(methodForRenewalPayment)) {
        setApplicationPremiumPaymentError("Method for renewal payment is required.");
        return;
      }
      if (!paymentProofImageDataUrl) {
        setApplicationPremiumPaymentError("Proof of payment image is required.");
        return;
      }
      ...
      setApplicationPremiumPaymentError(err?.message || "Failed to save premium payment transfer.");
    } finally {
                ...
                <section className="le-card">
                  ...
                  {showApplicationPanel && (
                    <>
                      ...
                      {hasSavedApplicationAttendance ? (
                        <div className="le-block">
                          <h4 className="le-blockTitle">Record Premium Payment Transfer</h4>
                          ...
                          {hasSavedApplicationPremiumPaymentTransfer ? (
                            <>
                              ...
                              <div className="le-formRow">
                                <label className="le-label">Total {requestedFrequencyFromNeedsAssessment || "Requested Frequency"} Premium (Php)</label>
                            ...
                            <>
                              ...
                              <div className="le-formRow">
                                ...
                                <input
                                  ...
                                  onChange={(e) => setApplicationPremiumPaymentForm((f) => ({ ...f, totalAnnualPremiumPhp: e.target.value }))}
                                  disabled={applicationPremiumPaymentSaving}
                                />
                              </div>

                              <div className="le-formRow">
                                <label className="le-label">Total {requestedFrequencyFromNeedsAssessment || "Requested Frequency"} Premium (in Php) *</label>
                                <input
                                  className="le-input"
                                  inputMode="decimal"
                                  value={applicationPremiumPaymentForm.totalFrequencyPremiumPhp}
                                  onChange={(e) => setApplicationPremiumPaymentForm((f) => ({ ...f, totalFrequencyPremiumPhp: e.target.value }))}
                                  disabled={applicationPremiumPaymentSaving}
                                />
                              </div>
                              ...
                              <div className="le-formRow">
                                ...
                                <select
                                  ...
                                  onChange={(e) => setApplicationPremiumPaymentForm((f) => ({ ...f, methodForRenewalPayment: e.target.value }))}
                                  disabled={applicationPremiumPaymentSaving}
                                >
                              ...
                              <div className="le-formRow">
                                ...
                                <input
                                  ...
                                  className="le-input"
                                  accept="image/jpeg,image/png,.jpg,.jpeg,.png"
                                  onChange={(e) => onApplicationPaymentProofPicked(e.target.files?.[0] || null)}
                                  disabled={applicationPremiumPaymentSaving}
                                />
