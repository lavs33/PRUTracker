...
function AgentLeadEngagement() {
  ...
  const displayedScheduledMeetingAttempts = useMemo(() => {
    if (!historyStageView) {
      return scheduledMeetingAttempts.filter((m) => Number(m?.attemptCycle || currentAttemptCycle) === currentAttemptCycle);
    }
    const selected = Number(selectedHistoryCycle || 0);
    if (!Number.isFinite(selected) || selected <= 0) return scheduledMeetingAttempts;
    return scheduledMeetingAttempts.filter((m) => Number(m?.attemptCycle || selected) === selected);
  }, [historyStageView, scheduledMeetingAttempts, selectedHistoryCycle, currentAttemptCycle]);
  ...
  const currentAttempts = useMemo(
    () => attempts.filter((a) => Number(a?.attemptCycle || 1) === currentAttemptCycle),
