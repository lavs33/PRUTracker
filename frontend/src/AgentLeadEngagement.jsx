...
function AgentLeadEngagement() {
  ...
  const fetchEngagement = useCallback(
    async (signal) => {
      ...
      setProposalCurrentActivityKey(String(proposal?.currentActivityKey || data?.engagement?.currentActivityKey || "Generate Proposal").trim() || "Generate Proposal");
      setProposalGenerateForm({
        ...
        generatedAt: generate?.generatedAt || "",
      });
    },
  ...
  const proposalUiActivityKey = useMemo(() => {
    ...
    const raw = String(proposalCurrentActivityKey || engagement?.currentActivityKey || fallback).trim();
    return PROPOSAL_STEPS_UI.some((s) => s.key === raw) ? raw : fallback;
  }, [proposalCurrentActivityKey, engagement?.currentActivityKey, PROPOSAL_STEPS_UI]);
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/prospects/${prospectId}/leads/${leadId}/proposal/generate?userId=${user.id}`, {
        ...
        body: JSON.stringify({
          ...
          chosenProductName: proposalGenerateForm.chosenProductName,
          chosenProductDescription: proposalGenerateForm.chosenProductDescription,
                      ...
                      {showProposalPanel && (
                        <div className="le-block">
                          {proposalUiActivityKey !== "Generate Proposal" ? (
                            <p className="le-smallNote" style={{ color: "#0f766e", marginBottom: 10 }}>
                              Generate Proposal completed.
                            </p>
                          ) : null}

                          <h4 className="le-blockTitle">Generate Proposal</h4>
                          <p className="le-smallNote" style={{ marginBottom: 10 }}>
                            Complete this first before proceeding to proposal presentation attendance and presentation activities.
                          </p>

                          <div className="le-attemptMeta">
                            {String(proposalGenerateForm.chosenProductName || "").trim() ? (
                              <div>
                                <span className="le-metaLabel">Chosen Product</span>
                                <span className="le-metaValue">{proposalGenerateForm.chosenProductName}</span>
                              </div>
                            ) : (
                              <div>
                                <span className="le-metaLabel">Chosen Product</span>
                                <span className="le-metaValue">No selected product found from Needs Assessment.</span>
                              </div>
                            )}
                            {String(proposalGenerateForm.chosenProductDescription || "").trim() ? (
                              <div>
                                <span className="le-metaLabel">Product Description</span>
                                <span className="le-metaValue">{proposalGenerateForm.chosenProductDescription}</span>
                              </div>
                            ) : null}
                            <div>
                              <span className="le-metaLabel">Proposal Generation</span>
                              <span className="le-metaValue">
                          ...
                          {proposalUiActivityKey === "Generate Proposal" && isProposalEditableNow ? (
                            <>
                              ...
                              <div className="le-formRow">
                                <label className="le-check">
                                  <input
                                    ...
                                    disabled={proposalGenerateSaving}
                                  />
                                  <span>I confirm this proposal was sent to prospect email ({prospect?.email || "no email provided"}). *</span>
                                </label>
                              ...
                              <div className="le-actions">
                                ...
                                <button
                                  ...
                                  disabled={proposalGenerateSaving || !String(proposalGenerateForm.chosenProductName || "").trim()}
                                >
                                  {proposalGenerateSaving ? "Saving..." : "Complete Generate Proposal"}
                                </button>
                              </div>
                            ...
                            <div className="le-attemptItem" style={{ marginTop: 10 }}>
                              <div className="le-attemptSectionHeader">Generated Proposal Details</div>
                              <div className="le-attemptMeta">
                                {String(proposalGenerateForm.proposalFileName || "").trim() ? <div><span className="le-metaLabel">Proposal File</span><span className="le-metaValue">{proposalGenerateForm.proposalFileName}</span></div> : null}
                                {proposalGenerateForm.generatedAt ? <div><span className="le-metaLabel">Generated At</span><span className="le-metaValue">{formatDateTime(proposalGenerateForm.generatedAt)}</span></div> : null}
                                <div><span className="le-metaLabel">Sent to Prospect Email</span><span className="le-metaValue">{proposalGenerateForm.sentToProspectEmail ? "Yes" : "No"}</span></div>
                                {proposalGenerateForm.sentToProspectAt ? <div><span className="le-metaLabel">Sent At</span><span className="le-metaValue">{formatDateTime(proposalGenerateForm.sentToProspectAt)}</span></div> : null}
                                <div><span className="le-metaLabel">Status</span><span className="le-metaValue">{isGenerateProposalCompleted ? "Completed" : "Pending"}</span></div>
                              </div>
                              {proposalGenerateForm.proposalFileDataUrl ? (
