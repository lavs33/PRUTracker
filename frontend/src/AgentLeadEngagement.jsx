...
function AgentLeadEngagement() {
  ...
  const fetchEngagement = useCallback(
    async (signal) => {
      ...
      setProposalPresentationFurtherPromptSaved(false);
  ...
  const hasAddedFollowUpNeedsMeeting = useMemo(() => {
    ...
    if (Number.isFinite(decisionMs) && decisionMs > 0) {
      return scheduledMeetingAttempts.some((meeting) => {
  ...
  const proposalNeedsPrefillKey = latestScheduledMeeting?.meetingAt
    ...
    : "";
  const applicationMeetingSourcePrefillKey = latestOpenOrOverdueProposalMeeting?.startAt
    ? String(latestOpenOrOverdueProposalMeeting?.id || latestOpenOrOverdueProposalMeeting?.createdAt || latestOpenOrOverdueProposalMeeting?.startAt)
    : "";
  ...
  useEffect(() => {
    if (!showProposalPanel || proposalUiActivityKey !== "Schedule Application Submission" || applicationMeetingSaved || !latestOpenOrOverdueProposalMeeting?.startAt || !applicationMeetingSourcePrefillKey) {
      if (applicationMeetingSaved || !showProposalPanel || proposalUiActivityKey !== "Schedule Application Submission") {
        setApplicationMeetingPrefillKey("");
      }
      return;
    }
    if (applicationMeetingPrefillKey === applicationMeetingSourcePrefillKey) return;

    const sourceMeetingDate = toDateInputValue(latestOpenOrOverdueProposalMeeting.startAt);
    ...
    setApplicationMeetingForm((current) => ({
      ...
      meetingStartTime: "",
      meetingDurationMin: Number(latestOpenOrOverdueProposalMeeting?.durationMin || current.meetingDurationMin || 120),
      meetingMode: String(latestOpenOrOverdueProposalMeeting?.mode || current.meetingMode || ""),
      meetingPlatform: String(latestOpenOrOverdueProposalMeeting?.platform || current.meetingPlatform || ""),
      meetingPlatformOther: String(latestOpenOrOverdueProposalMeeting?.platformOther || current.meetingPlatformOther || ""),
      meetingLink: String(latestOpenOrOverdueProposalMeeting?.link || current.meetingLink || ""),
      meetingInviteSent:
        typeof latestOpenOrOverdueProposalMeeting?.inviteSent === "boolean"
          ? latestOpenOrOverdueProposalMeeting.inviteSent
          : Boolean(current.meetingInviteSent),
      meetingPlace: String(latestOpenOrOverdueProposalMeeting?.place || current.meetingPlace || ""),
    }));
    ...
    latestOpenOrOverdueProposalMeeting,
    proposalUiActivityKey,
    showProposalPanel,
  ]);
                ...
                <section className="le-card">
                        ...
                        <div className="le-block">
                          ...
                          {rescheduleFollowUpNeedsMeetingMode || rescheduleFromNeedsMode || contactingRescheduleMode ? (
                            <p className="le-smallNote" style={{ marginTop: 8 }}>
                              This updates the current meeting instead of adding another follow-up meeting.
                            </p>
                          ) : null}
                                ...
                                >
                                  {scheduledMeetingAttempts.length > 1 ? (
                                    <div className="le-attemptSectionHeader">
                                      {idx === 0 ? "Most Recent Meeting" : `Previous Meeting #${scheduledMeetingAttempts.length - idx}`}
                                    </div>
                                  ) : null}
                  ...
                  {showNeedsAssessmentPanel && (
                    <>
                      ...
                      {isNeedsAnalysisViewed && needsAnalysisEditMode && needsAssessmentForm.attendanceChoice === "YES" && String(needsAssessmentForm.attendanceProofImageDataUrl || "").trim() && (
                        <div className="le-block">
                          <div className="le-blockHeader">
