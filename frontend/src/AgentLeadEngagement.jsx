...
function AgentLeadEngagement() {
  ...
  const fetchNeedsAssessment = useCallback(async () => {
    ...
    try {
      ...
      setNeedsAnalysisEditMode(!(String(data?.needsAssessment?.outcomeActivity || "").trim() === "Perform Needs Analysis"));
  ...
  const meetingAttempts = useMemo(() => {
    return (Array.isArray(attempts) ? attempts : []).filter((a) =>
      Boolean(a?.meetingAt) ||
      Boolean(String(a?.meetingMode || "").trim()) ||
      Boolean(String(a?.meetingPlatform || "").trim()) ||
      Boolean(String(a?.meetingPlatformOther || "").trim()) ||
      Boolean(String(a?.meetingLink || "").trim()) ||
      Boolean(String(a?.meetingPlace || "").trim()) ||
      Boolean(Number(a?.meetingDurationMin || 0)) ||
      Boolean(a?.meetingEndAt)
    );
  }, [attempts]);

  const hasSavedContactMeeting = meetingAttempts.length > 0;
                ...
                <section className="le-card">
                      ...
                      {showContactingPanel && !showAddAttempt && isScheduleMeetingViewed && (
                        <div className="le-block">
                          ...
                          {isScheduleMeetingEditable ? (
                            ...
                            <>
                              {meetingAttempts.map((meeting, idx) => (
                                <div key={String(meeting?.attemptId || idx)} className="le-attemptMeta" style={{ marginTop: idx === 0 ? 8 : 14 }}>
                                  {meeting?.meetingAt ? <div><span className="le-metaLabel">Meeting Date & Time</span><span className="le-metaValue">{formatDateTime(meeting.meetingAt)}</span></div> : null}
                                  {Number(meeting?.meetingDurationMin || 0) > 0 ? <div><span className="le-metaLabel">Meeting Duration</span><span className="le-metaValue">{meeting.meetingDurationMin} mins</span></div> : null}
                                  {meeting?.meetingEndAt ? <div><span className="le-metaLabel">Meeting Ends</span><span className="le-metaValue">{formatDateTime(meeting.meetingEndAt)}</span></div> : null}
                                  {String(meeting?.meetingMode || "").trim() ? <div><span className="le-metaLabel">Meeting Mode</span><span className="le-metaValue">{meeting.meetingMode}</span></div> : null}
                                  {String(meeting?.meetingPlatform || "").trim() ? <div><span className="le-metaLabel">Meeting Platform</span><span className="le-metaValue">{meeting.meetingPlatform}</span></div> : null}
                                  {String(meeting?.meetingPlatformOther || "").trim() ? <div><span className="le-metaLabel">Meeting Platform (Other)</span><span className="le-metaValue">{meeting.meetingPlatformOther}</span></div> : null}
                                  {String(meeting?.meetingLink || "").trim() ? <div><span className="le-metaLabel">Meeting Link</span><span className="le-metaValue">{meeting.meetingLink}</span></div> : null}
                                  {String(meeting?.meetingMode || "").trim() === "Online" ? <div><span className="le-metaLabel">Meeting Invite Sent</span><span className="le-metaValue">{meeting?.meetingInviteSent ? "Yes" : "No"}</span></div> : null}
                                  {String(meeting?.meetingPlace || "").trim() ? <div><span className="le-metaLabel">Meeting Place</span><span className="le-metaValue">{meeting.meetingPlace}</span></div> : null}
                                  {String(meeting?.meetingStatus || "").trim() ? <div><span className="le-metaLabel">Status</span><span className="le-metaValue">{meeting.meetingStatus}</span></div> : null}
                                </div>
                              ))}
                  ...
                  {showNeedsAssessmentPanel && (
                    <>
                      ...
                      {isNeedsAnalysisViewed && needsAnalysisEditMode && needsAssessmentForm.attendanceChoice === "YES" && !isNeedsAssessmentLocked && String(needsAssessmentForm.attendanceProofImageDataUrl || "").trim() && (
                        <div className="le-block">
                          <div className="le-blockHeader">
                          ...
                          <div className="le-blockHeader withTopBorder">
                              ...
                              {needsSectionOpen.productSelection && (
                                <>
                                  ...
                                  <p className="le-muted" style={{ marginTop: 4 }}>Recommended Products Based on Priority:</p>
                              ...
                              <div className="le-blockHeader withTopBorder">
                                ...
                                <button type="button" className="le-btn secondary" onClick={() => setNeedsSectionOpen((s) => ({ ...s, optionalRiders: !s.optionalRiders }))}>
                              ...
                              <div className="le-blockHeader withTopBorder">
                                <h4 className="le-blockTitle">Notes (Optional)</h4>
                                <button type="button" className="le-btn secondary" onClick={() => setNeedsSectionOpen((s) => ({ ...s, productRidersNotes: !s.productRidersNotes }))}>
                                  {needsSectionOpen.productRidersNotes ? "▴" : "▾"}
                                </button>
                              </div>
                              {needsSectionOpen.productRidersNotes ? (
                      ...
                      {isNeedsAnalysisViewed && !needsAnalysisEditMode && (
                        <div>
