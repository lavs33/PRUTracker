...
function AgentLeadEngagement() {
  ...
  const productRecommendationView = useMemo(() => {
    ...
    const rows = (availableProductsByPriority || []).map((product) => {
      ...
      const annualThreshold = Number.isFinite(Number(minAnnual?.amount)) ? Number(minAnnual.amount) : (minAnnualTier && Number.isFinite(Number(minAnnualTier?.amount)) ? Number(minAnnualTier.amount) : null);
      ...
      const sumThreshold = Number.isFinite(Number(minSum?.amount)) ? Number(minSum.amount) : (minSumTier && Number.isFinite(Number(minSumTier?.amount)) ? Number(minSumTier.amount) : null);
  ...
  useEffect(() => {
    try {
      if (!leadId) return;
      const payload = { needsAssessmentForm, needsSectionOpen, needsAnalysisEditMode, savedAt: Date.now() };
      sessionStorage.setItem(needsDraftStorageKey, JSON.stringify(payload));
    } catch {}
  }, [leadId, needsDraftStorageKey, needsAssessmentForm, needsSectionOpen, needsAnalysisEditMode]);
                      ...
                      {!isHistoryView && isNeedsAnalysisViewed && needsAnalysisEditMode && needsAssessmentForm.attendanceChoice === "YES" && String(needsAssessmentForm.attendanceProofImageDataUrl || "").trim() && (
                        <div className="le-block">
                          ...
                          <div className="le-blockHeader withTopBorder le-sectionHeader">
                              ...
                              {needsSectionOpen.productSelection && (
                                <>
                                  <p className="le-priorityHeadline">Prospect's financial priority: {needsPrioritiesDerived.priority || "—"}</p>
                                  <div className="le-priorityHeaderRow"><p className="le-priorityHeadline" style={{ marginTop: 4 }}>Recommended Products Based on Priority:</p><button type="button" className="le-btn secondary" onClick={() => setRecommendationRefreshToken((v) => v + 1)}>Refresh</button></div>
                                    ...
                                    <>
                                      <div className="le-attemptList" style={{ marginTop: 8 }}>
                                        {productRecommendationView.recommended.length === 0 ? <p className="le-muted">No recommended products based on current criteria.</p> : productRecommendationView.recommended.map(({ product, annualThreshold, sumThreshold }) => {
                                          ...
                                          return (
                                            <div key={String(product?._id || product?.productName)} className="le-attemptItem le-productCard recommended">
                                      ...
                                      <p className="le-priorityHeadline" style={{ marginTop: 10 }}>Alternative Products Based on Priority:</p>
                                      <div className="le-attemptList" style={{ marginTop: 8 }}>
                              ...
                              <div className="le-blockHeader withTopBorder le-subsectionHeaderBar">
