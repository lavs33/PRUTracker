...
function AgentLeadEngagement() {
  ...
  const showProposalSchedulingSection = needsActivityKeyRaw === "Schedule Proposal Presentation" && !isContactingReadOnly;
  ...
  const submitScheduleProposalPresentation = async () => {
    try {
      ...
      if (!meetingDate) {
        setProposalMeetingError("Meeting date is required.");
        return;
      }
      if (!meetingStartTime) {
        setProposalMeetingError("Meeting start time is required.");
        return;
      }
      if (![30, 60, 90, 120].includes(meetingDurationMin)) {
        setProposalMeetingError("Meeting duration must be 30, 60, 90, or 120 minutes.");
        return;
      }
      if (isSlotBooked(meetingDate, meetingStartTime, meetingDurationMin, proposalMeetingSaved?.startAt)) {
        setProposalMeetingError("Selected time is already booked.");
        return;
      }
      if (!["Online", "Face-to-face"].includes(meetingMode)) {
        setProposalMeetingError("Please select meeting mode.");
        return;
      }
      ...
      if (meetingMode === "Online") {
        ...
        if (!["Zoom", "Google Meet", "Other"].includes(platform)) {
          setProposalMeetingError("Please select online platform.");
          return;
        }
        if (platform === "Other" && !String(proposalMeetingForm.meetingPlatformOther || "").trim()) {
          setProposalMeetingError("Please specify other platform.");
          return;
        }
        const link = String(proposalMeetingForm.meetingLink || "").trim();
        if (!link) {
          setProposalMeetingError("Meeting link is required for online meetings.");
          return;
        }
        if (!isValidHttpUrl(link)) {
          setProposalMeetingError("Meeting link must be a valid http/https URL.");
          return;
        }
        if (proposalMeetingForm.meetingInviteSent !== true) {
          setProposalMeetingError("Meeting invite must be sent before saving.");
          return;
        }
      }

      if (meetingMode === "Face-to-face" && !String(proposalMeetingForm.meetingPlace || "").trim()) {
        setProposalMeetingError("Meeting place is required for face-to-face meetings.");
        return;
      }
...
{showProposalSchedulingSection && (
                        <>
                          <div className="le-block" style={{ marginTop: 16 }}>
                            ...
                            <div className="le-attemptMeta">
                              <div><span className="le-metaLabel">Full Name</span><span className="le-metaValue">{needsAssessmentForm.basicInformation?.fullName || "—"}</span></div>
                              <div><span className="le-metaLabel">Sex</span><span className="le-metaValue">{needsAssessmentForm.basicInformation?.sex || "—"}</span></div>
                              <div><span className="le-metaLabel">Civil Status</span><span className="le-metaValue">{needsAssessmentForm.basicInformation?.civilStatus || "—"}</span></div>
                              <div><span className="le-metaLabel">Birthday</span><span className="le-metaValue">{needsAssessmentForm.basicInformation?.birthday || "—"}</span></div>
                              <div><span className="le-metaLabel">Occupation</span><span className="le-metaValue">{needsAssessmentForm.basicInformation?.occupation || "—"}</span></div>
                              <div><span className="le-metaLabel">Address</span><span className="le-metaValue">{[needsAssessmentForm.basicInformation?.addressLine, needsAssessmentForm.basicInformation?.barangay, needsAssessmentForm.basicInformation?.city === "Other" ? needsAssessmentForm.basicInformation?.otherCity : needsAssessmentForm.basicInformation?.city, needsAssessmentForm.basicInformation?.region, needsAssessmentForm.basicInformation?.zipCode].filter(Boolean).join(", ") || "—"}</span></div>
                            </div>
                            <p className="le-smallNote" style={{ marginTop: 12 }}>Needs & Priorities</p>
                            <div className="le-attemptMeta">
                              <div><span className="le-metaLabel">Current Priority</span><span className="le-metaValue">{needsAssessmentForm.needsPriorities?.currentPriority || "—"}</span></div>
                              <div><span className="le-metaLabel">Income Band</span><span className="le-metaValue">{needsAssessmentForm.needsPriorities?.monthlyIncomeBand || "—"}</span></div>
                              <div><span className="le-metaLabel">Min Premium</span><span className="le-metaValue">{needsAssessmentForm.needsPriorities?.minPremium || "—"}</span></div>
                              <div><span className="le-metaLabel">Max Premium</span><span className="le-metaValue">{needsAssessmentForm.needsPriorities?.maxPremium || "—"}</span></div>
                              <div><span className="le-metaLabel">Selected Product</span><span className="le-metaValue">{(availableProducts || []).find((p) => String(p?._id || "") === String(needsAssessmentForm.needsPriorities?.productSelection?.selectedProductId || ""))?.productName || "—"}</span></div>
                              <div><span className="le-metaLabel">Requested Premium Payment</span><span className="le-metaValue">{needsAssessmentForm.needsPriorities?.productSelection?.requestedPremiumPayment || "—"}</span></div>
                              <div><span className="le-metaLabel">Requested Frequency</span><span className="le-metaValue">{needsAssessmentForm.needsPriorities?.productSelection?.requestedFrequency || "—"}</span></div>
                              <div><span className="le-metaLabel">Initial Payment Method</span><span className="le-metaValue">{needsAssessmentForm.needsPriorities?.productSelection?.methodForInitialPayment || "—"}</span></div>
                            </div>
                          </div>

                          <div className="le-block" style={{ marginTop: 16 }}>
                            <h4 className="le-blockTitle">Schedule Proposal Presentation</h4>
                            {proposalMeetingError ? <p className="le-smallNote" style={{ color: "#DA291C" }}>{proposalMeetingError}</p> : null}
                            ...
                            <div className="le-formRow">
                              ...
                              <input
                                ...
                                onChange={(e) => setProposalMeetingForm((f) => ({ ...f, meetingDate: e.target.value }))}
                                disabled={savingProposalMeeting}
                              />
                            </div>
                            <div className="le-formRow">
                              <label className="le-label">Meeting Time *</label>
                              <select
                                className="le-input"
                                value={proposalMeetingForm.meetingStartTime}
                                onChange={(e) => setProposalMeetingForm((f) => ({ ...f, meetingStartTime: e.target.value }))}
                                disabled={savingProposalMeeting}
                              >
                                <option value="">Select Time</option>
                                {proposalMeetingStartSlots.map((t) => (
                                  <option key={`proposal-${t}`} value={t}>{t}</option>
                                ))}
                              </select>
                            </div>
                            <div className="le-formRow">
                              <label className="le-label">Duration *</label>
                              <select className="le-input" value={proposalMeetingForm.meetingDurationMin} onChange={(e) => setProposalMeetingForm((f) => ({ ...f, meetingDurationMin: Number(e.target.value) }))} disabled={savingProposalMeeting}>
                                <option value={30}>30 mins</option>
                                <option value={60}>60 mins</option>
                                <option value={90}>90 mins</option>
                                <option value={120}>120 mins</option>
                            ...
                            <div className="le-formRow">
                              <label className="le-label">Meeting Mode *</label>
                              <select className="le-input" value={proposalMeetingForm.meetingMode} onChange={(e) => setProposalMeetingForm((f) => ({ ...f, meetingMode: e.target.value, meetingPlatform: "", meetingPlatformOther: "", meetingLink: "", meetingInviteSent: false, meetingPlace: "" }))} disabled={savingProposalMeeting}>
                                <option value="">Select</option>
                                <option value="Online">Online</option>
                                <option value="Face-to-face">Face-to-face</option>
                              </select>
                            ...
                            {proposalMeetingForm.meetingMode === "Online" && (
                              <>
                                <div className="le-formRow">
                                  ...
                                  <select className="le-input" value={proposalMeetingForm.meetingPlatform} onChange={(e) => setProposalMeetingForm((f) => ({ ...f, meetingPlatform: e.target.value }))} disabled={savingProposalMeeting}>
                                    <option value="">Select</option>
                                    <option value="Zoom">Zoom</option>
                                    <option value="Google Meet">Google Meet</option>
                                    <option value="Other">Other</option>
                                ...
                                {proposalMeetingForm.meetingPlatform === "Other" && (
                                  <div className="le-formRow">
                                    ...
                                    <input className="le-input" value={proposalMeetingForm.meetingPlatformOther} onChange={(e) => setProposalMeetingForm((f) => ({ ...f, meetingPlatformOther: e.target.value }))} disabled={savingProposalMeeting} />
                                  </div>
                                )}
                                <div className="le-formRow">
                                  <label className="le-label">Meeting Link *</label>
                                  <input className="le-input" value={proposalMeetingForm.meetingLink} onChange={(e) => setProposalMeetingForm((f) => ({ ...f, meetingLink: e.target.value }))} placeholder="https://..." disabled={savingProposalMeeting} />
                                </div>
                                <div className="le-formRow">
                                  <label className="le-check">
                                    <input type="checkbox" checked={proposalMeetingForm.meetingInviteSent} onChange={(e) => setProposalMeetingForm((f) => ({ ...f, meetingInviteSent: e.target.checked }))} disabled={savingProposalMeeting} />
                                    <span>I confirm invite link has been sent (required)</span>
                                  </label>
                                </div>
                              </>
                            ...
                            {proposalMeetingForm.meetingMode === "Face-to-face" && (
                              <div className="le-formRow">
                                <label className="le-label">Meeting Place *</label>
                                <input className="le-input" value={proposalMeetingForm.meetingPlace} onChange={(e) => setProposalMeetingForm((f) => ({ ...f, meetingPlace: e.target.value }))} disabled={savingProposalMeeting} />
                              </div>
                            )}
                            ...
                            <div className="le-actions">
                              ...
                              <button type="button" className="le-btn primary" onClick={submitScheduleProposalPresentation} disabled={savingProposalMeeting}>
                                {savingProposalMeeting ? "Saving..." : "Save Proposal Schedule"}
                              </button>
                            </div>
                          </div>
                        </>
