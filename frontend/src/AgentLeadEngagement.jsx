...
function AgentLeadEngagement() {
  ...
  const canEditProposalScheduleForm = isNeedsScheduleEditable || isProposalPresentationRescheduleInProgress;
  ...
  const submitProposalPresentationNotes = async () => {
    try {
      ...
      const preservedDecision = String(proposalPresentationForm.proposalAccepted || "").trim().toUpperCase();
      if (["YES", "NO"].includes(preservedDecision)) payload.proposalAccepted = preservedDecision;
      ...
      setProposalPresentationEditSnapshot((prev) => ({
        ...prev,
        proposalAccepted: prev.proposalAccepted,
