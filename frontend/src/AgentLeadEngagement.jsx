...
function AgentLeadEngagement() {
  ...
  const onSaveNeedsAssessment = async () => {
    ...
    if (needsAssessmentForm.attendanceChoice !== "YES") {
      addError("attendanceChoice", "Prospect attendance must be marked YES before saving.");
    }
    ...
    if (!attendanceProofImageDataUrl) {
      addError("attendanceProof", "Please upload a proof of attendance image before proceeding.");
    }
    if (attendanceProofImageDataUrl && !/^data:image\/(?:jpeg|png);base64,/i.test(attendanceProofImageDataUrl)) {
      addError("attendanceProof", "Proof of attendance must be a JPG, JPEG, or PNG image.");
    }
    if (attendanceProofFileName && !/\.(jpe?g|png)$/i.test(attendanceProofFileName)) {
      addError("attendanceProof", "Proof of attendance file type must be JPG, JPEG, or PNG.");
    }
    ...
    if (!civilStatus) addError("civilStatus", "Civil status is required.");
    if (!birthday) addError("birthday", "Birthday is required.");
    if (!["Employed", "Self-Employed", "Not Employed"].includes(occupationCategory)) addError("occupationCategory", "Occupation category is required.");
    if (["Employed", "Self-Employed"].includes(occupationCategory) && !occupation) addError("occupation", "Occupation is required for employed/self-employed prospects.");
    ...
    if (!addressLine) addError("addressLine", "Street address is required.");
    if (!barangay) addError("barangay", "Barangay is required.");
    if (!city) addError("city", "City is required.");
    if (city === "Other" && !otherCity) addError("otherCity", "Other city is required.");
    if (!region) addError("region", "Region is required.");
    if (!zipCode) addError("zipCode", "Zip code is required.");
    if (zipCode && !/^\d{4}$/.test(zipCode)) addError("zipCode", "Zip code must be 4 digits.");

    if (birthday) {
      const b = new Date(birthday);
      const now = new Date();
      if (Number.isNaN(b.getTime())) {
        addError("birthday", "Birthday is invalid.");
      } else {
        const d0 = new Date(b.getFullYear(), b.getMonth(), b.getDate());
        const t0 = new Date(now.getFullYear(), now.getMonth(), now.getDate());
        if (d0 > t0) addError("birthday", "Birthday cannot be in the future.");
        const computed = computeAgeFromBirthday(birthday);
        if (computed === null || computed < 18 || computed > 70) addError("birthday", "Prospect age must be between 18 and 70 years old.");
      }
    }

    if (!Number.isFinite(age) || age < 18 || age > 70) addError("birthday", "Prospect age must be between 18 and 70 years old.");

    for (let i = 0; i < (needsAssessmentForm.dependents || []).length; i += 1) {
      const d = needsAssessmentForm.dependents[i] || {};
      const depAge = Number(d.age);
      if (!String(d.name || "").trim() || !Number.isFinite(depAge) || depAge < 0 || depAge > 120 || !["Male", "Female"].includes(String(d.gender || "")) || !["Child", "Parent", "Sibling"].includes(String(d.relationship || ""))) {
        addError("dependents", `Dependent #${i + 1} has missing/invalid details.`);
      }
    }
    ...
    if (!["Protection", "Health", "Investment"].includes(currentPriority)) addError("currentPriority", "Current priority is required.");
    if (!INCOME_BAND_OPTIONS.some((o) => o.value === monthlyIncomeBand)) addError("monthlyIncomeBand", "Approximate monthly income bracket is required.");
    if (monthlyIncomeBand === "BELOW_15000" && (monthlyIncomeAmount === null || monthlyIncomeAmount >= 15000)) addError("monthlyIncomeAmount", "Manual monthly income must be below Php 15,000 for selected bracket.");
    if (monthlyIncomeBand === "ABOVE_500000" && (monthlyIncomeAmount === null || monthlyIncomeAmount <= 500000)) addError("monthlyIncomeAmount", "Manual monthly income must be above Php 500,000 for selected bracket.");

    const approxIncome = resolveApproxIncome(monthlyIncomeBand, np.monthlyIncomeAmount);
    if (approxIncome === null) addError("monthlyIncomeAmount", "Approximate monthly income amount is required for selected bracket.");

    if (minPremium === null) addError("minPremium", "Minimum willing monthly premium is required.");
    if (maxPremium === null) addError("maxPremium", "Maximum willing monthly premium is required.");
    if (approxIncome !== null && minPremium !== null && minPremium > approxIncome) addError("minPremium", "Minimum willing monthly premium cannot be higher than approximate monthly income.");
    if (approxIncome !== null && maxPremium !== null && maxPremium > approxIncome) addError("maxPremium", "Maximum willing monthly premium cannot be higher than approximate monthly income.");
    if (minPremium !== null && maxPremium !== null && maxPremium < minPremium) addError("maxPremium", "Maximum willing monthly premium must be equal to or higher than minimum.");
    ...
    if (!selectedProductId || !selectedProduct) addError("selectedProductId", "Product Selection: please select a product under the chosen priority.");
    if (!["Monthly", "Quarterly", "Half-yearly", "Yearly"].includes(requestedFrequency)) addError("requestedFrequency", "Product Selection: requested frequency is invalid.");
    if (requestedPremiumPayment === null) addError("requestedPremiumPayment", "Product Selection: requested premium payment is required.");
    if (!["Credit Card / Debit Card", "Mobile Wallet / GCash", "Dated Check", "Bills Payments"].includes(methodForInitialPayment)) addError("methodForInitialPayment", "Product Selection: method for initial payment is required.");
    ...
    if (currentPriority === "Protection") {
      ...
      if (monthlySpend === null) addError("protectionMonthlySpend", "Protection: approximate monthly spend is required.");
      if (approxIncome !== null && monthlySpend !== null && monthlySpend > approxIncome) addError("protectionMonthlySpend", "Protection: monthly spend cannot be higher than approximate monthly income.");
      if (savingsForProtection === null) addError("protectionSavings", "Protection: savings for protection is required.");
    }
    ...
    if (currentPriority === "Health") {
      ...
      if (amountToCoverCriticalIllness === null) addError("healthAmount", "Health: approximate amount to cover critical illness is required.");
      if (savingsForCriticalIllness === null) addError("healthSavings", "Health: savings for critical illness is required.");
      if (amountToCoverCriticalIllness !== null && savingsForCriticalIllness !== null && savingsForCriticalIllness > amountToCoverCriticalIllness) addError("healthSavings", "Health: savings for critical illness cannot be higher than amount to cover critical illness.");
    }
    ...
    if (currentPriority === "Investment") {
      ...
      if (!savingsPlan) addError("investmentSavingsPlan", "Investment: savings plan is required.");
      if (savingsPlan === "Other" && !savingsPlanOther) addError("investmentSavingsPlanOther", "Investment: please specify other savings plan.");
      if (targetSavingsAmount === null) addError("investmentTargetAmount", "Investment: target savings amount is required.");
      if (!Number.isFinite(targetUtilizationYear)) addError("investmentTargetYear", "Investment: target year to utilize savings is required.");
      else {
        const yearNow = new Date().getFullYear();
        if (targetUtilizationYear < yearNow + 2 || targetUtilizationYear > yearNow + 20) addError("investmentTargetYear", "Investment: target year must be between 2 and 20 years from current year.");
      }
      if (savingsForInvestment === null) addError("investmentSavings", "Investment: savings for investment is required.");
      if (targetSavingsAmount !== null && savingsForInvestment !== null && savingsForInvestment > targetSavingsAmount) addError("investmentSavings", "Investment: savings for investment cannot be higher than target savings amount.");

      const scored = scoreRiskProfile(rp);
      if (scored.score === null || !scored.category) addError("investmentRiskProfiler", "Investment Risk Profiler: please answer all survey questions.");
      ...
      if (selectedFunds.length === 0) addError("fundChoice", "Fund Choice: select at least one fund.");
      if (Math.abs(totalAllocation - 100) > 0.0001) addError("fundChoice", "Fund Choice: allocation in percentage must equal 100%.");
      if (fundMatch === "No" && !mismatchReason) addError("fundChoice", "Fund Choice: reason for mismatch is required when Fund Match is No.");
    }

    if (Object.keys(errors).length > 0) {
      setNeedsAssessmentFieldErrors(errors);
      return;
    }
  ...
  const renderNeedsAssessmentError = (fieldKey) => {
    const fieldMsg = String(needsAssessmentFieldErrors?.[fieldKey] || "").trim();
    if (fieldMsg) return <p className="le-smallNote" style={{ color: "#DA291C", marginTop: 6 }}>{fieldMsg}</p>;
    if (needsAssessmentErrorField === fieldKey) return <p className="le-smallNote" style={{ color: "#DA291C", marginTop: 6 }}>{errorText}</p>;
    return null;
  };

  const showTopNeedsAssessmentError = needsAssessmentErrorField === "general" && Object.keys(needsAssessmentFieldErrors || {}).length === 0;
                            ...
                            <div className="le-formRow" style={{ marginTop: 8 }}>
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  if (!extOk || !mimeOk) {
                                    setNeedsAssessmentError("");
                                    setNeedsAssessmentFieldErrors((prev) => ({ ...prev, attendanceProof: "Proof of attendance must be a JPG, JPEG, or PNG image." }));
                                    setNeedsAssessmentForm((f) => ({ ...f, attendanceProofImageDataUrl: "", attendanceProofFileName: "" }));
                                    e.target.value = "";
                                    return;
                                  }
                                  ...
                                  reader.onload = () => {
                                    setNeedsAssessmentError("");
                                    setNeedsAssessmentFieldErrors((prev) => ({ ...prev, attendanceProof: "" }));
                                    setNeedsAssessmentForm((f) => ({
                                      ...f,
