...
function AgentLeadEngagement() {
  ...
  const renderTierAmount = (value) => (Number.isFinite(Number(value)) ? `Php ${formatAmount(Number(value))}` : "No Standard");
                          ...
                          <div className="le-blockHeader withTopBorder le-sectionHeader">
                              ...
                              {needsSectionOpen.productSelection && (
                                <>
                                  ...
                                  <div className="le-priorityHeaderRow le-priorityHeaderSpaced"><p className="le-priorityHeadline le-priorityHeadlineMedium">Recommended Products Based on Priority:</p><button type="button" className="le-btn secondary" onClick={() => setRecommendationRefreshToken((v) => v + 1)}>Refresh</button></div>
                                        ...
                                        {productRecommendationView.recommended.length === 0 ? <p className="le-muted">No recommended products based on current criteria.</p> : productRecommendationView.recommended.map(({ product, annualThreshold, sumThreshold, minAnnual, minSum }) => {
                                          ...
                                          return (
                                            <div key={String(product?._id || product?.productName)} className="le-attemptItem le-productCard alternative">
                                              ...
                                              {Array.isArray(minAnnual?.tiers) && minAnnual.tiers.length > 0 ? <div className="le-tierBlock"><p className="le-smallNote le-tierTitle">Annual Premium Tiers</p>{minAnnual.tiers.map((t, idx) => <p key={`ann-tier-${idx}`} className="le-smallNote le-tierRow">• Ages {t?.minAge ?? "—"}-{t?.maxAge ?? "—"}: {renderTierAmount(t?.amount)} <span className="le-tierEq">(Monthly Eq: {Number.isFinite(Number(t?.amount)) ? `Php ${formatAmount(Number(t.amount) / 12)}` : "No Standard"})</span></p>)}</div> : null}
                                              {Array.isArray(minSum?.tiers) && minSum.tiers.length > 0 ? <div className="le-tierBlock"><p className="le-smallNote le-tierTitle">Sum Assured Tiers</p>{minSum.tiers.map((t, idx) => <p key={`sum-tier-${idx}`} className="le-smallNote le-tierRow">• Ages {t?.minAge ?? "—"}-{t?.maxAge ?? "—"}: {renderTierAmount(t?.amount)}</p>)}</div> : null}
                                              <div className="le-actions" style={{ marginTop: 8 }}>
                                        ...
                                        {productRecommendationView.alternatives.length === 0 ? <p className="le-muted">No alternative products to show.</p> : productRecommendationView.alternatives.map(({ product, annualThreshold, sumThreshold, reasons, minAnnual, minSum }) => {
                                          ...
                                          return (
                                            <div key={`alt-${String(product?._id || product?.productName)}`} className="le-attemptItem le-productCard recommended">
                                              ...
                                              <p className="le-smallNote">Minimum Monthly Premium: {Number.isFinite(annualThreshold) ? `Php ${formatAmount(annualThreshold / 12)}` : "No Standard"}</p>
                                              {Array.isArray(minAnnual?.tiers) && minAnnual.tiers.length > 0 ? <div className="le-tierBlock"><p className="le-smallNote le-tierTitle">Annual Premium Tiers</p>{minAnnual.tiers.map((t, idx) => <p key={`alt-ann-tier-${idx}`} className="le-smallNote le-tierRow">• Ages {t?.minAge ?? "—"}-{t?.maxAge ?? "—"}: {renderTierAmount(t?.amount)} <span className="le-tierEq">(Monthly Eq: {Number.isFinite(Number(t?.amount)) ? `Php ${formatAmount(Number(t.amount) / 12)}` : "No Standard"})</span></p>)}</div> : null}
                                              {Array.isArray(minSum?.tiers) && minSum.tiers.length > 0 ? <div className="le-tierBlock"><p className="le-smallNote le-tierTitle">Sum Assured Tiers</p>{minSum.tiers.map((t, idx) => <p key={`alt-sum-tier-${idx}`} className="le-smallNote le-tierRow">• Ages {t?.minAge ?? "—"}-{t?.maxAge ?? "—"}: {renderTierAmount(t?.amount)}</p>)}</div> : null}
