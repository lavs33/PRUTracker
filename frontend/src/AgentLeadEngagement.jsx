      ...
      try {
        ...
        if (rawDraft) {
          ...
          const hasMeaningfulDraft = Boolean(
            draftForm && (
              String(draftForm?.basicInformation?.civilStatus || "").trim() ||
              String(draftForm?.basicInformation?.sex || "").trim() ||
              String(draftForm?.basicInformation?.birthday || "").trim() ||
              String(draftForm?.needsPriorities?.currentPriority || "").trim() ||
              (Array.isArray(draftForm?.dependents) && draftForm.dependents.length > 0)
            )
          );
          const canApplyDraft = Boolean(draftForm) && hasMeaningfulDraft;
          if (canApplyDraft) {
                      ...
                      {!isHistoryView && isNeedsAnalysisViewed && needsAnalysisEditMode && needsAssessmentForm.attendanceChoice === "YES" && String(needsAssessmentForm.attendanceProofImageDataUrl || "").trim() && (
                        <div className="le-block">
                          ...
                          <div className="le-blockHeader withTopBorder le-sectionHeader">
                              ...
                              {needsSectionOpen.productSelection && (
                                <>
                                  <p className="le-priorityHeadline le-priorityHeadlineMedium">Prospect's financial priority: {needsPrioritiesDerived.priority || "—"}</p>
