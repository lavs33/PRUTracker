...
function AgentLeadEngagement() {
  ...
  const applicationMeetingMinimumDateFromProposal = useMemo(() => {
    const today = toLocalDateInputValue(new Date());
    if (!latestProposalMeeting?.startAt) return today;
    const openProposalDate = toDateInputValue(latestProposalMeeting.startAt);
  ...
  const applicationMeetingStartSlotsFiltered = useMemo(() => {
    ...
    let minMinutes = null;
    if (latestProposalMeeting?.startAt) {
  ...
  }, [applicationMeetingForm.meetingDate, applicationMeetingStartSlots, getNextHalfHourSlotAfterNow, latestProposalMeeting?.durationMin, latestProposalMeeting?.endAt, latestProposalMeeting?.startAt]);
  ...
  useEffect(() => {
    if (!showProposalPanel || proposalUiActivityKey !== "Schedule Application Submission" || applicationMeetingSaved || !latestProposalMeeting?.startAt || !applicationMeetingSourcePrefillKey) {
