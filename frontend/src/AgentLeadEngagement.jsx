...
function AgentLeadEngagement() {
  ...
  const [proposalAttendanceNoRescheduleConsumed, setProposalAttendanceNoRescheduleConsumed] = useState(false);
  ...
  const canRescheduleFromLatestProposalAttendanceNo = useMemo(() => {
    ...
    if (proposalAttendanceNoRescheduleConsumed) return false;
    const attendanceNoAtMs = proposalAttendanceForm?.attendedAt ? new Date(proposalAttendanceForm.attendedAt).getTime() : NaN;
  ...
  }, [isProposalAttendanceNoRescheduleMode, proposalAttendanceNoRescheduleConsumed, proposalAttendanceForm?.attendedAt, proposalMeetingSaved?.createdAt, proposalMeetingSaved?.updatedAt, proposalMeetingSaved?.startAt]);
  ...
  const isNeedsScheduleEditable =
    ((isNeedsAssessmentCurrentViewEditable && needsActivityKeyRaw === "Schedule Proposal Presentation") || canRescheduleFromLatestProposalAttendanceNo) &&
  ...
  const submitScheduleProposalPresentation = async () => {
    try {
      ...
      setProposalMeetingRescheduleOriginal(null);
      setProposalMeetingScheduleMode("");
      setProposalAttendanceNoRescheduleConsumed(true);
      setProposalAttendanceForm({
        attendanceChoice: "",
        attendanceProofImageDataUrl: "",
        attendanceProofFileName: "",
  ...
  const submitProposalAttendance = async (attendedOverride = null) => {
    try {
      ...
      setProposalAttendanceProofEditMode(false);
      if (!attended) {
        setProposalAttendanceNoRescheduleConsumed(false);
        await fetchNeedsAssessment();
        await fetchEngagement();
              ...
              <div className="le-mainRow">
                <section className="le-card">
                  ...
                  {showContactingPanel && !showAddAttempt && isValidateContactViewed && isEngagementBlocked && (
                      ...
                      {showProposalPanel && isProposalScheduleApplicationViewed && isHistoryView && !applicationMeetingSaved ? (
                        <div className="le-block"><p className="le-muted" style={{ marginTop: 8 }}>No details were saved for this subactivity in the selected engagement cycle.</p></div>
                      ) : null}
                      {showProposalPanel && isProposalScheduleApplicationViewed && (!isHistoryView || applicationMeetingSaved) ? (
                        <div className="le-block">
