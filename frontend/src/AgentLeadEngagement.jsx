...
function AgentLeadEngagement() {
  ...
  const [needsAssessmentActivityKey, setNeedsAssessmentActivityKey] = useState("Record Prospect Attendance");
  ...
  const fetchNeedsAssessment = useCallback(async () => {
    ...
    try {
      ...
      setNeedsAssessmentActivityKey(String(data?.needsAssessment?.currentActivityKey || "Record Prospect Attendance"));
  ...
  const onSaveNeedsAttendance = async () => {
  ...
  const onScheduleProposalPresentation = async () => {
    setNeedsAssessmentSavedAt("");
    setNeedsAssessmentError("");
    ...
    if (!meetingDate || !meetingStartTime) {
      setNeedsAssessmentError("Meeting date and start time are required.");
      return;
    }
    if (![30, 60, 90, 120].includes(meetingDurationMin)) {
      setNeedsAssessmentError("Duration must be 30, 60, 90, or 120 minutes.");
      return;
    }
    if (!['Online', 'Face-to-face'].includes(meetingMode)) {
      setNeedsAssessmentError("Meeting mode is required.");
      return;
    }

    try {
      setNeedsAssessmentSaving(true);
      const res = await fetch(`${API_BASE}/api/prospects/${prospectId}/leads/${leadId}/needs-assessment/schedule-proposal?userId=${user.id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          meetingDate,
          ...
          meetingPlatformOther: meetingMode === "Online" && meetingForm.meetingPlatform === "Other" ? String(meetingForm.meetingPlatformOther || "").trim() : undefined,
      ...
      setNeedsAssessmentError(err?.message || "Failed to schedule proposal presentation.");
    } finally {
      setNeedsAssessmentSaving(false);
    }
  };
  ...
  const onSaveNeedsAssessment = async () => {
    ...
    if (needsAssessmentForm.attendanceChoice !== "YES") {
      setNeedsAssessmentError("Prospect attendance is required and must be answered as attended before proceeding.");
      return;
    }
  ...
  const needsActivityKeyRaw = String(needsAssessmentActivityKey || "Record Prospect Attendance").trim();
  const stageActivityBadge =
    showContactingPanel && isViewingCurrentStage
      ? currentActivityLabel
      : showNeedsAssessmentPanel && isViewingCurrentStage
      ? needsActivityKeyRaw
      : "";
                ...
                <section className="le-card">
                  ...
                  {showNeedsAssessmentPanel && (
                    <>
                      ...
                      {needsAssessmentForm.attendanceChoice === "YES" && needsAssessmentActivityKey === "Record Prospect Attendance" && isNeedsAssessmentEditableNow && (
                        <div className="le-actions" style={{ marginTop: 10 }}>
                          <button type="button" className="le-btn primary" onClick={onSaveNeedsAttendance} disabled={needsAssessmentSaving}>
                            {needsAssessmentSaving ? "Saving..." : "Save Attendance"}
                          </button>
                        </div>
                      )}

                      {needsAssessmentActivityKey !== "Record Prospect Attendance" && (
                        <div className="le-block">
                          ...
                          <div className="le-formRow">
                            ...
                            <select
                              ...
                              disabled={!isNeedsAssessmentEditableNow || !!needsAssessmentForm.basicInformation.civilStatus}
                            >
                              <option value="">Select</option>
                          ...
                          <div className="le-formRow">
                            ...
                            <input
                              ...
                              disabled={!isNeedsAssessmentEditableNow || !!needsAssessmentForm.basicInformation.birthday}
                            />
                          </div>
                          ...
                          <div className="le-formRow">
                            ...
                            <input
                              ...
                              disabled={!isNeedsAssessmentEditableNow || !!needsAssessmentForm.basicInformation.occupation}
                            />
                          </div>
                          ...
                          {isNeedsAssessmentEditableNow && needsAssessmentActivityKey === "Perform Needs Analysis" && (
                            <div className="le-actions" style={{ marginTop: 14 }}>
                              <button type="button" className="le-btn primary" onClick={onSaveNeedsAssessment} disabled={needsAssessmentSaving}>
                                {needsAssessmentSaving ? "Saving..." : "Save Needs Analysis"}
                              </button>
                            </div>
                          )}

                          {isNeedsAssessmentEditableNow && needsAssessmentActivityKey === "Schedule Proposal Presentation" && (
                            <>
                              <h5 className="le-attemptSectionHeader" style={{ marginTop: 16 }}>Schedule Proposal Presentation</h5>
                              <div className="le-formRow">
                                <label className="le-label">Meeting Date *</label>
                              ...
                              <div className="le-actions" style={{ marginTop: 14 }}>
                                <button type="button" className="le-btn primary" onClick={onScheduleProposalPresentation} disabled={needsAssessmentSaving}>{needsAssessmentSaving ? "Saving..." : "Schedule Proposal Presentation"}</button>
                              </div>
                            </>
                          )}
                        </div>
