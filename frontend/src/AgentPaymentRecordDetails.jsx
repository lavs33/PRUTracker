...
function AgentPaymentRecordDetails() {
  ...
  const handleEorSubmit = async (event) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/policyholders/${policyholderId}/annual-payments/${annualPaymentId}/payments/${paymentId}/eor-duplicate?userId=${user.id}&eorNumber=${encodedEorNumber}`);
  ...
  const paymentRecordLabel = payment.isMissedPaymentRecord ? "Missed Payment Record" : `${frequencyLabel} Payment Record`;
  ...
  const paymentDateDisplay = (
    <span className="pay-dateWithPill">
      <span>{formatDateOnly(payment.paymentDate)}</span>
      {payment.isPaidLate ? <span className="pay-latePill">Paid Late</span> : null}
    </span>
  );

  const transferRows = [
    ["Total Premium Paid", formatAmount(payment.totalPremiumPaidPhp)],
    ...(Number(payment.overdueFeePhp || 0) > 0 ? [["Overdue Fee", formatAmount(payment.overdueFeePhp)]] : []),
    ...(Number(payment.paymentCountCovered || 1) > 1 ? [["Payment Periods Covered", `${payment.paymentCountCovered} periods`]] : []),
            ...
            {loading ? (
              ...
              <>
                <div className="pay-hero">
                  <div>
                    <p className="ph-kicker">{paymentRecordLabel}</p>
