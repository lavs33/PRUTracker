...
function AgentSalesPerformance() {
  ...
  const kpis = [
    { label: "Total Ongoing Leads", value: data.totalOngoingLeads || 0 },
  ...
  const generatePdfReport = () => {
    ...
    const sourceRowsHtml = sourcePerformanceRows.map((row) => `
      <tr>
        ...
        <td>${Number(row.ongoingLeads || 0)}</td>
    ...
    const topSourceRowsHtml = topConvertedSourceRows.map((row) => `
      <tr>
        <td>${escapeHtml(row.label)}</td>
        <td>${Number(row.convertedLeads || 0)}/${Number(row.ongoingLeads || 0)}</td>
    ...
    const overviewPage = `
        ...
        <section class="section">
          <div class="insight-grid">
            ...
            <div class="insight-card"><h4>Best Lead Source</h4><p>${bestSource ? `${escapeHtml(bestSource.label)} has ${Number(bestSource.convertedAndActiveLeads || 0)} converted leads with active policies (${Number(bestSource.activeConversionRatePct || 0)}% with ${Number(bestSource.ongoingLeads || 0)} ongoing leads).` : "—"}</p></div>
          ...
          <div class="analytics-grid">
            <div class="panel">
              ...
              <table><thead><tr><th>Metric</th><th>Value</th></tr></thead><tbody>
                <tr><td>Total Ongoing Leads</td><td>${Number(data.totalOngoingLeads || 0)}</td></tr>
            ...
            <div class="panel">
              ...
              <table><thead><tr><th>Lead Source</th><th>Converted / Ongoing</th><th>Rate</th></tr></thead><tbody>${topSourceRowsHtml || '<tr><td colspan="3">No converted lead source data.</td></tr>'}</tbody></table>
            </div>
          </div>
        </section>
      `;
        ...
        <section class="section">
          <div class="analytics-grid">
            ...
            <div class="panel">
              ...
              <table><thead><tr><th>Lead Source</th><th>Total Ongoing Leads</th><th>Converted and Active</th><th>Active Rate</th></tr></thead><tbody>${sourceRowsHtml || '<tr><td colspan="4">No lead source data available.</td></tr>'}</tbody></table>
            </div>
          </div>
        </section>
        <section class="section">
          <div class="panel">
            <div class="panel-title-row"><h4>Converted Leads Trend</h4><strong>${bestTrendMonth ? `${Number(bestTrendMonth.converted || 0)} peak conversions` : "No trend"} • ${Number(data.totalOngoingLeads || 0)} ongoing leads</strong></div>
        ...
        <main className="sp-content">
          ...
          <section className="sp-highlights">
            ...
            <div className="sp-highlight">
              ...
              <small>{bestSource ? `${bestSource.convertedAndActiveLeads || 0} converted and active (${bestSource.activeConversionRatePct || 0}% with ${bestSource.ongoingLeads || 0} ongoing)` : "No active-policy lead source pattern yet."}</small>
            </div>
            <div className="sp-highlight">
          ...
          <div className="sp-grid">
            <section className="sp-card">
              ...
              <div className="sp-progressRow">
                <label>Converted vs Total Ongoing Leads</label>
                <div className="sp-track"><span style={{ width: `${data.conversionRatePct || 0}%` }} /></div>
                <b>{data.conversionRatePct || 0}%</b>
              </div>
              <p className="sp-footnote">{data.convertedLeads || 0} converted leads with {data.totalOngoingLeads || 0} ongoing leads in this period.</p>
                ...
                {topConvertedSourceRows.length > 0 ? topConvertedSourceRows.map((row) => (
                  <div key={row.label} className="sp-sourceRow">
                    <div className="sp-sourceMeta">
                      ...
                      <span>{row.convertedLeads || 0} converted / {row.ongoingLeads || 0} ongoing • {row.conversionRatePct || 0}%</span>
                    </div>
            ...
            <section className="sp-card sp-wide">
              <div className="sp-cardHeader">
                ...
                <strong className="sp-cardTotal">{bestTrendMonth ? `${bestTrendMonth.converted || 0} peak conversions` : "No trend"} • {data.totalOngoingLeads || 0} ongoing leads</strong>
              </div>
