  ...
  const generatePdfReport = () => {
    ...
    const trendRows = Array.isArray(data.monthlyConvertedLeads)
      ? data.monthlyConvertedLeads.map((row) => `
        ...
        `).join("")
      : "";
    const salesTablePages = chunk(salesRows, 16).map((rows, pageIndex) => `
      <section class="pdf-page">
        <section class="section">
          <h2 class="section-title">Detailed Sales Data</h2>
          <div class="panel">
            <h4>Current Filter Scope • Page ${pageIndex + 1}</h4>
            <table>
              <thead>
                <tr>
                  <th>Lead Code</th>
              ...
              <tbody>
                ${rows.map((row) => `
                  <tr>
                    ...
                    <td>${escapeHtml(formatDate(row.leadCreatedAt))}</td>
                  </tr>
        ...
        <div class="report-footer"><div>Generated by PRUTracker • ${escapeHtml(formatDateTime(now))}</div><div>Sales Data</div></div>
      </section>
    `).join("");
              ...
              <section class="section">
                ...
                <div class="panel">
                  ...
                  <table>
                    ...
                    <tbody><tr><td colspan="8">No sales records available for the selected filters.</td></tr></tbody>
                  </table>
                </div>
              </section>
              <div class="report-footer"><div>Generated by PRUTracker • ${escapeHtml(formatDateTime(now))}</div><div>Sales Data</div></div>
                ...
                <table className="sp-table">
                  ...
                  <tbody>
                    {salesRows.length > 0 ? salesRows.map((row) => (
                      <tr key={`${row.leadCode}-${row.leadCreatedAt || row.prospectCode}`}>
                        <td>{row.leadCode || "—"}</td>
                        ...
                        <td>{formatDate(row.leadCreatedAt)}</td>
                      </tr>
                    )) : (
                      <tr>
