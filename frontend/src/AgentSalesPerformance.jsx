...
const formatReportPeriod = (reportContext) => {
  const label = reportContext?.periodLabel || "All available records";
  ...
  return start ? `${label}: ${start} to ${end}` : `${label} through ${end}`;
};
...
function AgentSalesPerformance() {
  ...
  const generatePdfReport = () => {
    ...
    const overviewPage = `
        ...
        <section class="section compact-header">
          <div class="top-grid">
            <div>
        ...
        <section class="section">
          <div class="analytics-grid three">
            <div class="panel">
              <h4>Total Frequency Premium Breakdown <span>(active policies only)</span></h4>
              <table><thead><tr><th>Frequency</th><th>Amount</th><th>Share</th></tr></thead><tbody>
                ${premiumBreakdownRows.map((row) => `<tr><td>${escapeHtml(row.label)}</td><td>₱ ${escapeHtml(money(row.amount))}</td><td>${row.sharePct}%</td></tr>`).join("")}
              </tbody></table>
            </div>
            <div class="panel">
        ...
        <section class="section">
          <div class="analytics-grid">
            <div class="panel">
              <h4>Converted by Policy Status</h4>
              <table><thead><tr><th>Status</th><th>Count</th><th>Share</th></tr></thead><tbody>${convertedLeadPolicyStatusRowsHtml || '<tr><td colspan="3">No converted policy status data.</td></tr>'}</tbody></table>
            </div>
            <div class="panel">
              <h4>Unconverted by Lead Status</h4>
              <table><thead><tr><th>Status</th><th>Count</th><th>Share</th></tr></thead><tbody>${unconvertedLeadStatusRowsHtml || '<tr><td colspan="3">No unconverted lead status data.</td></tr>'}</tbody></table>
            </div>
          </div>
        </section>
      `;
    ...
    const analyticsPage = `
        <div class="header-band"></div>
        <section class="section">
          <h2 class="section-title">Lead Source + Conversion Trend</h2>
          <div class="analytics-grid">
            <div class="panel">
              <h4>Lead Source Quality <span>Converted and Active</span></h4>
        ...
        <section class="section">
          <div class="panel">
            <h4>Converted Leads Trend <span>All converted leads regardless of policy status</span></h4>
        ...
        <head>
          ...
          <style>
            ...
            * { box-sizing: border-box; }
            ...
            .meta-row.tight { max-width: 420px; }
            ...
            .compact-header { margin-bottom: 5px; }
