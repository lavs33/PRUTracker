...
function AgentSalesPerformance() {
  ...
  const bestSourceSummary = bestSources.length > 1
    ? `${bestSourceNames} share the lead with ${bestSourceCount} converted active lead${bestSourceCount === 1 ? "" : "s"}.`
    : bestSource
      ? `${bestSource.label} has ${bestSourceCount} converted active lead${bestSourceCount === 1 ? "" : "s"} (${bestSource.activeConversionRatePct || 0}% of ${bestSource.handledLeads || 0} handled).`
