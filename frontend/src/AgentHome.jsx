  ...
  const renderConcern = (notification) => {
    ...
    return <article key={notification._id} className={`home-actionItem ${notification.priority}`}>
      ...
      <div className="home-actionBody">
        ...
        {concern.isSalesRecommendation && String(notification?.metadata?.personalizedMessage || "").trim() ? <p className="home-actionContext"><strong>Personalized message from your {notification?.metadata?.senderRole === "AUM" ? "Assistant Unit Manager" : "Unit Manager"}:</strong> {notification.metadata.personalizedMessage}</p> : null}
