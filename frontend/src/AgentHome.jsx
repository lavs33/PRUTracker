...
function AgentHome() {
  ...
  const fetchHomeData = useCallback(async (signal) => {
    ...
    const [tasksRes, clientsRes, salesRes] = await Promise.all([
      fetch(`${API_BASE}/api/tasks/summary?userId=${user.id}&includeRefs=1`, signal ? { signal } : undefined),
      fetch(
        `${API_BASE}/api/clients/relationship/dashboard?${new URLSearchParams({
          userId: user.id,
          datePreset: "ALL",
          source: "ALL",
          marketType: "ALL",
      ...
      fetch(
        `${API_BASE}/api/sales/performance?${new URLSearchParams({
          userId: user.id,
          datePreset: "ALL",
          leadSource: "ALL",
          policyStatus: "ALL",
    ...
    const [tasksPayload, clientsPayload, salesPayload] = await Promise.all([
      tasksRes.json(),
      clientsRes.json(),
      salesRes.json(),
    ]);

    if (!tasksRes.ok) throw new Error(tasksPayload?.message || "Failed to load task summary.");
    if (!clientsRes.ok) throw new Error(clientsPayload?.message || "Failed to load client dashboard.");
    if (!salesRes.ok) throw new Error(salesPayload?.message || "Failed to load sales dashboard.");

    const sourceBreakdown = Array.isArray(salesPayload?.leadSourceBreakdown) ? salesPayload.leadSourceBreakdown : [];
    const bestSource =
      sourceBreakdown.length > 0
        ? [...sourceBreakdown].sort((a, b) => Number(b?.conversionRatePct || 0) - Number(a?.conversionRatePct || 0))[0]
        : null;

    setHomeData({
      tasks: {
        dueTodayTop5: Array.isArray(tasksPayload?.dueTodayTop5) ? tasksPayload.dueTodayTop5 : [],
        recentlyAddedTop5: Array.isArray(tasksPayload?.recentlyAddedTop5) ? tasksPayload.recentlyAddedTop5 : [],
      },
      clients: {
        totalProspects: Number(clientsPayload?.totals?.prospects || 0),
        totalPolicyholders: Number(clientsPayload?.totals?.policyholders || 0),
        conversionRate: Number(clientsPayload?.conversionRatePct || 0),
        activePolicyRate: Number(clientsPayload?.activePolicyRatePct || 0),
        recentProspects: Array.isArray(clientsPayload?.recentProspects) ? clientsPayload.recentProspects.slice(0, 3) : [],
      },
      sales: {
        conversionRatePct: Number(salesPayload?.conversionRatePct || 0),
        totalPolicies: Number(salesPayload?.totalPolicies || 0),
        totalAnnualPremiumPhp: Number(salesPayload?.totalAnnualPremiumPhp || 0),
        bestSource,
      },
    });
  }, [user?.id]);
