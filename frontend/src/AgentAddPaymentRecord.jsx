...
function AgentAddPaymentRecord() {
  ...
  useEffect(() => {
    ...
    const total = Math.round(((Number(basePremiumAmount || 0) * Math.max(1, count)) + safeOverdueFee) * 100) / 100;
    setForm((prev) => {
  ...
  }, [basePremiumAmount, form.overdueFeePhp, isMissedPaymentRecord, missedPaymentPeriod.count]);
                ...
                <form className="addpay-form" onSubmit={handleSubmit}>
                  <label className="addpay-field">
                    ...
                    <input
                      ...
                      readOnly
                    />
                    {isMissedPaymentRecord ? <small>Computed from {missedPaymentPeriod.count || 1} covered payment period(s), plus any overdue fee.</small> : null}
                ...
                {isEorConfirmOpen ? (
                  <div className="addpay-confirmOverlay" role="dialog" aria-modal="true" aria-labelledby="addpay-payment-confirm-title">
                    <div className="addpay-confirmModal">
                      ...
                      <p>Review the premium payment transfer and eOR details below before saving this payment record as processed. Once confirmed, the eOR details can no longer be edited thereafter.</p>
                      <div className="addpay-confirmGrid">
                        <div><span>Total Premium Paid</span><strong>Php {form.totalPremiumPaidPhp || "—"}</strong></div>
                        <div><span>Frequency</span><strong>{annualPayment.frequencyOfPayment || "—"}</strong></div>
                        <div><span>Payment Date</span><strong>{form.paymentDate || "—"}</strong></div>
