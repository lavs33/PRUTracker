...
function AgentCancelPolicy() {
  ...
  const formatTermValue = (type, years, untilAge) => {
    ...
    if (["RANGE_TO_AGE", "MIXED"].includes(normalized)) {
      ...
      if (Number.isFinite(age) && age > 0) return `Until age ${age}`;
      if (Number.isFinite(y) && y > 0) return `${y} ${y === 1 ? "year" : "years"}`;
    }
    return "";
  };
  ...
  const policyDetailSections = [
    {
      ...
      rows: [
        ...
        ["Policy Status", policyholder.status || "—"],
      ],
    },
    {
          ...
          <div className="cancelpol-card">
            {loading ? (
              ...
              <>
                ...
                <div className="cancelpol-detailSections">
                  {policyDetailSections.map((section) => (
                    <section key={section.title} className="cancelpol-detailSection">
                      <h2 className="cancelpol-detailSectionTitle">{section.title}</h2>
                      <div className="cancelpol-detailGrid">
                        {section.rows.map(([label, value]) => (
                          <div key={label} className="cancelpol-detailItem">
                            <span className="cancelpol-detailLabel">{label}</span>
                            <span className="cancelpol-detailValue">{value || "—"}</span>
                          </div>
                        ))}
                      </div>
                    </section>
                ...
                <form className="cancelpol-form" onSubmit={handleOpenConfirm}>
                  ...
                  <div className="cancelpol-field">
                    <label>Proof of Approved Policy Surrender (Image) <span>*</span></label>
