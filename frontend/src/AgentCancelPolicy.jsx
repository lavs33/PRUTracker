...
function AgentCancelPolicy() {
  ...
  const handleSurrenderProofFileChange = async (event) => {
    ...
    if (!["image/jpeg", "image/png"].includes(file.type)) {
      ...
      setFieldErrors((prev) => ({
        ...prev,
        proofOfApprovedPolicySurrenderImageDataUrl: "Proof of approved policy surrender must be a JPG or PNG image.",
      }));
      return;
    }
  ...
  const validateForm = () => {
    ...
    if (!form.accomplishedPolicySurrenderFormFileDataUrl) {
      next.accomplishedPolicySurrenderFormFileDataUrl = "Accomplished policy surrender form PDF is required.";
    ...
    if (!form.approvedCancellationDate) {
      next.approvedCancellationDate = "Approved cancellation date is required.";
    ...
    if (!form.proofOfApprovedPolicySurrenderImageDataUrl) {
      next.proofOfApprovedPolicySurrenderImageDataUrl = "Proof of approved policy surrender image is required.";
    } else if (!/^data:image\/(?:jpeg|png);base64,/i.test(form.proofOfApprovedPolicySurrenderImageDataUrl)) {
      next.proofOfApprovedPolicySurrenderImageDataUrl = "Proof of approved policy surrender must be a JPG or PNG image.";
    }
                ...
                <div className="cancelpol-header">
                  <div>
                    ...
                    <p className="cancelpol-subtitle">
                      {policyholder.policyholderCode || "—"} • {policyholder.policyNumber || "—"} • {product.productName || "—"}
                    </p>
                  </div>
                ...
                <form className="cancelpol-form" onSubmit={handleOpenConfirm}>
                  <div className="cancelpol-field">
                    ...
                    {form.accomplishedPolicySurrenderFormFileDataUrl ? (
                      <div className="cancelpol-previewBlock">
                        <div className="cancelpol-previewHeader">
                          <strong>{form.accomplishedPolicySurrenderFormFileName || "Policy surrender form"}</strong>
                          <a href={form.accomplishedPolicySurrenderFormFileDataUrl} target="_blank" rel="noreferrer">Open PDF</a>
                        </div>
                        <iframe title="Accomplished policy surrender form preview" src={form.accomplishedPolicySurrenderFormFileDataUrl} className="cancelpol-pdfPreview" />
                      </div>
                    ) : null}
                  </div>
                  ...
                  <div className="cancelpol-field">
                    <label htmlFor="approvedCancellationDate">Approved Cancellation Date <span>*</span></label>
                    <input
                    ...
                    />
                    <p className="cancelpol-help">Allowed dates are after policy issuance date through today.</p>
                    {fieldErrors.approvedCancellationDate ? <p className="cancelpol-error">{fieldErrors.approvedCancellationDate}</p> : null}
                  </div>

                  <div className="cancelpol-field">
                    <label>Proof of Approved Policy Surrender (JPG/PNG) <span>*</span></label>
                    <input ref={surrenderProofInputRef} type="file" accept="image/jpeg,image/png" onChange={handleSurrenderProofFileChange} />
                    ...
                    {form.proofOfApprovedPolicySurrenderImageDataUrl ? (
                      <div className="cancelpol-previewBlock cancelpol-imagePreviewBlock">
                        <div className="cancelpol-previewHeader">
                          <strong>{form.proofOfApprovedPolicySurrenderFileName || "Proof of approved surrender"}</strong>
                          <a href={form.proofOfApprovedPolicySurrenderImageDataUrl} target="_blank" rel="noreferrer">Open image</a>
                        </div>
        ...
        <div className="cancelpol-modalOverlay" role="dialog" aria-modal="true" aria-labelledby="cancel-policy-confirm-title">
          <div className="cancelpol-modal">
            ...
            <div className="cancelpol-confirmGrid">
              ...
              <div><span>Approved Cancellation Date</span><strong>{formatDateOnly(form.approvedCancellationDate)}</strong></div>
