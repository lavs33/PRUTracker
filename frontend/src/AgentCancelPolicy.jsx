...
function AgentCancelPolicy() {
  ...
  const minCancellationDate = lastPaymentDateInputValue;
  ...
  const handleSurrenderProofFileChange = async (event) => {
    ...
    if (!looksImage) {
      ...
      setFieldErrors((prev) => ({
        ...prev,
        proofOfApprovedPolicySurrenderImageDataUrl: "Proof of approved policy surrender must be a JPG, JPEG, or PNG image.",
      }));
      return;
    }
  ...
  const validateForm = () => {
    ...
    if (!form.approvedCancellationDate) {
      ...
      next.approvedCancellationDate = "Last payment date is unavailable.";
    } else if (form.approvedCancellationDate < minCancellationDate) {
      next.approvedCancellationDate = "Cancellation date must be on or after the last payment date.";
    }

    if (!String(form.cancellationReason || "").trim()) {
      next.cancellationReason = "Reason for cancellation is required.";
    }
    ...
    if (!form.proofOfApprovedPolicySurrenderImageDataUrl) {
      ...
      next.proofOfApprovedPolicySurrenderImageDataUrl = "Proof of approved policy surrender must be a JPG, JPEG, or PNG image.";
    }
            ...
            {loading ? (
              ...
              <>
                ...
                <form className="cancelpol-form" onSubmit={handleOpenConfirm}>
                  ...
                  <div className="cancelpol-field">
                    <label htmlFor="cancellationReason">Reason for Cancellation <span>*</span></label>
                    <textarea
                      id="cancellationReason"
                      rows="4"
                      value={form.cancellationReason}
                      onChange={(event) => {
                        setForm((prev) => ({ ...prev, cancellationReason: event.target.value }));
                        setFieldErrors((prev) => ({ ...prev, cancellationReason: undefined }));
                      }}
                      placeholder="Enter reason for cancellation"
                    />
                    {fieldErrors.cancellationReason ? <p className="cancelpol-error">{fieldErrors.cancellationReason}</p> : null}
                  </div>
                  ...
                  <div className="cancelpol-field">
                    <label>Proof of Approved Policy Surrender (JPG, JPEG, PNG) <span>*</span></label>
                    <input ref={surrenderProofInputRef} type="file" accept="image/jpeg,image/png,.jpg,.jpeg,.png" onChange={handleSurrenderProofFileChange} />
        ...
        <div className="cancelpol-modalOverlay" role="dialog" aria-modal="true" aria-labelledby="cancel-policy-confirm-title">
          <div className="cancelpol-modal">
            ...
            <div className="cancelpol-confirmGrid">
              ...
              <div><span>Reason for Cancellation</span><strong>{String(form.cancellationReason || "").trim() || "—"}</strong></div>
