...
function AdminOrganizationPage() {
  ...
  useEffect(() => {
    ...
    const candidates = formOptions.agents.filter((agent) => agent.role === 'AG' && agent.status === 'Active' && agent.unitId === createManagerForm.unitId && agent.agentId !== currentScopeManager?.agentId);
                ...
                {managerView === 'list' ? (
                  ...
                  <div className="aop-form-grid">
                    ...
                    {createManagerForm.managerType === 'AUM' ? (
                      <div className="aop-form-note">
                        AUM assignment is based on current-month sales performance. Only active agents who meet both their Active Policies and Closing Ratio KPI targets can be selected.
                      </div>
                    ) : null}
                    ...
                    {createManagerForm.managerType === 'AUM' && selectedCreateManagerAgent ? (() => {
                      ...
                      const displayTarget = (kpi) => {
                        const target = [kpi?.targetValue, kpi?.targetMin].find((value) => Number.isFinite(Number(value)) && Number(value) > 0);
                        return target === undefined ? '—' : `${Number(target).toLocaleString('en-PH')}${kpi?.valueType === 'Percent' ? '%' : ''}`;
                      };
