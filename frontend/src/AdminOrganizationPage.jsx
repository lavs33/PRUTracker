...
function AdminOrganizationPage() {
  ...
  const togglePanel = (panelName) => {
    ...
    setActivePanel((current) => {
      if (current === panelName) return PANEL_NONE;
      return panelName;
    });

    if (panelName === PANEL_EDIT && !editAreaId && areas[0]?.id) {
      setEditAreaId(areas[0].id);
    }

    if (panelName === PANEL_DELETE && !deleteAreaId && areas[0]?.id) {
      setDeleteAreaId(areas[0].id);
    }
  };
        ...
        <section className="aop-section">
          ...
          <div className="aop-toolbar">
            <div className="aop-toolbar-actions">
              <button
                type="button"
                className={`aop-action-btn${activePanel === PANEL_ADD ? " active" : ""}`}
                onClick={() => togglePanel(PANEL_ADD)}
              >
                <FaPlus /> <span>Add Area</span>
              </button>
              <button
                type="button"
                className={`aop-action-btn${activePanel === PANEL_EDIT ? " active" : ""}`}
                onClick={() => togglePanel(PANEL_EDIT)}
              >
                <FaUserEdit /> <span>Edit Area</span>
              </button>
              <button
                type="button"
                className={`aop-action-btn danger${activePanel === PANEL_DELETE ? " active" : ""}`}
                onClick={() => togglePanel(PANEL_DELETE)}
              >
          ...
          {activePanel === PANEL_ADD && (
            <div className="aop-management-grid single">
              <form className="aop-form-card" onSubmit={handleAddArea}>
                <div className="aop-form-head">
                  ...
                  <button type="button" className="aop-close-btn" onClick={() => setActivePanel(PANEL_NONE)}>
                    Close
                  </button>
                </div>
            ...
            <div className="aop-management-grid single">
              <form className="aop-form-card danger" onSubmit={handleDeleteArea}>
                ...
                <div className="aop-warning-box">
                  ...
                  <p>
                    Deleting this area will also delete the branches, units, agents, branch managers, unit managers,
                    and assistant unit managers under it.
                    {deleteArea && (
                      <>
