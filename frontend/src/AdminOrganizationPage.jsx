...
function matchesSearch(value, searchTerm) {
  ...
  return String(value || '').toLowerCase().includes(searchTerm.trim().toLowerCase());
}
...
function AdminOrganizationPage() {
  ...
  const [showManagerForm, setShowManagerForm] = useState(true);
  ...
  const areaNameById = useMemo(
    () => new Map(formOptions.areas.map((area) => [area.id, area.areaName])),
    [formOptions.areas]
  );

  const activeBmByBranchId = useMemo(
    () => new Map((managerDirectory.BM || []).filter((manager) => !manager.isBlocked).map((manager) => [manager.branchId, manager])),
    [managerDirectory.BM]
  );

  const activeUmByUnitId = useMemo(
    () => new Map((managerDirectory.UM || []).filter((manager) => !manager.isBlocked).map((manager) => [manager.unitId, manager])),
    [managerDirectory.UM]
  );

  const activeAumByUnitId = useMemo(
    () => new Map((managerDirectory.AUM || []).filter((manager) => !manager.isBlocked).map((manager) => [manager.unitId, manager])),
    [managerDirectory.AUM]
  );
  ...
  const branchList = useMemo(
    () =>
      formOptions.branches.map((branch) => ({
        ...branch,
        areaName: areaNameById.get(branch.areaId) || '—',
        branchManager: activeBmByBranchId.get(branch.id) || null,
      })),
    [activeBmByBranchId, areaNameById, formOptions.branches]
  );

  const unitList = useMemo(
    () =>
      formOptions.units.map((unit) => {
        const branch = formOptions.branches.find((item) => item.id === unit.branchId) || null;
        return {
          ...unit,
          branchName: branch?.branchName || '—',
          areaName: branch ? areaNameById.get(branch.areaId) || '—' : '—',
          umManager: activeUmByUnitId.get(unit.id) || null,
          aumManager: activeAumByUnitId.get(unit.id) || null,
        };
      }),
    [activeAumByUnitId, activeUmByUnitId, areaNameById, formOptions.branches, formOptions.units]
  );
    ...
    () =>
      ['BM', 'UM', 'AUM'].flatMap((type) =>
        (managerDirectory[type] || [])
          ...
          .map((manager) => ({ ...manager, managerType: type }))
      ),
    [managerDirectory]
  );

  const filteredAreas = useMemo(
    () => formOptions.areas.filter((area) => matchesSearch(`${area.areaName}`, areaSearch)),
    [areaSearch, formOptions.areas]
  );
  ...
  const filteredBranches = useMemo(
    () =>
      branchList.filter((branch) =>
        matchesSearch(
          `${branch.branchName} ${branch.areaName} ${branch.branchManager?.username || ''} ${branch.branchManager?.firstName || ''} ${branch.branchManager?.lastName || ''}`,
          branchSearch
        )
      ),
  ...
  const filteredUnits = useMemo(
    () =>
      unitList.filter((unit) =>
        matchesSearch(
          `${unit.unitName} ${unit.branchName} ${unit.areaName} ${unit.umManager?.username || ''} ${unit.umManager?.firstName || ''} ${unit.umManager?.lastName || ''} ${unit.aumManager?.username || ''} ${unit.aumManager?.firstName || ''} ${unit.aumManager?.lastName || ''}`,
          unitSearch
        )
      ),
    [unitList, unitSearch]
  ...
  const filteredManagers = useMemo(
    () =>
      managerList.filter((manager) =>
        matchesSearch(
          `${manager.managerType} ${manager.username} ${manager.firstName} ${manager.middleName} ${manager.lastName} ${manager.areaName} ${manager.branchName} ${manager.unitName}`,
          managerSearch
        )
      ),
  ...
  const filteredAgents = useMemo(
    () =>
      formOptions.agents.filter((agent) =>
        matchesSearch(
          `${agent.username} ${agent.firstName} ${agent.middleName} ${agent.lastName} ${agent.sex} ${agent.agentType} ${agent.areaName} ${agent.branchName} ${agent.unitName}`,
          agentSearch
        )
      ),
    [agentSearch, formOptions.agents]
  );
          ...
          <div className="aop-content-shell">
              ...
              <section className="aop-section aop-section-tight">
                ...
                <div className="aop-list-toolbar">
                  ...
                  <div className="aop-toolbar-actions">
                    <button type="button" className="aop-action-btn" onClick={() => setShowAddAreaForm((current) => !current)}>
                      {showAddAreaForm ? 'Hide Add Area' : 'Open Add Area'}
                    </button>
                    <button type="button" className="aop-action-btn" onClick={() => setShowEditAreaForm((current) => !current)}>
                      {showEditAreaForm ? 'Hide Edit Area' : 'Open Edit Area'}
                    </button>
                  </div>
                </div>
                ...
                {filteredAreas.length === 0 ? (
                  ...
                  <div className="aop-record-grid">
                    {filteredAreas.map((area) => (
                      <article key={area.id} className="aop-record-card dark">
                        <div className="aop-record-head">
                          <div>
                            <p>Area Name</p>
                            <h3>{area.areaName}</h3>
                          </div>
                ...
                {(showAddAreaForm || showEditAreaForm) && (
            ...
            {activeSection === 'branches' && (
              <section className="aop-section aop-section-tight">
                ...
                <div className="aop-list-toolbar">
                  <label className="aop-search" htmlFor="branches-search">
                    <input
                      id="branches-search"
                      type="search"
                      placeholder="Search branch, area, or branch manager"
                      value={branchSearch}
                  ...
                  <div className="aop-toolbar-actions">
                    <button type="button" className="aop-action-btn" onClick={() => setShowAddBranchForm((current) => !current)}>
                      {showAddBranchForm ? 'Hide Add Branch' : 'Open Add Branch'}
                    </button>
                    <button type="button" className="aop-action-btn" onClick={() => setShowEditBranchForm((current) => !current)}>
                      {showEditBranchForm ? 'Hide Edit Branch' : 'Open Edit Branch'}
                    </button>
                  </div>
                </div>
                ...
                {filteredBranches.length === 0 ? (
                  ...
                  <div className="aop-record-grid">
                    {filteredBranches.map((branch) => (
                      <article key={branch.id} className="aop-record-card dark">
                        <div className="aop-record-head">
                          <div>
                            <p>Branch Name</p>
                        ...
                        <div className="aop-record-meta">
                          <div>
                            <span>Area</span>
                            <strong>{branch.areaName}</strong>
                          </div>
                          <div>
                            <span>Branch Manager</span>
                            <strong>{branch.branchManager ? `${branch.branchManager.username} · ${[branch.branchManager.firstName, branch.branchManager.lastName].filter(Boolean).join(' ')}` : 'Unassigned'}</strong>
                          </div>
                        </div>
                        <TimestampRow createdAt={branch.createdAt} updatedAt={branch.updatedAt} />
                      </article>
                ...
                {(showAddBranchForm || showEditBranchForm) && (
                  <div className="aop-form-grid two-up">
                    {showAddBranchForm && (
            ...
            {activeSection === 'units' && (
              <section className="aop-section aop-section-tight">
                ...
                <div className="aop-list-toolbar">
                  <label className="aop-search" htmlFor="units-search">
                    <input
                      id="units-search"
                      type="search"
                      placeholder="Search unit, branch, area, UM, or AUM"
                      value={unitSearch}
                  ...
                  <div className="aop-toolbar-actions">
                    <button type="button" className="aop-action-btn" onClick={() => setShowAddUnitForm((current) => !current)}>
                      {showAddUnitForm ? 'Hide Add Unit' : 'Open Add Unit'}
                    </button>
                    <button type="button" className="aop-action-btn" onClick={() => setShowEditUnitForm((current) => !current)}>
                      {showEditUnitForm ? 'Hide Edit Unit' : 'Open Edit Unit'}
                    </button>
                  </div>
                </div>
                ...
                {filteredUnits.length === 0 ? (
                  ...
                  <div className="aop-record-grid">
                    {filteredUnits.map((unit) => (
                      <article key={unit.id} className="aop-record-card dark">
                        <div className="aop-record-head">
                          <div>
                            <p>Unit Name</p>
                            <h3>{unit.unitName}</h3>
                        ...
                        <div className="aop-record-meta">
                          <div>
                            <span>Branch</span>
                            <strong>{unit.branchName}</strong>
                          </div>
                          <div>
                            <span>Area</span>
                            <strong>{unit.areaName}</strong>
                          </div>
                          <div>
                            <span>UM</span>
                            <strong>{unit.umManager ? `${unit.umManager.username} · ${[unit.umManager.firstName, unit.umManager.lastName].filter(Boolean).join(' ')}` : 'Unassigned'}</strong>
                          </div>
                          <div>
                            <span>AUM</span>
                            <strong>{unit.aumManager ? `${unit.aumManager.username} · ${[unit.aumManager.firstName, unit.aumManager.lastName].filter(Boolean).join(' ')}` : 'Unassigned'}</strong>
                          </div>
                        </div>
                        <TimestampRow createdAt={unit.createdAt} updatedAt={unit.updatedAt} />
                      </article>
                ...
                {(showAddUnitForm || showEditUnitForm) && (
                  <div className="aop-form-grid two-up">
                    {showAddUnitForm && (
            ...
            {activeSection === 'managers' && (
              <section className="aop-section aop-section-tight">
                <div className="aop-section-head">
                  <h2>All Managers</h2>
                  <p>Review manager records using user-schema details, search the list, and open the reassignment form when needed.</p>
                </div>
                ...
                <div className="aop-list-toolbar">
                  <label className="aop-search" htmlFor="managers-search">
                    <input
                      id="managers-search"
                      type="search"
                      placeholder="Search manager, username, role, branch, or unit"
                      value={managerSearch}
                  ...
                  <div className="aop-toolbar-actions">
                    <button type="button" className="aop-action-btn" onClick={() => setShowManagerForm((current) => !current)}>
                      {showManagerForm ? 'Hide Reassignment Form' : 'Open Reassignment Form'}
                    </button>
                  </div>
                </div>
                ...
                {filteredManagers.length === 0 ? (
                  ...
                  <div className="aop-record-grid">
                    {filteredManagers.map((manager) => (
                      <article key={manager.managerId} className="aop-record-card dark">
                        <div className="aop-record-head">
                          <div>
                            <p>{manager.managerType}</p>
                        ...
                        <div className="aop-record-meta">
                          <div>
                            <span>Name</span>
                            <strong>{[manager.firstName, manager.middleName, manager.lastName].filter(Boolean).join(' ') || '—'}</strong>
                          </div>
                          <div>
                            <span>Sex</span>
                            <strong>{manager.sex || '—'}</strong>
                          </div>
                          <div>
                            <span>Age</span>
                          ...
                          <div>
                            <span>Birthday</span>
                            <strong>{formatDateInput(manager.birthday) || '—'}</strong>
                          </div>
                          <div>
                            <span>Date Employed</span>
                            <strong>{formatDateInput(manager.dateEmployed) || '—'}</strong>
                          </div>
                          <div>
                            <span>Assignment</span>
                            <strong>{[manager.areaName, manager.branchName, manager.unitName].filter(Boolean).join(' / ') || '—'}</strong>
                          </div>
                        </div>
            ...
            {activeSection === 'agents' && (
              <section className="aop-section aop-section-tight">
                <div className="aop-section-head">
                  <h2>All Agents</h2>
                  <p>Review all agent records using user-schema details, search the list, and open the add or edit forms from the action bar.</p>
                </div>
                ...
                <div className="aop-list-toolbar">
                  <label className="aop-search" htmlFor="agents-search">
                    <input
                      id="agents-search"
                      type="search"
                      placeholder="Search agent, username, sex, type, branch, or unit"
                      value={agentSearch}
                  ...
                  <div className="aop-toolbar-actions">
                    <button type="button" className="aop-action-btn" onClick={() => setShowAddAgentForm((current) => !current)}>
                      {showAddAgentForm ? 'Hide Add Agent' : 'Open Add Agent'}
                    </button>
                    <button type="button" className="aop-action-btn" onClick={() => setShowEditAgentForm((current) => !current)}>
                      {showEditAgentForm ? 'Hide Edit Agent' : 'Open Edit Agent'}
                    </button>
                  </div>
                </div>
                ...
                {filteredAgents.length === 0 ? (
                  ...
                  <div className="aop-record-grid">
                    {filteredAgents.map((agent) => (
                      <article key={agent.agentId} className="aop-record-card dark">
                        <div className="aop-record-head">
                          <div>
                            <p>Agent Username</p>
                        ...
                        <div className="aop-record-meta">
                          <div>
                            <span>Name</span>
                            <strong>{[agent.firstName, agent.middleName, agent.lastName].filter(Boolean).join(' ') || '—'}</strong>
                          </div>
                          <div>
                            <span>Sex</span>
                            <strong>{agent.sex || '—'}</strong>
                          </div>
                          <div>
                            <span>Age</span>
                            <strong>{agent.age || '—'}</strong>
                          </div>
                          <div>
                            <span>Birthday</span>
                            <strong>{formatDateInput(agent.birthday) || '—'}</strong>
                          </div>
                          <div>
                            <span>Date Employed</span>
                            <strong>{formatDateInput(agent.dateEmployed) || '—'}</strong>
                          </div>
                          <div>
                          ...
                          <div>
                            <span>Assignment</span>
                            <strong>{[agent.areaName, agent.branchName, agent.unitName].filter(Boolean).join(' / ') || '—'}</strong>
                          </div>
                        </div>
                        <TimestampRow createdAt={agent.createdAt} updatedAt={agent.updatedAt} />
                      </article>
                ...
                {(showAddAgentForm || showEditAgentForm) && (
                  <div className="aop-form-grid two-up">
                    {showAddAgentForm && (
