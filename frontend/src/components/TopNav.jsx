...
function TopNav({ user, onLogoClick, onProfileClick, onLogout, onNotificationsClick }) {
  ...
  const routeMeta = useMemo(() => {
    ...
    if (path.includes("/notifications")) {
      return { eyebrow: "Workspace", title: "Notifications", metricLabel: "Unread alerts" };
    }

    if (path.includes("/sales/performance")) {
      return { eyebrow: "Revenue", title: "Sales Performance", metricLabel: "Unread alerts" };
    }

    if (path.includes("/tasks/progress")) {
      return { eyebrow: "Execution", title: "Task Progress", metricLabel: "Unread alerts" };
    }

    if (path.includes("/tasks/all")) {
      return { eyebrow: "Execution", title: "All Tasks", metricLabel: "Unread alerts" };
    }

    if (path.includes("/tasks")) {
      return { eyebrow: "Execution", title: "Tasks", metricLabel: "Unread alerts" };
    }

    if (path.includes("/clients/relationship")) {
      return { eyebrow: "Relationships", title: "Client Relationships", metricLabel: "Unread alerts" };
    }

    if (path.includes("/policyholders")) {
      return { eyebrow: "Coverage", title: "Policyholders", metricLabel: "Unread alerts" };
    }

    if (path.includes("/prospects")) {
      return { eyebrow: "Pipeline", title: "Prospects", metricLabel: "Unread alerts" };
    }

    if (path.match(/\/agent\/[^/]+\/profile/)) {
      return { eyebrow: "Profile", title: "Profile", metricLabel: "Unread alerts" };
    }

    return { eyebrow: "Workspace", title: "Agent Dashboard", metricLabel: "Unread alerts" };
  }, [location.pathname]);
