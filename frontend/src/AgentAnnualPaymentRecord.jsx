...
function AgentAnnualPaymentRecord() {
  ...
  useEffect(() => {
    ...
    const period = details?.annualPayment?.label;
    document.title = `${user.username} | ${code || "Policyholder"} | ${period || "Annual Payment"}`;
  }, [details?.annualPayment?.label, details?.policyholder?.policyholderCode, user]);
              ...
              <>
                ...
                <section className="ph-annualSummaryGrid" aria-label="Annual payment summary">
                  ...
                  <div className="ph-summaryTile">
                    ...
                    <strong>{policyholder.policyNumber || "—"}</strong>
                  </div>
                </section>
                ...
                <section className="ph-records ph-paymentTimelineSection">
                  <div className="ph-recordsHeader">
                    <div>
                      <h2 className="ph-recordsTitle">Individual Payment Records</h2>
                      ...
                      {payments.map((payment, index) => {
                        ...
                        return (
                          <button
                            key={paymentId || index}
                            type="button"
                            className={`ph-paymentRecordCard ${isSelected ? "selected" : ""}`.trim()}
                            onClick={() => setSelectedPaymentId(isSelected ? "" : paymentId)}
                          ...
                          >
                            ...
                            <span className="ph-paymentRecordMain">
                              ...
                              <span className="ph-paymentRecordMetaGrid">
                                <span><b>Payment Date:</b> {formatDateOnly(payment.paymentDate)}</span>
                                <span><b>Method:</b> {payment.methodForPayment || "—"}</span>
                                <span><b>Proof:</b> {payment.proofOfPaymentFileName || "—"}</span>
                                <span><b>eOR:</b> {payment.eorNumber || "Pending"}</span>
                              </span>
                              {isSelected ? (
                                <span className="ph-paymentRecordExpanded">
                                  <span><b>Saved At:</b> {formatDateOnly(payment.savedAt)}</span>
                                  <span><b>Receipt Date:</b> {formatDateOnly(payment.receiptDate)}</span>
                                  <span><b>eOR File:</b> {payment.eorFileName || "—"}</span>
                                </span>
                              ) : null}
                            </span>
