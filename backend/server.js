...
app.get("/api/prospects/:prospectId/leads/:leadId/needs-assessment", async (req, res) => {
  try {
    ...
    const engagement = await LeadEngagement.findOne({ leadId: leadObjectId }).select("_id").lean();
    ...
    const needsAssessment = await NeedsAssessment.findOne({ leadEngagementId: engagement._id })
      .select("attendanceConfirmed attendedAt dependents")
      .lean();
...
app.put("/api/prospects/:prospectId/leads/:leadId/needs-assessment", async (req, res) => {
  ...
  try {
    const { userId } = req.query;
    const { prospectId, leadId } = req.params;
    const { attendanceConfirmed, basicInformation, dependents } = req.body || {};
    ...
    if (attendanceConfirmed !== true) {
      return res.status(400).json({ message: "Prospect attendance must be confirmed before saving needs analysis." });
    }
    ...
    await session.withTransaction(async () => {
      ...
      const payloadDependents = normalizedDependents.map((d) => ({
      ...
      await NeedsAssessment.findOneAndUpdate(
        { leadEngagementId: engagement._id },
        {
          $set: {
            attendanceConfirmed: true,
            attendedAt: new Date(),
            dependents: payloadDependents,
          },
        },
    ...
    return res.json({ message: "Needs assessment section saved." });
  } catch (err) {
