...
app.get("/api/prospects/:prospectId/leads/:leadId/engagement", async (req, res) => {
  try {
    ...
    const applicationDoc = await Application.findOne({ leadEngagementId: engagement._id })
      .select("outcomeActivity chosenProductId recordProspectAttendance recordPremiumPaymentTransfer")
      .lean();
...
app.post("/api/prospects/:prospectId/leads/:leadId/application/premium-payment-transfer", async (req, res) => {
  ...
  try {
    ...
    const annualPremium = toNonNegativeNumber(totalAnnualPremiumPhp);
    const frequencyPremium = toNonNegativeNumber(totalFrequencyPremiumPhp);
    ...
    if (annualPremium === null) return res.status(400).json({ message: "Total annual premium is required." });
    if (frequencyPremium === null) return res.status(400).json({ message: "Total requested-frequency premium is required." });
