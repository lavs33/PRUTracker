...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (user.role === "AG") {
      ...
      const manager = await ManagerModel.findOne({ userId: user._id }).populate(buildManagerPopulateQuery()).lean();
      ...
      const manager = await BM.findOne({ userId: user._id }).populate(buildManagerPopulateQuery()).lean();
