...
function getManagerProfileFromAgent(managerDoc) {
  const agent = managerDoc?.agentId || {};
  const user = agent.userId || {};
  const unit = agent.unitId || {};
  const branch = unit.branchId || {};
...
function formatManagerRecord(managerDoc, type) {
  ...
  const profile = getManagerProfileFromAgent(managerDoc);
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (user.role === "AG") {
      ...
      const profile = getManagerProfileFromAgent(manager);
...
async function buildAdminOrganizationTree(areaSearchRaw = "") {
  ...
  const searchRegex = areaSearch ? new RegExp(escapeRegex(areaSearch), "i") : null;
  ...
  if (searchRegex) {
    ...
    for (const area of areas) {
      if (searchRegex.test(area.areaName || "")) {
        matchedAreaIds.add(String(area._id));
      }
    }

    for (const branch of branches) {
      if (searchRegex.test(branch.branchName || "")) {
        matchedAreaIds.add(String(branch.areaId));
      }
    }

    for (const unit of units) {
      if (!searchRegex.test(unit.unitName || "")) continue;
      const branch = branchById.get(String(unit.branchId));
      if (branch?.areaId) matchedAreaIds.add(String(branch.areaId));
    }
    ...
    for (const agent of agents) {
      ...
      if (!searchHaystack || !searchRegex.test(searchHaystack)) continue;

      const unit = unitById.get(String(agent.unitId));
      const branch = unit ? branchById.get(String(unit.branchId)) : null;
      if (branch?.areaId) matchedAreaIds.add(String(branch.areaId));
    }

    filteredAreas = areas.filter((area) => matchedAreaIds.has(String(area._id)));
  }
...
app.post("/api/admin/organization/managers/assign", async (req, res) => {
  try {
    ...
    const sourceAgent = await Agent.findById(sourceAgentId)
      .populate({ path: "userId", select: "role username firstName lastName" })
      .populate({
        path: "unitId",
        select: "unitName branchId",
        populate: {
    ...
    const nextManager = existingManagerRecord
      ? await ManagerModel.findByIdAndUpdate(
          existingManagerRecord._id,
          { agentId: sourceAgentId, isBlocked: false, blockedAt: null },
          { new: true, runValidators: true }
        )
          .populate(buildManagerPopulateQuery())
          .lean()
      : await ManagerModel.create({ agentId: sourceAgentId, isBlocked: false, blockedAt: null }).then((doc) =>
          ManagerModel.findById(doc._id).populate(buildManagerPopulateQuery()).lean()
        );

    await User.findByIdAndUpdate(sourceAgent.userId._id, { role: managerType }, { new: false });
