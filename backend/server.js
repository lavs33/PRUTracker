...
app.get("/api/clients/relationship/dashboard", async (req, res) => {
  try {
    ...
    const buildSeries = (items, dateKey) => {
      ...
      const desiredBuckets = unit === "week" ? 6 : 6;
      ...
      const seriesStart = startDate
        ? bucketDate(startDate, unit)
        : validDates.length
          ? bucketDate(validDates.sort((a, b) => a - b)[Math.max(0, validDates.length - desiredBuckets)], unit)
          : bucketDate(now, unit);
    ...
    const weakestStage = [...stageProgress].sort((a, b) => a.count - b.count)[0] || null;
