  ...
  const [tasks, prospects, longLeaves] = await Promise.all([
    ...
    scopedAgentIds.length || scopedUserIds.length
      ? LongLeave.find({
          ...
          .select("agentId userId leaveStartDate leaveEndDate leaveApplicationForm approvedLeaveProof status includeOngoingPolicyholders createdAt updatedAt")
...
app.patch("/api/manager/long-leave/:longLeaveId/status", async (req, res) => {
  try {
    ...
    const longLeave = await LongLeave.findByIdAndUpdate(
      longLeaveId,
      { $set: { status, ...(req.body?.includeOngoingPolicyholders !== undefined ? { includeOngoingPolicyholders: req.body.includeOngoingPolicyholders === true } : {}) } },
