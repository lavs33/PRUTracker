...
app.post("/api/manager/retirement/:retirementId/reassign-prospect", async (req, res) => {
  try {
    ...
    const leads = await Lead.find({ prospectId: prospect._id }).select("_id").lean();
    const leadIds = leads.map((lead) => lead._id);
    const engagements = leadIds.length ? await LeadEngagement.find({ leadId: { $in: leadIds } }).select("_id").lean() : [];
    const engagementIds = engagements.map((engagement) => engagement._id);
    ...
    retirement.affectedProspects = (Array.isArray(retirement.affectedProspects) ? retirement.affectedProspects : []).map((item) => {
      const itemObject = typeof item?.toObject === "function" ? item.toObject() : item;
      if (String(itemObject.prospectId || itemObject.id || "") !== String(prospect._id)) return itemObject;
      return {
        ...itemObject,
        reassigned: true,
        reassignedAt,
    ...
    return res.json({
      message: `${formatPersonName(prospect)} has been reassigned to ${reassigneeName} from ${originalAgentName || "the retired agent"}.`,
      retirement,
      reassignment: {
        reassignedAt,
        reassignedToUserId,
