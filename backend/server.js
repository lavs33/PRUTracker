...
function normalizeKpiList(scopeType, savedKpis = []) {
  ...
  return defaults.map((definition) => {
    ...
    const primaryTarget = targets.find((target) => target.period === (saved.period || definition.period)) || targets[0] || {};
    return {
      ...definition,
    ...
    for (const definition of defaults) {
      ...
      for (const period of KPI_FREQUENCIES) {
        ...
        if ((hasMin && !hasMax) || (!hasMin && hasMax)) {
          return res.status(400).json({ message: `${definition.label} (${period}): Min and max must both be filled or both be blank.` });
        }
      ...
      const primaryTarget = normalizedTargets.find((target) => target.period === definition.period) || normalizedTargets[0] || {};
