...
async function buildManagerPortalPayload(user, { taskDatePreset = "ALL", salesDatePreset = "ALL", unitPerformanceDatePreset = "ALL" } = {}) {
  ...
  const applyTaskMetrics = (taskList, metricsByUserId) => {
    for (const task of taskList) {
      ...
      const isDueThisMonth = Number.isFinite(dueAtMs) && dueAtMs >= currentMonthStartMs && dueAtMs <= currentMonthEndMs;
      if (taskType === "APPROACH" && isDueThisMonth) metrics.openApproachTasksDueThisMonth += 1;
  ...
  const reassignmentMonthlyRows = buildRowsForUnitPerformanceContext(currentMonthContext);
  ...
  const agentKpiAssignment = context.branchId
    ...
    : null;
  const currentMonthApproachKpi = normalizeKpiList("AGENT", agentKpiAssignment?.kpis || [])
    .find((kpi) => kpi.key === "weekly_approaches");
  const currentMonthApproachAssignment = currentMonthApproachKpi?.monthlyAssignments?.find((row) => row.monthKey === currentMonthKey) || {};
  const currentMonthApproachTarget = Number([
    currentMonthApproachAssignment.targetValue,
    currentMonthApproachAssignment.targetMin,
    currentMonthApproachAssignment.targetMax,
    currentMonthApproachKpi?.targetValue,
    currentMonthApproachKpi?.targetMin,
    currentMonthApproachKpi?.targetMax,
  ].find((value) => Number.isFinite(Number(value)) && Number(value) > 0) || 0) || 50;
