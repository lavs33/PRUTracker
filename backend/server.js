...
app.get("/api/sales/performance", async (req, res) => {
  try {
    ...
    const resolveSourceBucket = (sourceValue) => {
      if (sourceValue === "Agent-Sourced") return "Agent-Sourced";
      if (sourceValue === "System-Assigned") return "System-Assigned";
      return "Unknown";
    };

    for (const lead of leads) {
      const prospectId = String(lead?.prospectId || "");
      const bucket = resolveSourceBucket(prospectToSource.get(prospectId));
      leadSourceBreakdown[bucket].totalLeads += 1;
    }
    ...
    for (const engagementId of convertedLeadMomentsByEngagement.keys()) {
      ...
      const bucket = resolveSourceBucket(src);

      if (bucket === "Agent-Sourced") conversionBySource["Agent-Sourced"] += 1;
      if (bucket === "System-Assigned") conversionBySource["System-Assigned"] += 1;
      leadSourceBreakdown[bucket].convertedLeads += 1;
    }

    for (const sourceMetrics of Object.values(leadSourceBreakdown)) {
