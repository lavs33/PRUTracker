...
function formatManagerRecord(managerDoc, type) {
  ...
  return {
    ...
    dateEmployed: managerDoc.dateEmployed || profile.user?.dateEmployed || null,
    managerType: type,
  ...
  try {
    ...
    const nextManagerRecord = existingManagerRecord
      ? await ManagerModel.findByIdAndUpdate(
          ...
          { agentId: sourceAgentId, ...scopeUpdate, dateEmployed: employedDate, isBlocked: false, blockedAt: null },
          { new: true, runValidators: true }
        )
      : await ManagerModel.create({ agentId: sourceAgentId, ...scopeUpdate, dateEmployed: employedDate, isBlocked: false, blockedAt: null }).then((doc) =>
          doc.toObject()
        );
