...
async function buildManagerPortalPayload(user, { taskDatePreset = "ALL", salesDatePreset = "ALL", unitPerformanceDatePreset = "ALL" } = {}) {
  ...
  const [tasks, prospects, longLeaves, retirements] = await Promise.all([
    ...
    scopedAgentIds.length || scopedUserIds.length
      ? Retirement.find({
          ...
          .select("agentId userId retirementDate retirementLetter approvedRetirementProof status includeOngoingPolicyholders affectedProspects affectedPolicyholders createdAt updatedAt")
          .sort({ createdAt: -1, _id: -1 })
          .lean()
      : [],
  ]);
  ...
  for (const retirement of retirements) {
    ...
    records.push({
      ...
      includeOngoingPolicyholders: retirement.includeOngoingPolicyholders === true,
  ...
  const leadCodesByProspectId = new Map();
  for (const lead of leads) {
    const prospectId = String(lead?.prospectId || "");
    if (!prospectId) continue;
    const codes = leadCodesByProspectId.get(prospectId) || [];
    if (lead?.leadCode) codes.push(lead.leadCode);
    leadCodesByProspectId.set(prospectId, codes);
  }
  const policyholderCodesByProspectId = new Map();
  for (const policyholder of policyholders) {
    const leadId = engagementIdToLeadId.get(String(policyholder?.leadEngagementId || "")) || "";
    const prospectId = leadIdToProspectId.get(leadId) || "";
    if (!prospectId) continue;
    const codes = policyholderCodesByProspectId.get(prospectId) || [];
    if (policyholder?.policyholderCode) codes.push(policyholder.policyholderCode);
    policyholderCodesByProspectId.set(prospectId, codes);
  }
  ...
  for (const prospect of prospects) {
    ...
    rows.push({
      ...
      leadCodes: leadCodesByProspectId.get(prospectId) || [],
      policyholderCodes: policyholderCodesByProspectId.get(prospectId) || [],
    });
  ...
  try {
    ...
    const retirement = await Retirement.findByIdAndUpdate(
      retirementId,
      {
        $set: {
          status: nextStatus,
          includeOngoingPolicyholders: req.body?.includeOngoingPolicyholders === true,
    ...
    if (nextStatus === "Endorsed") await Agent.findByIdAndUpdate(retirement.agentId, { $set: { status: "Retired" } });
