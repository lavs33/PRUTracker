...
app.get("/api/prospects/:prospectId/leads/:leadId/engagement", async (req, res) => {
  try {
    ...
    const contactAttempts = attempts.map((a) => ({
      ...
      meetingEndAt: a.meetingEndAt || null,
      meetingDurationMin: Number(a.meetingDurationMin || 0) || null,
      meetingMode: a.meetingMode || "",
      meetingPlatform: a.meetingPlatform || "",
      meetingPlatformOther: a.meetingPlatformOther || "",
      meetingLink: a.meetingLink || "",
      meetingInviteSent: Boolean(a.meetingInviteSent),
      meetingPlace: a.meetingPlace || "",
    }));
...
async function getAgentMeetingWindows(userObjectId, from, to, session) {
  ...
  const leads = await Lead.find({ prospectId: { $in: prospectIds } })
    .select("_id")
    .session(session || null)
    .lean();
  ...
  const engagements = await LeadEngagement.find({ leadId: { $in: leadIds } })
    .select("_id")
    .session(session || null)
    .lean();
  ...
  if (from || to) {
    q.meetingAt = { ...(q.meetingAt || {}) };
    if (from) q.meetingAt.$gte = from;
    if (to) q.meetingAt.$lt = to;
  }

  const attempts = await ContactAttempt.find(q)
    .select("meetingAt meetingEndAt meetingDurationMin")
    .session(session || null)
    .lean();

  return attempts
    .map((a) => {
      const start = a.meetingAt ? new Date(a.meetingAt) : null;
      if (!start || Number.isNaN(start.getTime())) return null;

      let end = a.meetingEndAt ? new Date(a.meetingEndAt) : null;
      if (!end || Number.isNaN(end.getTime())) {
        const duration = Number(a.meetingDurationMin || 120);
...
app.post("/api/prospects/:prospectId/leads/:leadId/schedule-meeting", async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      attempt.meetingAt = dt;
      attempt.meetingEndAt = endAt;
      attempt.meetingDurationMin = durationMin;
      attempt.meetingMode = mode;
      attempt.meetingPlatform = mode === "Online" ? platform : undefined;
      attempt.meetingPlatformOther = mode === "Online" && platform === "Other" ? platformOther : undefined;
      attempt.meetingLink = mode === "Online" ? link : undefined;
      attempt.meetingInviteSent = Boolean(meetingInviteSent);
      attempt.meetingPlace = mode === "Face-to-face" ? place : undefined;
