...
app.get("/api/prospects/:prospectId/leads/:leadId/engagement", async (req, res) => {
  try {
    ...
    const selectedProductId = needsAssessment?.needsPriorities?.productSelection?.selectedProductId;
    const selectedProduct = selectedProductId && mongoose.isValidObjectId(selectedProductId)
      ? await Product.findById(selectedProductId)
          .select("_id productName description")
          .lean()
      : null;

    const proposalDoc = await Proposal.findOne({ leadEngagementId: engagement._id })
      .select("currentActivityKey chosenProduct generateProposal")
      .lean();

    const proposalCurrentActivityKey = String(proposalDoc?.currentActivityKey || engagement?.currentActivityKey || "Generate Proposal").trim() || "Generate Proposal";
    ...
    return res.json({
      ...
      engagement: {
        ...
        proposal: {
          ...
          chosenProduct: proposalDoc?.chosenProduct?.productId || proposalDoc?.chosenProduct?.productName || selectedProduct
            ? {
                _id: proposalDoc?.chosenProduct?.productId || selectedProduct?._id || null,
                productName: proposalDoc?.chosenProduct?.productName || selectedProduct?.productName || "",
                description: proposalDoc?.chosenProduct?.description || selectedProduct?.description || "",
              }
            : null,
          generateProposal: {
            productId: proposalDoc?.chosenProduct?.productId || selectedProduct?._id || null,
            productName: proposalDoc?.chosenProduct?.productName || selectedProduct?.productName || "",
            productDescription: proposalDoc?.chosenProduct?.description || selectedProduct?.description || "",
            ...
            generatedAt: proposalSaved?.generatedAt || null,
          },
...
app.get("/api/prospects/:prospectId/leads/:leadId/needs-assessment", async (req, res) => {
  try {
    ...
    const effectiveNeedsActivityKey = needsSteps.includes(engagementActivity)
      ...
      ? "Record Prospect Attendance"
      : ["Perform Needs Analysis", "Schedule Proposal Presentation"].includes(String(needsAssessment.outcomeActivity || ""))
      ? "Schedule Proposal Presentation"
      : "Perform Needs Analysis";
    ...
    await session.withTransaction(async () => {
      ...
      await Proposal.updateOne(
        { leadEngagementId: engagement._id },
        {
          ...
          $set: {
            currentActivityKey: "Generate Proposal",
          },
        },
        { upsert: true, session }
      );
...
app.post("/api/prospects/:prospectId/leads/:leadId/proposal/generate", async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const proposalDoc = await Proposal.findOne({ leadEngagementId: engagement._id })
        .select("currentActivityKey")
        .session(session)
        .lean();

      const activityKey = String(proposalDoc?.currentActivityKey || engagement.currentActivityKey || "Generate Proposal").trim() || "Generate Proposal";
      ...
      await Proposal.updateOne(
        { leadEngagementId: engagement._id },
        {
          ...
          $set: {
            ...
            chosenProduct: {
              productId: selectedProduct?._id || (mongoose.isValidObjectId(selectedProductId) ? new mongoose.Types.ObjectId(selectedProductId) : null),
              productName: String(chosenProductName || selectedProduct?.productName || "").trim(),
              description: String(chosenProductDescription || selectedProduct?.description || "").trim(),
            },
            generateProposal: {
