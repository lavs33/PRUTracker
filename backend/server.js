...
app.get("/api/prospects/:prospectId/leads/:leadId/engagement", async (req, res) => {
  try {
    ...
    const proposalDoc = await Proposal.findOne({ leadEngagementId: engagement._id })
      .select("outcomeActivity chosenProductId generateProposal")
      .lean();

    const selectedProductId = proposalDoc?.chosenProductId || needsAssessment?.needsPriorities?.productSelection?.selectedProductId;
    ...
    const proposalCurrentActivityKey = String(engagement?.currentActivityKey || proposalDoc?.outcomeActivity || "Generate Proposal").trim() || "Generate Proposal";
