...
function buildDefaultKpiTargets(definition = {}, saved = {}) {
  ...
  return KPI_FREQUENCIES.map((period) => {
    ...
    return {
      period,
      targetMin: target.targetMin ?? saved.targetMin ?? definition.targetMin ?? null,
      targetMax: target.targetMax ?? saved.targetMax ?? definition.targetMax ?? null,
      targetValue: target.targetValue ?? saved.targetValue ?? definition.targetValue ?? null,
    };
  });
}
