...
app.get("/api/prospects/:prospectId/leads/:leadId/needs-assessment", async (req, res) => {
  try {
    ...
    const prospect = await Prospect.findOne({ _id: prospectObjectId, assignedToUserId: userObjectId })
      .select("firstName middleName lastName sex civilStatus birthday age occupation")
      .lean();
    ...
    let needsAssessment = await NeedsAssessment.findOne({ leadEngagementId: engagement._id })
      .select("currentActivityKey attendanceConfirmed attendedAt needsAnalysisCompletedAt proposalPresentationScheduledAt dependents")
      .lean();
    ...
    return res.json({
      ...
      needsAssessment: {
        currentActivityKey: needsAssessment.currentActivityKey || "Record Prospect Attendance",
        attendanceConfirmed: Boolean(needsAssessment.attendanceConfirmed),
        attendedAt: needsAssessment.attendedAt || null,
        needsAnalysisCompletedAt: needsAssessment.needsAnalysisCompletedAt || null,
        proposalPresentationScheduledAt: needsAssessment.proposalPresentationScheduledAt || null,
...
app.post("/api/prospects/:prospectId/leads/:leadId/needs-assessment/attendance", async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (na.currentActivityKey === "Record Prospect Attendance") {
        na.currentActivityKey = "Perform Needs Analysis";
      }
      await na.save({ session });

      if (engagement.currentStage === "Needs Assessment") {
        engagement.currentActivityKey = na.currentActivityKey;
        await engagement.save({ session });
      }
    });
...
app.put("/api/prospects/:prospectId/leads/:leadId/needs-assessment", async (req, res) => {
  ...
  try {
    const { userId } = req.query;
    const { prospectId, leadId } = req.params;
    const { basicInformation, dependents } = req.body || {};
    ...
    await session.withTransaction(async () => {
      ...
      if (!na.attendanceConfirmed) {
        throw Object.assign(new Error("Record prospect attendance first."), { status: 409 });
      }
      ...
      prospect.occupation = occupation || prospect.occupation;
      ...
      na.needsAnalysisCompletedAt = new Date();
      na.currentActivityKey = "Schedule Proposal Presentation";
      await na.save({ session });
...
app.post("/api/prospects/:prospectId/leads/:leadId/needs-assessment/schedule-proposal", async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const na = await NeedsAssessment.findOne({ leadEngagementId: engagement._id }).session(session);
      if (!na || na.currentActivityKey !== "Schedule Proposal Presentation") {
      ...
      na.proposalPresentationScheduledAt = new Date();
      await na.save({ session });
