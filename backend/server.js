...
app.post("/api/prospects/:prospectId/leads/:leadId/policy-issuance/coverage-duration", async (req, res) => {
  try {
    ...
    const policyDoc = await Policy.findOne({ leadEngagementId: engagement._id })
      .select("chosenProductId uploadPolicySummary.policyNumber recordPolicyApplicationStatus.issuanceDate")
      .lean();
    if (!policyDoc) return res.status(404).json({ message: "Policy record not found." });

    const productId = policyDoc?.chosenProductId;
    if (!productId || !mongoose.isValidObjectId(productId)) {
      return res.status(409).json({ message: "Chosen product is missing for this policy." });
    }

    const product = await Product.findById(productId)
      .select("paymentTermOptions coverageDurationRule")
      .lean();
    if (!product) return res.status(404).json({ message: "Chosen product not found." });

    const issuanceDate = policyDoc?.recordPolicyApplicationStatus?.issuanceDate
      ? new Date(policyDoc.recordPolicyApplicationStatus.issuanceDate)
      : null;
    if (!issuanceDate || Number.isNaN(issuanceDate.getTime())) {
      return res.status(409).json({ message: "Policy issuance date is required before saving coverage duration details." });
    }

    const birthDate = prospect?.birthday ? new Date(prospect.birthday) : null;
    if (!birthDate || Number.isNaN(birthDate.getTime())) {
      return res.status(400).json({ message: "Prospect birthday is required to compute age-based terms." });
    }
    ...
    if (issuanceAge < 0) return res.status(400).json({ message: "Invalid prospect birthday for issuance-age computation." });

    const paymentOptions = Array.isArray(product?.paymentTermOptions) ? product.paymentTermOptions : [];
    if (!paymentOptions.length) {
      return res.status(400).json({ message: "Chosen product has no payment-term options configured." });
    }
    ...
    const coverageYears = coverageDurationYears !== undefined && coverageDurationYears !== null && coverageDurationYears !== ""
      ? Number(coverageDurationYears)
      : null;
    ...
    if (!matchedPayment) {
      return res.status(400).json({ message: "Selected payment term is invalid for the chosen product." });
    }

    const coverageRule = product?.coverageDurationRule || null;
    if (!coverageRule || !coverageRule.type) {
      return res.status(400).json({ message: "Coverage duration rule is not configured for the chosen product." });
    }
    ...
    if (normalizedCoverageType !== coverageRuleType || normalizedCoverageLabel !== coverageRuleLabel) {
      return res.status(400).json({ message: "Coverage duration selection does not match product rule." });
    }

    const computedCoverageUntilAge = coverageRuleType === "RANGE_TO_AGE" ? coverageUntilAge : (coverageRule?.untilAge ?? null);
    const computedCoverageYears = coverageRuleType === "FIXED_YEARS" ? (coverageRule?.years ?? null) : (coverageRuleType === "RANGE_TO_AGE" ? null : null);
    ...
    if (coverageRuleType === "RANGE_TO_AGE") {
      ...
      if (!Number.isFinite(coverageUntilAge) || coverageUntilAge < minAge || coverageUntilAge > maxAge) {
        return res.status(400).json({ message: `Coverage duration age must be between ${minAge} and ${maxAge}.` });
      }
    }
    ...
    if (!Number.isFinite(yearsToAdd) || yearsToAdd <= 0) {
      return res.status(400).json({ message: "Unable to compute policy end date from selected coverage duration." });
    }
