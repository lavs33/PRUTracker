...
async function buildAdminOrganizationTree(areaSearchRaw = "") {
  ...
  for (const manager of branchManagers) {
    ...
    bmByBranchId.set(branchId, formatManagerLabel(profile.user));
  }
  ...
  for (const manager of unitManagers) {
    ...
    umByUnitId.set(unitId, formatManagerLabel(profile.user));
  }
  ...
  for (const manager of assistantUnitManagers) {
    ...
    const unitId = profile.unit?._id ? String(profile.unit._id) : "";
    if (!unitId || !unitIds.some((id) => String(id) === unitId)) continue;
    aumByUnitId.set(unitId, formatManagerLabel(profile.user));
  }
