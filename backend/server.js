...
app.put("/api/manager/kpi-assignments/:scopeType/:scopeId", async (req, res) => {
  try {
    ...
    for (const definition of defaults) {
      ...
      if (assigned === false) {
        const normalizedTargets = KPI_FREQUENCIES.map((period) => {
          ...
          return {
            period,
            targetMin: hasTarget ? null : (hasMin ? targetMin : null),
            targetMax: hasTarget ? null : (hasMax ? targetMax : null),
            targetValue: hasMin || hasMax ? null : (hasTarget ? targetValue : null),
          };
        });
