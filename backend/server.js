...
function nextMonthKey(value = new Date()) {
  const [year, month] = monthKeyForDate(value).split("-").map(Number);
...
function kpiNotificationResolution(type = "") {
  return String(type).endsWith("_KPI_UNASSIGNED")
    ? { resolutionStatus: "Unresolved", resolvedAt: null }
    : { resolutionStatus: "Not Applicable", resolvedAt: null };
}

function kpiUnassignedManagerMessage(message, label, scopeLabel) {
  return `${message} Please assign a target in order to track progress for ${label} for ${scopeLabel}.`;
}

async function resolvePriorKpiUnassignedNotifications({ recipientIds, scopeType, assignmentId, kpiKey, monthKey, type }) {
  if (!String(type).endsWith("_KPI_ASSIGNED") || !recipientIds.length) return;
  await Notification.updateMany(
    {
      assignedToUserId: { $in: recipientIds },
      type: `${scopeType}_KPI_UNASSIGNED`,
      entityType: "KpiAssignment",
      entityId: assignmentId,
      "metadata.scopeType": scopeType,
      "metadata.kpiKey": kpiKey,
...
async function createBranchKpiNotifications({ branchId, branchName, assignmentId, previousKpi, nextKpi }) {
  ...
  const kpiKey = nextKpi?.key || previousKpi?.key || "";
  const branchManagerIds = new Set(branchManagers.map((manager) => String(manager.userId || "")).filter(Boolean));
  ...
  await Notification.insertMany(recipientIds.map((assignedToUserId) => ({
    ...
    message: type === "BRANCH_KPI_UNASSIGNED" && branchManagerIds.has(String(assignedToUserId))
      ? kpiUnassignedManagerMessage(message, label, branchName || "the branch")
      : message,
    ...kpiNotificationResolution(type),
    entityType: "KpiAssignment",
    entityId: assignmentId,
    metadata: { scopeType: "BRANCH", scopeId: String(branchId), kpiKey, monthKey },
  })));
}

async function createUnitKpiNotifications({ branchId, branchName, assignmentId, previousKpi, nextKpi }) {
  ...
  const notifications = units.flatMap((unit) => [...(recipientsByUnit.get(String(unit._id)) || [])].map((assignedToUserId) => ({
    ...
    entityType: "KpiAssignment",
    entityId: assignmentId,
    metadata: { scopeType: "UNIT", scopeId: String(unit._id), branchAssignmentScopeId: String(branchId), kpiKey, monthKey },
  })));
  notifications.push(...managerRecipientIds.map((assignedToUserId) => ({
    assignedToUserId,
    type,
    title: `${label} KPI ${titleAction} for all units in ${branchName || "Branch"}.`,
    message: type === "UNIT_KPI_UNASSIGNED"
      ? kpiUnassignedManagerMessage(message, label, `all units in ${branchName || "the branch"}`)
      : message,
    ...kpiNotificationResolution(type),
    entityType: "KpiAssignment",
    entityId: assignmentId,
    metadata: { scopeType: "UNIT", scopeId: String(branchId), kpiKey, monthKey },
  })));
...
async function createAgentKpiNotifications({ branchId, branchName, assignmentId, previousKpi, nextKpi }) {
  ...
  const branchManagers = await BM.find({ branchId, isBlocked: { $ne: true } }).select("userId").lean();
  const managerRecipientIds = [...new Set(branchManagers.map((manager) => String(manager.userId || "")).filter(Boolean))];
  ...
  const notifications = recipients.map((recipient) => ({
    ...
    entityType: "KpiAssignment",
    entityId: assignmentId,
    metadata: { scopeType: "AGENT", scopeId: String(branchId), kpiKey, monthKey },
  }));
  notifications.push(...managerRecipientIds.map((assignedToUserId) => ({
    assignedToUserId,
    type,
    title: `${label} KPI ${titleAction} for agents in ${branchName || "Branch"}.`,
    message: type === "AGENT_KPI_UNASSIGNED"
      ? kpiUnassignedManagerMessage(message, label, `all agents in ${branchName || "the branch"}`)
      : message,
    ...kpiNotificationResolution(type),
    entityType: "KpiAssignment",
    entityId: assignmentId,
...
const automaticKpiNotificationRuns = new Map();

async function ensureCurrentMonthKpiUnassignedNotifications({ branchId, branchName }) {
  const monthKey = monthKeyForDate();
  const runKey = `${branchId}:${monthKey}`;
  if (automaticKpiNotificationRuns.has(runKey)) return automaticKpiNotificationRuns.get(runKey);
  const run = (async () => {
    for (const scopeType of ["AGENT", "UNIT", "BRANCH"]) {
      let assignment = await KpiAssignment.findOne({ scopeType, scopeId: branchId }).lean();
      if (!assignment) {
        const kpis = normalizeKpiList(scopeType, []).map((kpi) => ({
          key: kpi.key,
          label: kpi.label,
          valueType: kpi.valueType,
          monthlyAssignments: kpi.monthlyAssignments,
        }));
        assignment = await KpiAssignment.findOneAndUpdate(
          { scopeType, scopeId: branchId },
          { $setOnInsert: { kpis } },
          { new: true, upsert: true, setDefaultsOnInsert: true }
        ).lean();
      }
      const kpis = normalizeKpiList(scopeType, assignment.kpis || []);
      for (const kpi of kpis) {
        const current = kpi.monthlyAssignments.find((row) => row.monthKey === monthKey);
        if (current?.assigned === true) continue;
        const notificationType = `${scopeType}_KPI_UNASSIGNED`;
        const alreadyGenerated = await Notification.exists({
          entityType: "KpiAssignment",
          entityId: assignment._id,
          type: notificationType,
          "metadata.kpiKey": kpi.key,
          "metadata.monthKey": monthKey,
          softDeletedAt: null,
        });
        ...
        const previousKpi = {
          ...kpi,
          monthlyAssignments: kpi.monthlyAssignments.map((row) => row.monthKey === monthKey
            ? { ...row, assigned: true }
            : row),
        };
        const args = { branchId, branchName, assignmentId: assignment._id, previousKpi, nextKpi: kpi };
        if (scopeType === "AGENT") await createAgentKpiNotifications(args);
        else if (scopeType === "UNIT") await createUnitKpiNotifications(args);
        else await createBranchKpiNotifications(args);
      }
    }
  })().finally(() => automaticKpiNotificationRuns.delete(runKey));
  automaticKpiNotificationRuns.set(runKey, run);
  return run;
}

async function ensureAllBranchesCurrentMonthKpiNotifications() {
  const managers = await BM.find({ isBlocked: { $ne: true }, branchId: { $ne: null } }).select("branchId").lean();
  const branchIds = [...new Set(managers.map((manager) => String(manager.branchId || "")).filter(Boolean))];
  if (!branchIds.length) return;
  const branches = await Branch.find({ _id: { $in: branchIds } }).select("branchName").lean();
  const branchNameById = new Map(branches.map((branch) => [String(branch._id), branch.branchName]));
  await Promise.all(branchIds.map((branchId) => ensureCurrentMonthKpiUnassignedNotifications({
    branchId,
    branchName: branchNameById.get(branchId) || "Branch",
  })));
}
...
function buildResignationEndorsementNotificationMessage(resignation = {}) {
  ...
  return `Prospects endorsed: ${prospects.map((prospect) => {
    ...
    return `${prospect.prospectCode || "—"} / Leads: ${leadCodes} / Policies: ${policyholderCodes} / ${prospect.name || "—"}`;
  }).join("; ")}.`;
}
...
async function buildManagerPortalPayload(user, { taskDatePreset = "ALL", salesDatePreset = "ALL", unitPerformanceDatePreset = monthKeyForDate(), reassignmentMonth = "" } = {}) {
  ...
  const selectedReassignmentMonth = /^\d{4}-(0[1-9]|1[0-2])$/.test(String(reassignmentMonth || ""))
    ? String(reassignmentMonth)
    : currentMonthKey;
  const [reassignmentYear, reassignmentMonthNumber] = selectedReassignmentMonth.split("-").map(Number);
  const reassignmentMonthContext = {
    key: selectedReassignmentMonth,
    startDate: new Date(Date.UTC(reassignmentYear, reassignmentMonthNumber - 1, 1) - (8 * 60 * 60 * 1000)),
    endDate: new Date(Date.UTC(reassignmentYear, reassignmentMonthNumber, 1) - (8 * 60 * 60 * 1000) - 1),
    periodLabel: formatKpiMonthLabel(selectedReassignmentMonth),
  };
  ...
  const buildRows = (metricsByUserId) =>
    [...metricsByUserId.values()].map((metrics) => {
      ...
      return {
        ...
        persistencyTotalPolicies: metrics.persistencyTotalPolicies,
        persistedPolicies: metrics.persistedPolicies,
  ...
  const [policyholders, policies, applications, needsAssessments, payments, annualPayments] = await Promise.all([
    scopedUserIds.length || engagementIds.length
      ? Policyholder.find({
          ...
          .select("assignedToUserId reassignedToUserId leadEngagementId productId policyholderCode policyNumber status statusHistory cancellationDetails.cancelledAt cancellationDetails.approvedCancellationDate createdAt updatedAt annualPaymentRecords")
  ...
  const policyholderStatusAsOf = (policyholder, asOfDate) => {
    const asOfMs = new Date(asOfDate).getTime();
    const history = [...(policyholder?.statusHistory || [])]
      .filter((entry) => new Date(entry?.effectiveAt).getTime() <= asOfMs)
      .sort((left, right) => new Date(right.effectiveAt).getTime() - new Date(left.effectiveAt).getTime());
    if (history[0]?.status) return String(history[0].status);
    const cancellationAt = policyholder?.cancellationDetails?.approvedCancellationDate
      || policyholder?.cancellationDetails?.cancelledAt;
    if (cancellationAt && new Date(cancellationAt).getTime() <= asOfMs) return "Cancelled";
    const updatedAtMs = new Date(policyholder?.updatedAt).getTime();
    if (Number.isFinite(updatedAtMs) && updatedAtMs <= asOfMs) return String(policyholder?.status || "Active");
    return "Active";
  };

  const applyPersistencyMetrics = (presetContext, metricsByUserId) => {
    const configuredEndMs = new Date(presetContext?.endDate || new Date()).getTime();
    const asOfDate = new Date(Math.min(Number.isFinite(configuredEndMs) ? configuredEndMs : Date.now(), Date.now()));
    for (const policyholder of policyholders) {
      const issuanceDate = policyIssuanceDateForPolicyholder(policyholder);
      if (new Date(issuanceDate).getTime() > asOfDate.getTime()) continue;
      const statusAsOf = policyholderStatusAsOf(policyholder, asOfDate);
      if (statusAsOf === "Cancelled") continue;
      const metrics = metricsByUserId.get(String(policyholder?.assignedToUserId || ""));
      if (!metrics) continue;
      metrics.persistencyTotalPolicies += 1;
      if (["Active", "Paid-Up"].includes(statusAsOf)) metrics.persistedPolicies += 1;
    }
  };
  ...
  const buildSalesMetricInput = (presetContext) => {
    ...
    const rangeAsOfDate = new Date(Math.min(Number.isFinite(presetEndMs) ? presetEndMs : Date.now(), Date.now()));
    ...
    )).map((policyholder) => ({ ...policyholder, status: policyholderStatusAsOf(policyholder, rangeAsOfDate) }));
  ...
  const buildRowsForSalesContext = (presetContext) => {
    ...
    applyTaskMetrics(buildTasksForPerformanceContext(presetContext), metricsByUserId, presetContext?.endDate || new Date());
    ...
    applyPersistencyMetrics(presetContext, metricsByUserId);
  ...
  const buildRowsForReassignmentMonth = () => {
    const metricsByUserId = createMetricsMap(scopedAgents);
    const isHistoricalMonth = selectedReassignmentMonth !== currentMonthKey;
    const reassignmentTasks = isHistoricalMonth
      ? tasks.flatMap((task) => {
          const createdAt = new Date(task?.createdAt || 0);
          if (Number.isNaN(createdAt.getTime()) || createdAt > reassignmentMonthContext.endDate) return [];
          const completedAt = new Date(task?.completedAt || 0);
          const completedInSelectedMonth = !Number.isNaN(completedAt.getTime())
            && completedAt >= reassignmentMonthContext.startDate
            && completedAt <= reassignmentMonthContext.endDate;
          if (completedInSelectedMonth) return [task];
          if (!Number.isNaN(completedAt.getTime()) && completedAt <= reassignmentMonthContext.endDate) return [];
          return [{ ...task, status: "Open", completedAt: null }];
        })
      : tasks;
    applyTaskMetrics(
      reassignmentTasks.filter((task) => {
        const isDone = String(task?.status || "").toLowerCase() === "done";
        return isDone ? isWithinPreset(task?.completedAt, reassignmentMonthContext, task?.updatedAt) : true;
      }),
      metricsByUserId
    );
    return buildRows(metricsByUserId);
  ...
  const reassignmentMonthlyRows = buildRowsForReassignmentMonth();
  const reassignmentMonthlySalesRows = buildRowsForSalesContext(reassignmentMonthContext);
  ...
  const agentKpiAssignment = context.branchId
    ...
    : null;
  const reassignmentMonthAgentKpis = normalizeKpiList("AGENT", agentKpiAssignment?.kpis || []);
  const reassignmentMonthKpiTarget = (kpiKey, fallback) => {
    const kpi = reassignmentMonthAgentKpis.find((item) => item.key === kpiKey);
    const assignment = kpi?.monthlyAssignments?.find((row) => row.monthKey === selectedReassignmentMonth) || {};
    return Number([
  ...
  const currentMonthApproachTarget = reassignmentMonthKpiTarget("weekly_approaches", 50);
  const currentMonthClosingRatioTarget = reassignmentMonthKpiTarget("monthly_closing_ratio", 20);
  const currentMonthActivePoliciesTarget = reassignmentMonthKpiTarget("monthly_policies", 2);
...
async function syncBmRecommendationResolutionsForUser(userId) {
  ...
  if (!notifications.length) return new Map();

  const user = await User.findOne({ _id: userId, role: "UM" }).lean();
  if (!user) return new Map();
  const currentMonth = monthKeyForDate();
  const relevant = notifications.filter((notification) => String(notification?.metadata?.monthKey || "") === currentMonth);
  if (!relevant.length) return new Map();

  const [portalResult, assignmentResult] = await Promise.all([
    buildManagerPortalPayload(user, { unitPerformanceDatePreset: currentMonth }),
    buildKpiAssignmentPayload(user),
  ]);
  const portal = portalResult?.payload;
  const assignments = assignmentResult?.payload?.assignments || [];
  if (!portal) return new Map();
  const allRows = portal.kpiSalesRowsByFrequency?.[currentMonth] || [];
  const unitRows = allRows.filter((row) => String(row?.unit || "") === String(portal.scope?.unitName || ""));
  const summarize = (rows) => rows.reduce((summary, row) => ({
    annualPremium: summary.annualPremium + Number(row?.annualPremium || 0),
    totalPolicies: summary.totalPolicies + Number(row?.totalPolicies || 0),
    activePolicies: summary.activePolicies + Number(row?.activePolicies || 0),
    persistencyTotalPolicies: summary.persistencyTotalPolicies + Number(row?.persistencyTotalPolicies || 0),
    persistedPolicies: summary.persistedPolicies + Number(row?.persistedPolicies || 0),
  }), { annualPremium: 0, totalPolicies: 0, activePolicies: 0, persistencyTotalPolicies: 0, persistedPolicies: 0 });
  const branchSummary = summarize(allRows);
  const unitSummary = summarize(unitRows);
  const branchAssignment = assignments.find((assignment) => assignment.scopeType === "BRANCH");
  const unitAssignment = assignments.find((assignment) => assignment.scopeType === "UNIT");
  const monthlyKpi = (assignment, key) => {
    const kpi = (assignment?.kpis || []).find((item) => item.key === key) || {};
    const month = (kpi.monthlyAssignments || []).find((item) => item.monthKey === currentMonth) || {};
    return { ...kpi, ...month };
  };
  const targetIsMet = (actual, kpi) => {
    const hasMin = String(kpi?.targetMin ?? "").trim() !== "";
    const hasValue = String(kpi?.targetValue ?? "").trim() !== "";
    const hasMax = String(kpi?.targetMax ?? "").trim() !== "";
    if (hasMin) return actual >= Number(kpi.targetMin);
    if (hasValue) return actual >= Number(kpi.targetValue);
    if (hasMax) return actual <= Number(kpi.targetMax);
    return false;
  };
  const branchSalesKpi = monthlyKpi(branchAssignment, "monthly_sales_production");
  const branchSalesTarget = Number([branchSalesKpi.targetValue, branchSalesKpi.targetMin, branchSalesKpi.targetMax].find((value) => String(value ?? "").trim() !== "") || 0);
  const actualByKey = {
    monthly_sales_production: unitSummary.annualPremium,
    monthly_target_achievement_index: branchSalesTarget ? Math.round((branchSummary.annualPremium / branchSalesTarget) * 100) : 0,
    monthly_active_agents: allRows.filter((row) => Number(row?.annualPremium || 0) > 0).length,
    monthly_persistency_rate: branchSummary.persistencyTotalPolicies ? Math.round((branchSummary.persistedPolicies / branchSummary.persistencyTotalPolicies) * 100) : 0,
  };
  const resolutions = new Map();
  const writes = relevant.flatMap((notification) => {
    const key = String(notification?.metadata?.kpiKey || "");
    const kpi = monthlyKpi(key === "monthly_sales_production" ? unitAssignment : branchAssignment, key);
    if (!targetIsMet(Number(actualByKey[key] || 0), kpi)) return [];
    const resolvedAt = new Date();
    resolutions.set(String(notification._id), { resolutionStatus: "Resolved", resolvedAt });
    return [{ updateOne: { filter: { _id: notification._id, resolutionStatus: { $ne: "Resolved" } }, update: { $set: { resolutionStatus: "Resolved", resolvedAt } } } }];
  });
...
async function resolveFormerAgentClientNotifications({
  assignedToUserId,
  prospectId,
  reassignedToUserId,
  leadIds = [],
  ...
  if (!assignedToUserId || !prospectId) return;
  const stringLeadIds = leadIds.map((id) => String(id || "")).filter(Boolean);
  const stringPolicyholderIds = policyholderIds.map((id) => String(id || "")).filter(Boolean);
  const stringTaskIds = taskIds.map((id) => String(id || "")).filter(Boolean);
  const entityIds = [prospectId, ...leadIds, ...policyholderIds, ...taskIds].filter(Boolean);
  const relatedClauses = [
    { entityId: { $in: entityIds } },
    { "metadata.prospectId": String(prospectId) },
  ];
  if (stringLeadIds.length) relatedClauses.push({ "metadata.leadId": { $in: stringLeadIds } });
  if (stringPolicyholderIds.length) relatedClauses.push({ "metadata.policyholderId": { $in: stringPolicyholderIds } });
  if (stringTaskIds.length) relatedClauses.push({ "metadata.taskId": { $in: stringTaskIds } });

  const resolvedAt = new Date();
  const relatedNotificationFilter = { assignedToUserId, $or: relatedClauses };
  await Notification.updateMany(
    relatedNotificationFilter,
    {
      $set: {
        "metadata.transferredAway": true,
        "metadata.reassignedToUserId": String(reassignedToUserId || ""),
      },
    },
  );
  await Notification.updateMany(
...
app.post("/api/manager/resignation/:resignationId/reassign-prospect", async (req, res) => {
  try {
    ...
    await resolveFormerAgentClientNotifications({
      assignedToUserId: originalUserId,
      prospectId: prospect._id,
      reassignedToUserId,
      leadIds,
      policyholderIds: policyholders.map((policyholder) => policyholder._id),
      taskIds: oldTaskIds,
    });
    ...
    await Notification.create([
      {
        ...
        entityType: "Prospect",
        entityId: prospect._id,
        metadata: { prospectId: String(prospect._id), leadId: String(activeLead._id), transferredAway: true, reassignedToUserId: String(reassignedToUserId), longLeaveId: String(longLeave._id) },
        createdAt: orphanNotificationCreatedAt,
        updatedAt: orphanNotificationCreatedAt,
      },
      {
...
app.post("/api/manager/long-leave/:longLeaveId/reassign-policyholder", async (req, res) => {
  try {
    ...
    if (originalUserId) {
      const oldTaskIds = await Task.find({
        assignedToUserId: originalUserId,
      ...
      await resolveFormerAgentClientNotifications({
        ...
        reassignedToUserId,
        leadIds,
        policyholderIds: prospectPolicyholders.map((item) => item._id),
        taskIds: oldTaskIds,
      });
    }
    ...
    const orphanNotifications = [
      ...(originalUserId ? [{
        ...
        entityType: "Prospect",
        entityId: prospect._id,
        metadata: { prospectId: String(prospect._id), policyholderId: String(policyholder._id), transferredAway: true, reassignedToUserId: String(reassignedToUserId), longLeaveId: String(longLeave._id) },
        createdAt: orphanNotificationCreatedAt,
        updatedAt: orphanNotificationCreatedAt,
      }] : []),
      {
...
app.get("/api/manager/kpi-assignments", async (req, res) => {
  try {
    ...
    if (String(user.role || "").trim().toUpperCase() === "BM") {
      await ensureCurrentMonthKpiUnassignedNotifications({
        branchId: result.payload.scope.branchId,
        branchName: result.payload.scope.branchName,
      });
    }
...
app.get("/api/agent/kpi-progress", async (req, res) => {
  try {
    const { userId, month = monthKeyForDate() } = req.query;
    if (!userId) return res.status(400).json({ message: "Missing userId." });
    if (!mongoose.isValidObjectId(userId)) return res.status(400).json({ message: "Invalid userId." });
    ...
    const withinRange = (value) => {
      ...
      return Number.isFinite(ms) && ms >= startDate.getTime() && ms <= endDate.getTime();
    };
    ...
    const kpiForSelectedMonth = (kpi) => {
      const monthlyAssignment = kpi?.monthlyAssignments?.find((row) => row.monthKey === selectedMonth);
      return {
        ...kpi,
        monthKey: selectedMonth,
        period: "Monthly",
        assigned: monthlyAssignment?.assigned === true,
        targetMin: monthlyAssignment?.targetMin ?? null,
        targetMax: monthlyAssignment?.targetMax ?? null,
        targetValue: monthlyAssignment?.targetValue ?? null,
      };
    };
    const unitSalesProductionKpi = normalizeKpiList("UNIT", unitAssignment?.kpis || [])
      .map(kpiForSelectedMonth)
    ...
    const policyholders = await Policyholder.find({
      ...
      .lean();
    const policyEngagementIds = policyholders.map((policyholder) => policyholder.leadEngagementId).filter(Boolean);
    const issuedPolicies = policyEngagementIds.length
      ? await Policy.find({
        leadEngagementId: { $in: policyEngagementIds },
        "recordPolicyApplicationStatus.status": "Issued",
      }).select("leadEngagementId recordPolicyApplicationStatus.issuanceDate").lean()
      : [];
    const issuanceDateByEngagementId = new Map(issuedPolicies.map((policy) => [
      String(policy.leadEngagementId || ""), policy?.recordPolicyApplicationStatus?.issuanceDate || null,
    ]));
    const policyholderIssuedInRange = (policyholder) => withinRange(
    ...
    const activePolicyholdersInRange = policyholders.filter((policyholder) => (
      String(policyholder?.status || "").toLowerCase() === "active" && policyholderIssuedInRange(policyholder)
    ));
    ...
    if (unitSalesProductionKpiForPeriod) {
      ...
      const unitActivePolicyholderEngagementIds = unitActivePolicyholders
        ...
        .filter(Boolean);
      const unitIssuedPolicies = unitActivePolicyholderEngagementIds.length
        ? await Policy.find({
          leadEngagementId: { $in: unitActivePolicyholderEngagementIds },
          "recordPolicyApplicationStatus.status": "Issued",
        }).select("leadEngagementId recordPolicyApplicationStatus.issuanceDate").lean()
        : [];
      const unitIssuanceDateByEngagementId = new Map(unitIssuedPolicies.map((policy) => [
        String(policy.leadEngagementId || ""), policy?.recordPolicyApplicationStatus?.issuanceDate || null,
      ]));
      const unitPolicyholdersInRange = unitActivePolicyholders.filter((policyholder) => withinRange(
        unitIssuanceDateByEngagementId.get(String(policyholder.leadEngagementId || "")) || policyholder.createdAt
      ));
      const unitPolicyholderEngagementIdsInRange = unitPolicyholdersInRange
        .map((policyholder) => policyholder.leadEngagementId)
        .filter(Boolean);
      const unitAnnualPayments = unitPolicyholderEngagementIdsInRange.length
        ? await AnnualPayment.find({ leadEngagementId: { $in: unitPolicyholderEngagementIdsInRange } })
          .select("leadEngagementId annualPaymentPeriod totalAnnualPremiumPhp createdAt updatedAt")
          .lean()
        : [];
      const unitAnnualPaymentResolvers = buildAnnualPaymentResolvers(unitAnnualPayments);
      unitSalesProduction = unitPolicyholdersInRange
...
app.listen(PORT, () => {
  ...
  ensureAllBranchesCurrentMonthKpiNotifications().catch((error) => {
    console.error("Initial monthly KPI notification generation failed:", error);
  });
});

const monthlyKpiNotificationTimer = setInterval(() => {
  ensureAllBranchesCurrentMonthKpiNotifications().catch((error) => {
    console.error("Scheduled monthly KPI notification generation failed:", error);
  });
}, 60 * 60 * 1000);
monthlyKpiNotificationTimer.unref?.();