...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (user.role === "AG") {
      ...
      if (manager) {
        payload.unitName = manager.unitId?.unitName || "";
        payload.branchName = manager.unitId?.branchId?.branchName || "";
        payload.areaName = manager.unitId?.branchId?.areaId?.areaName || "";
      }
    } else if (user.role === "BM") {
      const manager = await BM.findOne({ userId: user._id })
        .populate({
          path: "branchId",
          select: "branchName areaId",
          populate: {
      ...
      if (manager) {
        payload.branchName = manager.branchId?.branchName || "";
        payload.areaName = manager.branchId?.areaId?.areaName || "";
      }
    }
...
========================================================= */
async function buildAdminOrganizationTree(areaSearchRaw = "") {
  const areaSearch = String(areaSearchRaw || "").trim();
  const areaQuery = {};

  if (areaSearch) {
    areaQuery.areaName = { $regex: new RegExp(escapeRegex(areaSearch), "i") };
  }

  const areas = await Area.find(areaQuery).sort({ areaName: 1 }).lean();
  ...
  const [branchManagers, unitManagers, assistantUnitManagers] = await Promise.all([
    branchIds.length
      ? BM.find({ branchId: { $in: branchIds } })
          .populate({ path: "userId", select: "username firstName lastName" })
          .lean()
      : [],
    unitIds.length
      ? UM.find({ unitId: { $in: unitIds } })
          .populate({ path: "userId", select: "username firstName lastName" })
          .lean()
      : [],
    unitIds.length
      ? AUM.find({ unitId: { $in: unitIds } })
          .populate({ path: "userId", select: "username firstName lastName" })
          .lean()
      : [],
  ]);
  ...
  const bmByBranchId = new Map(
    branchManagers.map((manager) => [String(manager.branchId), formatManagerLabel(manager.userId)])
  );
  const umByUnitId = new Map(unitManagers.map((manager) => [String(manager.unitId), formatManagerLabel(manager.userId)]));
  const aumByUnitId = new Map(
    assistantUnitManagers.map((manager) => [String(manager.unitId), formatManagerLabel(manager.userId)])
  );
  ...
  for (const unit of units) {
    ...
    unitsByBranchId.get(branchKey).push({
      ...
      um: umByUnitId.get(String(unit._id)) || "Unassigned",
      aum: aumByUnitId.get(String(unit._id)) || "Unassigned",
  ...
  for (const branch of branches) {
    ...
    branchesByAreaId.get(areaKey).push({
      ...
      bm: bmByBranchId.get(String(branch._id)) || "Unassigned",
...
app.get("/api/admin/organization/tree", async (req, res) => {
  try {
    const areas = await buildAdminOrganizationTree(req.query.areaSearch);
  ...
  try {
    const [areas, branches, units, agents, branchManagers, unitManagers, assistantUnitManagers] = await Promise.all([
      ...
      Agent.find()
        .populate({
          path: "userId",
          select: "username firstName middleName lastName birthday sex age displayPhoto dateEmployed",
        })
        .populate({
          path: "unitId",
          select: "unitName branchId",
    ...
    const mapManagerRecord = (manager, type) => ({
      managerId: manager._id,
      userId: manager.userId?._id || "",
      username: manager.userId?.username || "",
      firstName: manager.userId?.firstName || "",
      middleName: manager.userId?.middleName || "",
      lastName: manager.userId?.lastName || "",
      birthday: manager.userId?.birthday || null,
      sex: manager.userId?.sex || "",
      age: manager.userId?.age || "",
      displayPhoto: manager.userId?.displayPhoto || "",
      dateEmployed: manager.userId?.dateEmployed || null,
      managerType: type,
      branchId: type === "BM" ? manager.branchId?._id || "" : manager.unitId?.branchId?._id || "",
      branchName: type === "BM" ? manager.branchId?.branchName || "" : manager.unitId?.branchId?.branchName || "",
      areaId: type === "BM" ? manager.branchId?.areaId?._id || "" : manager.unitId?.branchId?.areaId?._id || "",
      areaName: type === "BM" ? manager.branchId?.areaId?.areaName || "" : manager.unitId?.branchId?.areaId?.areaName || "",
      unitId: type === "BM" ? "" : manager.unitId?._id || "",
      unitName: type === "BM" ? "" : manager.unitId?.unitName || "",
    });

    return res.json({
      areas: areas.map((area) => ({ id: area._id, areaName: area.areaName })),
      branches: branches.map((branch) => ({ id: branch._id, branchName: branch.branchName, areaId: branch.areaId })),
      units: units.map((unit) => ({ id: unit._id, unitName: unit.unitName, branchId: unit.branchId })),
      agents: agentOptions,
      managers: {
        bm: branchManagers.map((manager) => mapManagerRecord(manager, "BM")),
        um: unitManagers.map((manager) => mapManagerRecord(manager, "UM")),
        aum: assistantUnitManagers.map((manager) => mapManagerRecord(manager, "AUM")),
      },
    });
  } catch (err) {
...
app.delete("/api/admin/organization/areas/:areaId", async (req, res) => {
  try {
    ...
    const [branchManagers, unitManagers, assistantUnitManagers, agents] = await Promise.all([
      branchIds.length ? BM.find({ branchId: { $in: branchIds } }, { _id: 1, userId: 1 }).lean() : [],
      unitIds.length ? UM.find({ unitId: { $in: unitIds } }, { _id: 1, userId: 1 }).lean() : [],
      unitIds.length ? AUM.find({ unitId: { $in: unitIds } }, { _id: 1, userId: 1 }).lean() : [],
      unitIds.length ? Agent.find({ unitId: { $in: unitIds } }, { _id: 1, userId: 1 }).lean() : [],
    ]);
    const agentIds = agents.map((agent) => agent._id);
    const bmIds = branchManagers.map((manager) => manager._id);
    const umIds = unitManagers.map((manager) => manager._id);
    const aumIds = assistantUnitManagers.map((manager) => manager._id);
    const userIds = [
      ...branchManagers.map((manager) => manager.userId),
      ...unitManagers.map((manager) => manager.userId),
      ...assistantUnitManagers.map((manager) => manager.userId),
