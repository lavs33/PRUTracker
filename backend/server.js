...
app.post("/api/manager/retirement/:retirementId/reassign-prospect", async (req, res) => {
  try {
    ...
    await Task.deleteMany({ assignedToUserId: originalUserId, prospectId: prospect._id, status: { $in: ["Open", "Overdue"] } });
    ...
    if (taskNotificationPayload?.task) {
      ...
      if (isDueTodayInManila(task.dueAt)) {
        ...
        await Notification.updateOne(
          { assignedToUserId: reassignedToUserId, dedupeKey: `TASK_DUE_TODAY:${task._id}` },
          {
            $setOnInsert: {
              assignedToUserId: reassignedToUserId,
              type: "TASK_DUE_TODAY",
              title: "Task due today",
              message: `${task.title} is due today for ${prospectName}.`,
              status: "Unread",
              entityType: "Task",
              entityId: task._id,
              dedupeKey: `TASK_DUE_TODAY:${task._id}`,
