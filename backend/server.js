...
async function buildManagerPortalPayload(user, { taskDatePreset = "ALL", salesDatePreset = "ALL", kpiDatePreset = "DEFAULT" } = {}) {
  const context = await getManagerScopeContext(user);
  if (context.error) return context;
  ...
  const kpiContext = String(kpiDatePreset || "DEFAULT").trim().toUpperCase() === "DEFAULT"
    ? { key: "DEFAULT", startDate: null, periodLabel: "Default by KPI assignment frequency" }
    : buildPresetContext(kpiDatePreset);
  ...
  const kpiMetricsByUserId = createMetricsMap(scopedAgents);
  ...
  applySalesMetrics({
    ...buildSalesMetricInput(kpiContext),
    metricsByUserId: kpiMetricsByUserId,
  });
  ...
  const kpiSalesRows = buildRows(kpiMetricsByUserId);
  ...
  return {
    payload: {
      ...
      reportContext: {
        ...
        kpiDatePreset: kpiContext.key,
        kpiPeriodLabel: kpiContext.periodLabel,
      },
      summary: summarizeRows(allRows),
      taskSummary: summarizeRows(taskRows),
      salesSummary: summarizeRows(salesRows),
      kpiSalesSummary: summarizeRows(kpiSalesRows),
      kpiSalesRowsByFrequency,
      agents,
      taskRows: sortedTaskRows,
      salesRows: sortedSalesRows,
      kpiSalesRows: [...kpiSalesRows].sort((left, right) => {
        if (right.annualPremium !== left.annualPremium) return right.annualPremium - left.annualPremium;
        if (right.totalPolicies !== left.totalPolicies) return right.totalPolicies - left.totalPolicies;
        return byName(left, right);
      }),
    },
  };
