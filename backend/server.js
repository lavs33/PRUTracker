      ...
      [
        {
          ...
          message: `${task.title} for ${prospectFullName} (Lead ${leadCode || "—"}) is due today.`,
          status: "Unread",
          entityType: "Task",
          entityId: task._id,
...
app.get("/api/prospects/:prospectId/leads/:leadId/engagement", async (req, res) => {
  try {
    ...
    const attemptIds = attempts.map((a) => a._id).filter(Boolean);
    const scheduledMeetings = attemptIds.length
      ? await ScheduledMeeting.find({
          assignedToUserId: userObjectId,
          leadEngagementId: engagement._id,
          contactAttemptId: { $in: attemptIds },
          status: { $ne: "Cancelled" },
        })
          .select("contactAttemptId startAt endAt durationMin mode platform platformOther link inviteSent place")
          .lean()
      : [];

    const meetingByAttemptId = new Map(
      scheduledMeetings.map((m) => [String(m.contactAttemptId), m])
    );
    ...
    const contactAttempts = attempts.map((a) => ({
      ...
      ...(function () {
        const m = meetingByAttemptId.get(String(a._id));
        return {
...
app.post("/api/prospects/:prospectId/leads/:leadId/schedule-meeting", async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const appointmentDescription = `Attend scheduled meeting with ${prospect.firstName}${prospect.middleName ? ` ${prospect.middleName}` : ""} ${prospect.lastName} (Lead ${lead.leadCode || "—"}).`;
      const appointmentDueAt = dt;
