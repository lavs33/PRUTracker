    ...
    if (nextStatus === "Endorsed") {
      ...
      if (agentUser?._id) {
        await Notification.deleteMany({
          assignedToUserId: agentUser._id,
          type: {
            $in: [
              "UNIT_KPI_ASSIGNED", "UNIT_KPI_TARGET_UPDATED", "UNIT_KPI_UNASSIGNED",
              "AGENT_KPI_ASSIGNED", "AGENT_KPI_TARGET_UPDATED", "AGENT_KPI_UNASSIGNED",
            ],
          },
        });
      }
...
async function deleteFormerAgentClientNotifications({
  assignedToUserId,
  prospectId,
  leadIds = [],
  policyholderIds = [],
  ...
  await Notification.deleteMany({ assignedToUserId, $or: relatedClauses });
}
...
app.post("/api/manager/resignation/:resignationId/reassign-prospect", async (req, res) => {
  try {
    ...
    await deleteFormerAgentClientNotifications({
