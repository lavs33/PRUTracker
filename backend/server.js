...
app.put("/api/prospects/:prospectId", async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const cleanOccupationCategory = String(occupation ?? "").trim() && !occupationCategory
        ? "Employed"
        : String(occupationCategory ?? "").trim();
      ...
      if (["Employed", "Self-Employed"].includes(cleanOccupationCategory) && !cleanOccupation) {
      ...
      // Address (Philippines only)
      const addressIn = address && typeof address === "object" ? address : {};
      const line1 = String(addressIn.line1 ?? "").trim();
      const line2 = String(addressIn.line2 ?? "").trim();
      ...
      if (!line1) throw Object.assign(new Error("Street Address (Line 1) is required."), { status: 400 });
      ...
      existing.occupationCategory = cleanOccupationCategory;
      existing.occupation = cleanOccupationCategory === "Not Employed" ? "" : cleanOccupation;
      existing.address = {
        line1,
        line2,
        barangay,
...
app.get("/api/prospects/:prospectId/leads/:leadId/needs-assessment", async (req, res) => {
  try {
    ...
    let needsAssessment = await NeedsAssessment.findOne({ leadEngagementId: engagement._id })
      .select("outcomeActivity attendanceConfirmed attendedAt dependents")
      .lean();
...
app.put("/api/prospects/:prospectId/leads/:leadId/needs-assessment", async (req, res) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (occupationCategory && !["Employed", "Self-Employed", "Not Employed"].includes(occupationCategory)) {
        throw Object.assign(new Error("Invalid occupation category."), { status: 400 });
      }
      if (occupationCategory && occupationCategory !== "Not Employed" && !occupation) {
        throw Object.assign(new Error("Occupation is required for employed/self-employed prospects."), { status: 400 });
      }
      ...
      prospect.occupationCategory = occupationCategory || prospect.occupationCategory || "Not Employed";
      prospect.occupation = prospect.occupationCategory === "Not Employed" ? "" : (occupation || prospect.occupation);
