...
/* ===========================
   ...
   - response/result cannot be edited in this endpoint.
   - Editable fields (for now): primaryChannel, otherChannels, notes.
=========================== */
app.patch("/api/prospects/:prospectId/leads/:leadId/contact-attempts/:attemptId", async (req, res) => {
  ...
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(req.body || {}, "response")) {
      return res.status(400).json({ message: "response cannot be edited for an existing contact attempt." });
    }
