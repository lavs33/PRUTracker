...
app.get("/api/sales/performance", async (req, res) => {
  try {
    ...
    const leads = await Lead.find(leadQuery)
      .select("_id prospectId leadCode source otherSource status droppedAt createdAt updatedAt")
      .lean();
    const leadIds = leads.map((l) => l._id);
    ...
    const leadWasOngoingAtPeriodEnd = (lead) => {
      ...
      if (activeLeadGapStatuses.has(String(lead?.status || "").trim().toLowerCase())) return true;
      ...
      return terminalTimes.some((terminalTime) => terminalTime > reportEndTime);
    };
    const ongoingLeads = reportingLeads.filter(leadWasOngoingAtPeriodEnd);
    const ongoingLeadIds = new Set(ongoingLeads.map((lead) => String(lead._id)));
    const totalOngoingLeads = ongoingLeads.length;
    const unconvertedLeads = totalOngoingLeads;
    const leadGap = totalOngoingLeads;
    const pipelineLeadCount = totalOngoingLeads + convertedLeads;
    const conversionRatePct = pipelineLeadCount ? Math.round((convertedLeads / pipelineLeadCount) * 100) : 0;
    ...
    const leadStatusBreakdown = buildLeadStatusBreakdown(ongoingLeads, totalOngoingLeads);
    const unconvertedLeadStatusBreakdown = buildLeadStatusBreakdown(ongoingLeads, totalOngoingLeads);

    const leadSourceBreakdownMap = new Map();

    for (const lead of reportingLeads) {
      const leadId = String(lead._id || "");
      if (!ongoingLeadIds.has(leadId) && !convertedLeadIds.has(leadId)) continue;
      ...
      if (ongoingLeadIds.has(leadId)) leadSourceBreakdownMap.get(bucket).ongoingLeads += 1;
    }
    ...
    const leadSourceBreakdown = [...leadSourceBreakdownMap.values()]
      .map((sourceMetrics) => ({
        ...sourceMetrics,
        conversionRatePct: (Number(sourceMetrics.ongoingLeads || 0) + sourceMetrics.convertedLeads)
          ? Math.round((sourceMetrics.convertedLeads / (Number(sourceMetrics.ongoingLeads || 0) + sourceMetrics.convertedLeads)) * 100)
          : 0,
        activeConversionRatePct: (Number(sourceMetrics.ongoingLeads || 0) + sourceMetrics.convertedLeads)
          ? Math.round((sourceMetrics.convertedAndActiveLeads / (Number(sourceMetrics.ongoingLeads || 0) + sourceMetrics.convertedLeads)) * 100)
          : 0,
      }))
      .sort((a, b) => {
        ...
        if (b.ongoingLeads !== a.ongoingLeads) return b.ongoingLeads - a.ongoingLeads;
