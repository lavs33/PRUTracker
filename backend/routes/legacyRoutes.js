function registerLegacyRoutes(app, deps) {
  ...
  async function syncPolicyholderPaymentDates(policyholderDoc) {
    ...
    if (String(policyholderDoc.status || "") === "Cancelled") return;
...
app.get("/api/policyholders/:policyholderId/details", async (req, res) => {
  try {
    ...
    const policyholderDoc = await Policyholder.findOne({
      ...
      .select("policyholderCode productId policyNumber leadEngagementId lastPaidDate nextPaymentDate status annualPaymentRecords cancellationDetails createdAt updatedAt");
...
app.post("/api/policyholders/:policyholderId/cancellation", async (req, res) => {
  try {
    ...
    const policyholder = await Policyholder.findOne({
      _id: policyholderObjectId,
      assignedToUserId: userObjectId,
    }).select("policyholderCode policyNumber leadEngagementId status nextPaymentDate cancellationDetails");

    if (!policyholder) return res.status(404).json({ message: "Policyholder not found." });
    if (String(policyholder.status || "") === "Cancelled") {
      return res.status(409).json({ message: "Policy is already cancelled." });
    }
    ...
    } else if (!/^data:image\/[a-z0-9.+-]+;base64,/i.test(proofImageDataUrl)) {
      fieldErrors.proofOfApprovedPolicySurrenderImageDataUrl = "Proof of approved policy surrender must be an image file.";
    }
    ...
    policyholder.cancellationDetails = {
      ...
      proofOfApprovedPolicySurrenderFileMimeType: String(proofOfApprovedPolicySurrenderFileMimeType || "image/*").trim() || "image/*",
...
app.post("/api/policyholders/:policyholderId/annual-payments/:annualPaymentId/payments", async (req, res) => {
  try {
    ...
    if (!policyholder) return res.status(404).json({ message: "Policyholder not found." });
    if (String(policyholder.status || "") === "Cancelled") {
      return res.status(409).json({ message: "Payment records cannot be added to a cancelled policy." });
    }
...
app.post("/api/policyholders/:policyholderId/annual-payments/:annualPaymentId/payments/:paymentId/eor-reminder", async (req, res) => {
  try {
    ...
    const policyholder = await Policyholder.findOne({
      _id: policyholderObjectId,
      assignedToUserId: userObjectId,
    }).select("policyholderCode policyNumber productId leadEngagementId nextPaymentDate annualPaymentRecords status").lean();

    if (!policyholder) return res.status(404).json({ message: "Policyholder not found." });
    if (String(policyholder.status || "") === "Cancelled") {
      return res.status(409).json({ message: "Payment reminders cannot be enabled for a cancelled policy." });
    }
