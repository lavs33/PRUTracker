  ...
  try {
    ...
    const buildTaskReportContext = () => {
      ...
      if (selected === "YTD") {
        ...
        const endLabel = new Intl.DateTimeFormat("en-US", { month: "long", year: "numeric", timeZone: "Asia/Manila" }).format(nowDate);
        return { startDate, endDate: nowDate, periodLabel: `${startLabel} - ${endLabel}` };
      }
      if (/^\d{4}-(0[1-9]|1[0-2])$/.test(selected)) {
        ...
        const endDate = month === currentMonth ? nowDate : new Date(nextMonth.getTime() - 1);
        return {
          startDate,
          endDate,
          periodLabel: new Intl.DateTimeFormat("en-US", { month: "long", year: "numeric", timeZone: "Asia/Manila" }).format(new Date(startDate.getTime() + (8 * 60 * 60 * 1000))),
