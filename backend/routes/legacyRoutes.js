...
async function ensureTaskMissedNotificationsForUser(userObjectId, { forceUnread = false } = {}) {
  const now = new Date();
...
app.post("/api/prospects/:prospectId/leads/:leadId/schedule-meeting", async (req, res) => {
  ...
  try {
    ...
    await ensureTaskMissedNotificationsForUser(userObjectId, { forceUnread: true });
