...
app.post("/api/policyholders/:policyholderId/annual-payments/:annualPaymentId/payments", async (req, res) => {
  try {
    ...
    const issuancePaymentDate = annualPayment.annualPaymentPeriod?.startDate
      ? new Date(annualPayment.annualPaymentPeriod.startDate)
      : null;
    if (issuancePaymentDate && !Number.isNaN(issuancePaymentDate.getTime())) {
      const minimumPaymentDate = new Date(issuancePaymentDate);
      minimumPaymentDate.setHours(0, 0, 0, 0);
      if (submittedPaymentDate < minimumPaymentDate) {
        return res.status(400).json({ message: "Payment date must be on or after the policy issuance date." });
      }
    }
