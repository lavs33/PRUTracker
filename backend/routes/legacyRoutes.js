  ...
  try {
    ...
    if (!cancellationDateText) {
      ...
      if (Number.isNaN(cancellationDate.getTime())) {
        ...
        if (!lastPaidOnly) {
          fieldErrors.approvedCancellationDate = "Last payment date is unavailable.";
        } else if (cancelOnly < lastPaidOnly) {
          fieldErrors.approvedCancellationDate = "Cancellation date must be on or after the last payment date.";
        }
      }
    }
