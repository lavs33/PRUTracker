...
app.get("/api/sales/performance", async (req, res) => {
  try {
    ...
    const leadWasHandledDuringPeriod = (lead) => {
      ...
      return leadLifecycleTimes(lead).some((terminalTime) => terminalTime >= reportStartTime);
    };
    ...
    const ongoingLeadsAtPeriodEnd = handledLeads.filter((lead) => (
      ...
      || leadLifecycleTimes(lead).some((terminalTime) => terminalTime > reportEndTime)
    ));
    ...
    const unconvertedLeadStatusBreakdown = buildLeadStatusBreakdown(unconvertedLeadRows, unconvertedLeads);
