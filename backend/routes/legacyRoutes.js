...
app.get("/api/agent/home", async (req, res) => {
  try {
    ...
    const policyholderEngagementIds = new Set(policyholders.map((policyholder) => String(policyholder?.leadEngagementId || "")).filter(Boolean));
    const issuedPolicies = policyholderEngagementIds.size
      ? await Policy.find({
          leadEngagementId: { $in: [...policyholderEngagementIds] },
          "recordPolicyApplicationStatus.status": "Issued",
        }).select("leadEngagementId recordPolicyApplicationStatus.issuanceDate").lean()
      : [];
    const currentMonthIssuedEngagementIds = new Set(issuedPolicies
      .filter((policy) => {
        const issuedAt = new Date(policy?.recordPolicyApplicationStatus?.issuanceDate || 0).getTime();
        return Number.isFinite(issuedAt) && issuedAt >= currentMonthStart.getTime() && issuedAt < nextMonthStart.getTime();
      })
      .map((policy) => String(policy.leadEngagementId || "")));
    const currentMonthActiveEngagementIds = new Set(
      [...currentMonthIssuedEngagementIds].filter((engagementId) => activePolicyholderEngagementIds.has(engagementId))
    );
    const currentMonthConversionRatePct = currentMonthIssuedEngagementIds.size
      ? Math.round((currentMonthActiveEngagementIds.size / currentMonthIssuedEngagementIds.size) * 100)
      : 0;
