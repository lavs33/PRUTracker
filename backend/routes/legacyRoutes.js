...
app.post("/api/manager/agent-sales-recommendations/notify", async (req, res) => {
  try {
    const { userId, agentUserId, recommendationKey, periodLabel, agentContribution, unitProduction, fairShare } = req.body || {};
    ...
    await Notification.updateMany({
      ...
      resolutionStatus: { $ne: "Resolved" },
      "metadata.recommendationKey": recommendationKey,
      softDeletedAt: null,
    }, { $set: { resolutionStatus: "Resolved", resolvedAt: new Date(), "metadata.resolutionReason": "Superseded by a newer unit recommendation." } });
    const notification = await Notification.create({
      assignedToUserId: recipient._id, type,
      title: `${sender.role} ${sender.username} - ${senderName} recommends strengthening your sales production.`,
      message: `Your contribution for ${periodLabel || "the selected period"} is ${agentContribution || "₱0.00"} of the unit's ${unitProduction || "₱0.00"} sales production, below the fair-share benchmark of ${fairShare || "₱0.00"}.\n\nRecommended action: Strengthen sales production and increase your contribution toward the unit target.`,
      status: "Unread", resolutionStatus: "Unresolved", entityType: "Agent", entityId: agent._id,
      metadata: { recommendationKey, unitId: String(manager.unitId), agentUserId: String(agentUserId), senderRole: sender.role, senderName, periodLabel, agentContribution, unitProduction, fairShare, monthKey: String(recommendationKey).split(":")[0] },
    });
    return res.status(201).json({ message: `${recipientName} has been notified.`, notifiedAt: notification.createdAt, senderRole: sender.role, senderName });
  } catch (err) {
