function registerLegacyRoutes(app, deps) {
  ...
  async function syncPolicyholderPaymentDates(policyholderDoc) {
    ...
    if (["Active", "At Risk"].includes(currentStatus)) {
      ...
      if (todayDay !== null && nextPaymentDay !== null && nextPaymentDay < todayDay) {
        nextStatus = "At Risk";
      } else {
        nextStatus = "Active";
      }
...
app.post("/api/policyholders/:policyholderId/annual-payments/:annualPaymentId/payments", async (req, res) => {
  try {
    ...
    if (String(policyholder.status || "") === "At Risk") policyholder.status = "Active";
