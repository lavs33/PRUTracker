function registerLegacyRoutes(app, deps) {
  ...
  function buildNeedsAssessmentCycleSnapshot(source = {}, cycle) {
    return {
      attemptCycle: normalizeAttemptCycle(cycle, source.attemptCycle || 1),
      capturedAt: new Date(),
      outcomeActivity: source.outcomeActivity || "Record Prospect Attendance",
      followUpNeedsAssessmentRequired: String(source.followUpNeedsAssessmentRequired || ""),
      followUpNeedsAssessmentDecidedAt: source.followUpNeedsAssessmentDecidedAt || null,
      attendanceConfirmed: Boolean(source.attendanceConfirmed),
      attendedAt: source.attendedAt || null,
      attendanceProofImageDataUrl: String(source.attendanceProofImageDataUrl || ""),
      attendanceProofFileName: String(source.attendanceProofFileName || ""),
      dependents: clonePlain(Array.isArray(source.dependents) ? source.dependents : []),
      needsPriorities: clonePlain(source.needsPriorities || {}),
    };
  }

  function archiveNeedsAssessmentCycle(needsAssessmentDoc, cycle) {
    const archiveCycle = normalizeAttemptCycle(cycle);
    if (!needsAssessmentHasSavedDetails(needsAssessmentDoc)) return;
    const history = Array.isArray(needsAssessmentDoc.attemptCycleHistory)
      ? needsAssessmentDoc.attemptCycleHistory
      : [];
    needsAssessmentDoc.attemptCycleHistory = [
      ...history.filter((entry) => normalizeAttemptCycle(entry?.attemptCycle) !== archiveCycle),
      buildNeedsAssessmentCycleSnapshot(needsAssessmentDoc, archiveCycle),
    ];
  }
...
app.get("/api/sales/performance", async (req, res) => {
  try {
    ...
    const totalAnnualPremiumPhp = scopedAnnualPayments.reduce(
      (sum, annualPayment) => sum + Number(annualPayment?.totalAnnualPremiumPhp || 0),
      0
    );
...
app.get("/api/prospects/:prospectId/leads/:leadId/needs-assessment", async (req, res) => {
  try {
    ...
    const needsAssessmentSelect = "attemptCycle attemptCycleHistory outcomeActivity attendanceConfirmed attendedAt attendanceProofImageDataUrl attendanceProofFileName dependents needsPriorities followUpNeedsAssessmentRequired followUpNeedsAssessmentDecidedAt";
    ...
    const legacyHistorySnapshots = allCycleNeedsAssessments.flatMap((doc) => (
      Array.isArray(doc?.attemptCycleHistory) ? doc.attemptCycleHistory : []
    ));
    const targetHistorySnapshot = legacyHistorySnapshots
      .filter((entry) => normalizeAttemptCycle(entry?.attemptCycle) === targetAttemptCycle)
    ...
    const latestPriorNeedsSnapshot = legacyHistorySnapshots
      .filter((entry) => normalizeAttemptCycle(entry?.attemptCycle) < currentAttemptCycle && needsAssessmentHasSavedDetails(entry))
      .sort((left, right) => normalizeAttemptCycle(right?.attemptCycle) - normalizeAttemptCycle(left?.attemptCycle))[0] || null;
    const priorNeedsPrefillSource = latestPriorNeedsDoc || latestPriorNeedsSnapshot;
    const hasPriorNeedsPrefill = !isHistoryCycleRequest && Boolean(priorNeedsPrefillSource);

    if (!needsAssessment) {
      needsAssessment = targetHistorySnapshot || {
        attemptCycle: targetAttemptCycle,
