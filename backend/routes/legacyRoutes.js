...
app.post("/api/manager/kpi-recommendations/notify", async (req, res) => {
  try {
    ...
    const isUnitSalesDependent = ["monthly_sales_production", "monthly_target_achievement_index"].includes(String(kpiKey));
    const progressScope = isUnitSalesDependent ? "Unit Sales Production KPI" : `Branch ${String(kpiLabel || "KPI")}`;
    const contextReminder = isUnitSalesDependent
