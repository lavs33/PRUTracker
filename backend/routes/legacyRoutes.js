function registerLegacyRoutes(app, deps) {
  ...
  async function ensureScheduledMeetingAttemptCycleBackfill() {
    ...
    await ScheduledMeeting.updateMany(
      { $or: [{ attemptCycle: { $exists: false } }, { attemptCycle: null }] },
      { $set: { attemptCycle: 1 } }
    );
  ...
  async function ensureContactAttemptCycleBackfill() {
    ...
    for (const leadEngagementId of engagementIds) {
      ...
      attempts.forEach((attempt, idx) => {
        ...
        if (Number(attempt?.attemptCycle || 0) !== cycle) {
          ops.push({
            updateOne: {
              filter: { _id: attempt._id },
              update: { $set: { attemptCycle: cycle } },
            },
          });
        }
      ...
      if (ops.length) {
        await ContactAttempt.bulkWrite(ops, { ordered: true });
      }
    }
