...
app.post("/api/policyholders/:policyholderId/cancellation", async (req, res) => {
  try {
    ...
    if (!surrenderFormDataUrl) {
      fieldErrors.accomplishedPolicySurrenderFormFileDataUrl = "Accomplished policy surrender form PDF is required.";
    ...
    if (!cancellationDateText) {
      fieldErrors.approvedCancellationDate = "Approved cancellation date is required.";
    } else {
    ...
    if (!proofImageDataUrl) {
      fieldErrors.proofOfApprovedPolicySurrenderImageDataUrl = "Proof of approved policy surrender image is required.";
    } else if (!/^data:image\/(?:jpeg|png);base64,/i.test(proofImageDataUrl)) {
      fieldErrors.proofOfApprovedPolicySurrenderImageDataUrl = "Proof of approved policy surrender must be a JPG or PNG image.";
    }
