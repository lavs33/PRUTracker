...
// View Policyholder Details (under a Lead)
...
app.get("/api/prospects/:prospectId/leads/:leadId/policyholders/:policyholderId/details", async (req, res) => {
  try {
    ...
    const policyholder = await Policyholder.findOne({
      ...
      .select("policyholderCode productId policyNumber lastPaidDate nextPaymentDate status createdAt updatedAt")
      .lean();
    ...
    const [product, policy] = await Promise.all([
      policyholder.productId
        ? Product.findById(policyholder.productId).select("productName paymentTermOptions").lean()
        : null,
    ...
    const selectedPaymentTermLabelRaw = String(coverage.selectedPaymentTermLabel || "").trim();
    const selectedPaymentTermTypeRaw = String(coverage.selectedPaymentTermType || "").trim();
    const paymentTermOption = (Array.isArray(product?.paymentTermOptions) ? product.paymentTermOptions : []).find((opt) => {
      const optLabel = String(opt?.label || "").trim();
      const optType = String(opt?.type || "").trim();
      return optLabel === selectedPaymentTermLabelRaw && (!selectedPaymentTermTypeRaw || optType === selectedPaymentTermTypeRaw);
    }) || null;
    const selectedPaymentTermYears = coverage.selectedPaymentTermYears ?? paymentTermOption?.years ?? paymentTermOption?.minYears ?? null;
    const selectedPaymentTermUntilAge = coverage.selectedPaymentTermUntilAge ?? paymentTermOption?.untilAge ?? null;
    const prospectName = `${prospect.firstName}${prospect.middleName ? ` ${prospect.middleName}` : ""} ${prospect.lastName}`.trim();
    ...
    return res.json({
      ...
      coverage: {
        ...
        selectedPaymentTermLabel: coverage.selectedPaymentTermLabel || paymentTermOption?.label || "",
        selectedPaymentTermType: coverage.selectedPaymentTermType || paymentTermOption?.type || "",
