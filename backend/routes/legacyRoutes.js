...
app.post("/api/manager/kpi-recommendations/notify", async (req, res) => {
  try {
    ...
    const contextReminder = isSalesProduction
      ? `Your unit's progress directly contributes to the branch sales production target. The unit is currently at ${progressLabel || "—"} against its required${targetQualifier === "minimum" ? " MINIMUM" : ""} target of ${requiredTargetLabel || targetLabel || "—"}.`
      : `Your unit's contribution is important to achieving the branch target. Branch progress is currently ${progressLabel || "—"} against the${targetQualifier === "minimum" ? " MINIMUM" : ""} target of ${requiredTargetLabel || targetLabel || "—"}.`;
