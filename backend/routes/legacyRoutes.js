function registerLegacyRoutes(app, deps) {
  ...
  function paymentPremiumPaidAmount(payment = {}) {
    ...
    const overdueFee = Number(transfer.overdueFeePhp || 0);
    const premiumOnly = paid - (Number.isFinite(overdueFee) && overdueFee > 0 ? overdueFee : 0);
    return Number.isFinite(premiumOnly) && premiumOnly > 0 ? Math.round(premiumOnly * 100) / 100 : 0;
  }
...
app.post("/api/policyholders/:policyholderId/annual-payments/:annualPaymentId/payments", async (req, res) => {
  try {
    ...
    const totalPremiumForRecord = Math.round(((expectedAmount * paymentCountCovered) + (missedPaymentMode ? overdueFee : 0)) * 100) / 100;
...
app.put("/api/policyholders/:policyholderId/annual-payments/:annualPaymentId/payments/:paymentId/transfer", async (req, res) => {
  try {
    ...
    const overdueFee = Number(existingTransfer.overdueFeePhp || 0);
    const safeOverdueFee = Number.isFinite(overdueFee) && overdueFee > 0 ? overdueFee : 0;
    const isMissedPaymentRecord = existingTransfer.isMissedPaymentRecord === true;
    const totalPremiumForRecord = Math.round(((expectedAmount * safePaymentCountCovered) + (isMissedPaymentRecord ? safeOverdueFee : 0)) * 100) / 100;
