 ...
 * cycle shape.
 *
 * Normal leads remain on cycle 1. Reopened leads use application save dates
 * against LeadEngagement reopen boundaries; rows saved before a reopen stay on
 * their historical cycle, while undated reopened rows fall back to the current
 * LeadEngagement attempt cycle.
 *
 * Usage:
...
function inferApplicationBackfillCycle(application = {}, context = {}, applicationCountForEngagement = 1) {
  ...
  const cycleDate = preferredApplicationCycleDate(application);
  const dateInferredCycle = inferCycleForDate(context.ranges, cycleDate, fallbackCycle);

  /**
   * If an Application has a reliable save timestamp before the reopen boundary,
   * keep it on its historical cycle so old data only appears in engagement
   * history. Only undated legacy rows fall back to the current reopened cycle.
   */
  if (Number.isFinite(dateMs(cycleDate))) {
...
async function backfillApplicationAttemptCycles() {
  ...
  const summary = {
    ...
    undatedReopenedApplicationsPromotedToCurrentCycle: 0,
  };
  ...
  for (const application of applications) {
    ...
    if ((context.fallbackCycle || 1) > 1 && (!currentCycle || currentCycle === 1) && cycle > 1) {
      summary.undatedReopenedApplicationsPromotedToCurrentCycle += 1;
    }
