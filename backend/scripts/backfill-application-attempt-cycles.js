 ...
 * cycle shape.
 *
 * Normal leads remain on cycle 1. Reopened leads prefer the latest/current
 * LeadEngagement attempt cycle when migrating a legacy single Application row,
 * then fall back to application save dates and reopen boundaries for multi-row
 * histories. This prevents a reopened lead's current Application record from
 * staying on attemptCycle=1 and being overwritten by future saves for the same
 * LeadEngagement.
 *
 * Usage:
...
function inferApplicationBackfillCycle(application = {}, context = {}, applicationCountForEngagement = 1) {
  ...
  /**
   * Legacy Application used to be one row per LeadEngagement. For reopened leads,
   * that lone row represents the active/current application attempt and must move
   * to the reopened cycle. Relying only on updatedAt can strand it at cycle 1
   * when the application was saved before the recorded reopen boundary.
   */
  if (applicationCountForEngagement <= 1 && (!currentCycle || currentCycle === 1)) {
    return fallbackCycle;
  }

  const dateInferredCycle = inferCycleForDate(
    context.ranges,
    preferredApplicationCycleDate(application),
    fallbackCycle
  );

  if ((!currentCycle || currentCycle === 1) && dateInferredCycle === 1) {
    return fallbackCycle;
  }

  return dateInferredCycle;
}
...
async function backfillApplicationAttemptCycles() {
  ...
  const summary = {
    ...
    reopenedApplicationsPromotedToCurrentCycle: 0,
  };
  ...
  const applicationCounts = await Application.aggregate([
    { $group: { _id: "$leadEngagementId", count: { $sum: 1 } } },
  ]);
  const applicationCountByEngagementId = new Map(
  ...
  for (const application of applications) {
    ...
    const applicationCountForEngagement = applicationCountByEngagementId.get(String(leadEngagementId || "")) || 1;
    const cycle = inferApplicationBackfillCycle(application, context, applicationCountForEngagement);
    const currentCycle = validCycle(application.attemptCycle);
    if ((context.fallbackCycle || 1) > 1 && (!currentCycle || currentCycle === 1) && cycle > 1) {
      summary.reopenedApplicationsPromotedToCurrentCycle += 1;
    }
