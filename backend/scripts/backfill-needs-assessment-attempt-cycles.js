 ...
 * needs-assessment save date and the LeadEngagement reopen boundaries, so stale
...
function preferredNeedsAssessmentCycleDate(needsAssessment = {}) {
  // NeedsAssessment is a one-document-per-engagement record. For reopened leads,
  // updatedAt is usually the only reliable timestamp showing the document was
  // saved again in the reopened cycle, especially when agents edit analysis
  // fields without changing attendance/follow-up dates.
  return latestDate(
