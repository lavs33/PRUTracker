 ...
 * needs-assessment save date and LeadEngagement reopen boundaries. If a legacy
 * document has archived attemptCycleHistory snapshots, the script also writes
 * those snapshots back as separate NeedsAssessment documents for their original
 * cycles so engagement history can be fetched by { leadEngagementId, attemptCycle }.
 *
 * Usage:
...
function snapshotToNeedsAssessmentPayload(snapshot = {}, leadEngagementId) {
  return {
    leadEngagementId,
    attemptCycle: validCycle(snapshot.attemptCycle) || 1,
    outcomeActivity: snapshot.outcomeActivity || "Record Prospect Attendance",
    followUpNeedsAssessmentRequired: String(snapshot.followUpNeedsAssessmentRequired || ""),
    followUpNeedsAssessmentDecidedAt: snapshot.followUpNeedsAssessmentDecidedAt || null,
    attendanceConfirmed: Boolean(snapshot.attendanceConfirmed),
    attendedAt: snapshot.attendedAt || null,
    attendanceProofImageDataUrl: String(snapshot.attendanceProofImageDataUrl || ""),
    attendanceProofFileName: String(snapshot.attendanceProofFileName || ""),
    dependents: Array.isArray(snapshot.dependents) ? snapshot.dependents : [],
    needsPriorities: snapshot.needsPriorities || {},
  };
}
...
async function backfillNeedsAssessmentAttemptCycles() {
  ...
  const summary = {
    ...
    needsAssessmentsCreatedFromHistory: 0,
  ...
  const needsAssessments = await NeedsAssessment.find(query)
    .select("leadEngagementId attemptCycle attemptCycleHistory outcomeActivity attendanceConfirmed attendedAt attendanceProofImageDataUrl attendanceProofFileName dependents needsPriorities followUpNeedsAssessmentRequired followUpNeedsAssessmentDecidedAt createdAt updatedAt")
    .lean();
  ...
  for (const needsAssessment of needsAssessments) {
    ...
    const snapshots = Array.isArray(needsAssessment.attemptCycleHistory) ? needsAssessment.attemptCycleHistory : [];
    for (const snapshot of snapshots) {
      const snapshotCycle = validCycle(snapshot?.attemptCycle);
      if (!snapshotCycle) continue;
      const exists = await NeedsAssessment.exists({ leadEngagementId, attemptCycle: snapshotCycle });
      if (exists) continue;
      if (!dryRun) {
        await NeedsAssessment.create(snapshotToNeedsAssessmentPayload(snapshot, leadEngagementId));
      }
      summary.needsAssessmentsCreatedFromHistory += 1;
    }
  }
