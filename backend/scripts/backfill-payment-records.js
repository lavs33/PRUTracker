...
async function backfillPaymentRecords() {
  ...
  for (const application of applications) {
    ...
    const shouldKeepExistingPaymentId = Boolean(paymentId && hasApplicationPremiumSelection);
  ...
  for (const application of applicationsWithPaymentRefs) {
    ...
    const hasApplicationPremiumSelection = hasApplicationPremiumSelectionData(application?.recordPremiumPaymentTransfer || {});
    if (hasTransfer || hasApplicationPremiumSelection) continue;
  ...
  if (blankPaymentIds.length) {
    const [referencedByApplication, referencedByPolicy, referencedByPolicyholder] = await Promise.all([
      ...
      Policyholder.find({ "paymentRecords.paymentId": { $in: blankPaymentIds } }).distinct("paymentRecords.paymentId"),
    ]);
