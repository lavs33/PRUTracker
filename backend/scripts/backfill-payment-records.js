...
async function backfillPaymentRecords() {
  ...
  for (const application of applications) {
    ...
    if (hasCompletedPremiumTransfer && paymentId) applicationSet["recordPremiumPaymentTransfer.paymentId"] = paymentId;
      ...
      const result = await Application.collection.updateOne(
        { _id: application._id },
        {
          ...
          $unset: {
            ...
            ...(!hasCompletedPremiumTransfer ? { "recordPremiumPaymentTransfer.paymentId": "" } : {}),
          },
        }
      );
    ...
    if (hasCompletedPremiumTransfer && paymentId) paymentIdsByEngagementId.set(engagementId, paymentId);
  ...
  const applicationsWithPaymentRefs = await Application.find({
    ...
    .select("recordPremiumPaymentTransfer.paymentId")
    .lean();
