...
async function ensureAnnualPaymentLeadEngagementIndex() {
  ...
  if (legacyUniqueIndex) {
    console.log("Dropping legacy unique annualpayments leadEngagementId_1 index so multiple annual records can share one lead engagement.");
    await collection.dropIndex("leadEngagementId_1");
  }

  const refreshedIndexes = legacyUniqueIndex ? await collection.indexes() : indexes;
  const hasNonUniqueLeadEngagementIndex = refreshedIndexes.some((index) => {
    const key = index?.key || {};
    return index?.name === "leadEngagementId_1"
      && index?.unique !== true
      && Object.keys(key).length === 1
  ...
  if (!hasNonUniqueLeadEngagementIndex) {
...
async function main() {
  ...
  for (const policyholder of policyholders) {
    ...
    if (!DRY_RUN) {
      ...
      if (!nextAnnual) {
        await ensureAnnualPaymentLeadEngagementIndex();
        nextAnnual = await AnnualPayment.findOneAndUpdate(
          {
            leadEngagementId: policyholder.leadEngagementId,
            "annualPaymentPeriod.startDate": nextPeriod.startDate,
          },
          {
            $setOnInsert: {
              leadEngagementId: policyholder.leadEngagementId,
