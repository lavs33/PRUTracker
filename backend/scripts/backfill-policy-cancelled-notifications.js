 ...
 * ---------------------------------------
 * Creates or refreshes POLICY_CANCELLED notifications for existing Cancelled
 * policyholders so old notification messages include the cancellation date.
 *
 * Usage:
...
function cancellationDateForPolicyholder(policyholder = {}) {
  return policyholder?.cancellationDetails?.approvedCancellationDate
    || policyholder?.cancellationDetails?.cancelledAt
    || policyholder?.updatedAt
...
async function backfillPolicyCancelledNotifications() {
  ...
  const policyholders = await Policyholder.find(policyholderQuery)
    .select("assignedToUserId policyholderCode policyNumber productId leadEngagementId status cancellationDetails createdAt updatedAt")
  ...
  const summary = {
    ...
    refreshedExistingNotifications: 0,
  ...
  for (const policyholder of policyholders) {
    ...
    const hasExistingNotification = existingDedupeKeys.has(dedupeKey);
    ...
    const cancelledDate = cancellationDateForPolicyholder(policyholder);
    const message = `Policy cancelled on ${formatDateInManila(cancelledDate)}. Policyholder Code: ${policyholderCode}. Policyholder Name: ${policyholderName}. Policy Name: ${policyName}. Policy Number: ${policyNumber}.`;
    ...
    if (hasExistingNotification) {
      summary.refreshedExistingNotifications += 1;
    } else {
