/**
 * Remove legacy frequency-based Branch KPI assignment fields (BRANCH scope only).
 *
 * Monthly assignments (including historical months) are preserved verbatim.
 * AGENT and UNIT documents are deliberately not modified by this cleanup.
...
async function main() {
  ...
  const branchAssignments = await collection.find(
    { scopeType: "BRANCH" },
    { projection: { scopeId: 1, kpis: 1 } }
  ).toArray();

  const eligible = branchAssignments.filter((assignment) => (
    Array.isArray(assignment.kpis)
    && assignment.kpis.every((kpi) => Array.isArray(kpi.monthlyAssignments))
  ));
  const skipped = branchAssignments.length - eligible.length;
  ...
  console.log({
    ...
    branchAssignmentDocuments: branchAssignments.length,
  ...
  if (skipped > 0) {
    throw new Error("Cleanup stopped: at least one Branch KPI assignment has no monthly history. Load/save monthly assignments before retrying.");
  }
  ...
  if (remainingLegacyRows > 0) throw new Error("Cleanup verification failed: legacy Branch KPI fields remain.");
}
