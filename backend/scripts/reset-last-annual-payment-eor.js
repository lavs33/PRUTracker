...
async function main() {
  ...
  const annualPayment = await AnnualPayment.findOne(annualPaymentFilter).sort({
  ...
  const paymentFilter = PAYMENT_ID
    ? { _id: requiredObjectId(PAYMENT_ID, "Payment ID"), annualPaymentId: annualPayment._id }
    : { annualPaymentId: annualPayment._id };
  const payment = await Payment.findOne(paymentFilter).sort({
    "recordPremiumPaymentTransfer.paymentDate": -1,
    updatedAt: -1,
    createdAt: -1,
  });
  if (!payment) throw new Error("Payment record not found in the selected annual payment record.");
  ...
  console.log(`${DRY_RUN ? "Preview complete" : "Reset complete"}: payment is Pending, latest annual payment is Ongoing, and policyholder is Active with no next payment date.`);
}
