/**
 * Migrate and remove legacy frequency-based Unit KPI fields (UNIT scope only).
 *
 ...
 * AGENT and BRANCH documents are not modified.
...
async function main() {
  ...
  console.log({
    ...
    unitAssignmentDocuments: assignments.length,
  ...
  if (invalidDocuments.length) {
    throw new Error("Cleanup stopped: at least one Unit KPI assignment has an invalid kpis value.");
  }

  const operations = assignments.map((assignment) => ({
    updateOne: {
      filter: { _id: assignment._id, scopeType: "UNIT" },
  ...
  console.log({
    ...
    remainingDocumentsWithoutMonthlyHistory: remainingWithoutMonthlyHistory,
  });

  if (remainingLegacyRows > 0 || remainingWithoutMonthlyHistory > 0) {
    throw new Error("Cleanup verification failed for Unit KPI assignments.");
  }
}
