/**
 * Remove legacy frequency-based Unit KPI assignment fields (UNIT scope only).
 *
 * Monthly assignments (including historical months) are preserved verbatim.
 * AGENT and BRANCH documents are deliberately not modified by this cleanup.
 * The script is a dry run unless `--apply` is supplied.
 *
 * Usage from the repository root:
 *   node backend/scripts/clear-unit-kpi-frequency-targets.js
 *   node backend/scripts/clear-unit-kpi-frequency-targets.js --apply
 */
...
async function main() {
  ...
  const unitAssignments = await collection.find(
    { scopeType: "UNIT" },
    { projection: { scopeId: 1, kpis: 1 } }
  ).toArray();

  const eligible = unitAssignments.filter((assignment) => (
    Array.isArray(assignment.kpis)
    && assignment.kpis.every((kpi) => Array.isArray(kpi.monthlyAssignments))
  ));
  const skipped = unitAssignments.length - eligible.length;
  ...
  if (skipped > 0) {
    throw new Error("Cleanup stopped: at least one Unit KPI assignment has no monthly history. Load/save monthly assignments before retrying.");
  }
  ...
  if (remainingLegacyRows > 0) throw new Error("Cleanup verification failed: legacy Unit KPI fields remain.");
}

main()
  .catch((error) => {
    console.error("Unit KPI frequency cleanup failed:", error);
    process.exitCode = 1;
  })
  .finally(async () => {
