...
function createNotificationsController({
  ...
  const syncKpiUnassignedNotificationResolutions = async (uid) => {
    ...
    const notifications = await Notification.find({
      ...
      softDeletedAt: null,
    }).select("_id entityId metadata message resolutionStatus resolvedAt createdAt").lean();
    if (!notifications.length) return resolutionsByNotificationId;
    ...
    const writes = notifications.map((notification) => {
      ...
      const assignment = assignmentById.get(String(notification.entityId || ""))
      ...
      const kpi = assignment?.kpis?.find((row) => String(row?.key || "") === kpiKey);
      const monthAssignment = kpi?.monthlyAssignments?.find((row) => String(row?.monthKey || "") === monthKey);
      const resolved = monthAssignment?.assigned === true;
