  ...
  const listNotifications = async (req, res) => {
    try {
      ...
      if (String(includeRefs) === "1" && notifs.length) {
        ...
        const longLeaveIds = uniqueValidIds(notifs.filter((notification) => notification.entityType === "LongLeave").map((notification) => notification.entityId));
        const retirementIds = uniqueValidIds(notifs.filter((notification) => notification.entityType === "Retirement").map((notification) => notification.entityId));
        ...
        const endorsementIsResolved = (endorsementContext) => {
          if (!endorsementContext?.record) return false;
          const affectedRecords = endorsementContext.kind === "retirement"
            ? [...(endorsementContext.record.affectedProspects || [])]
            : [...(endorsementContext.record.affectedProspects || []), ...(endorsementContext.record.affectedPolicyholders || [])];
          return affectedRecords.every((record) => record?.reassigned === true);
        };
        ...
        notifs = notifs.map((n) => {
          ...
          const endorsementContext = n.type === "ORPHANS_ENDORSEMENTS" ? endorsementMap.get(String(n.entityId || "")) : null;
          const endorsementResolved = n.type === "ORPHANS_ENDORSEMENTS" && endorsementIsResolved(endorsementContext);
          return {
            ...n,
