...
function createNotificationsController({
  ...
  const ensureTaskMissed =
    typeof ensureTaskMissedNotificationsForUser === "function"
      ? ensureTaskMissedNotificationsForUser
      : async () => {};
  const removeResignedAgentKpiNotifications = async (userId) => {
    if (!Agent) return;
    const resignedAgent = await Agent.exists({ userId, status: "Resigned" });
    if (!resignedAgent) return;
    await Notification.deleteMany({
      assignedToUserId: userId,
      type: { $in: [...UNIT_KPI_NOTIFICATION_TYPES, ...AGENT_KPI_NOTIFICATION_TYPES] },
    });
  };
  const toValidId =
  ...
  const listNotifications = async (req, res) => {
    try {
      ...
      await removeResignedAgentKpiNotifications(uid);
