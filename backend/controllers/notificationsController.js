...
function createNotificationsController({
  ...
  const syncKpiUnassignedNotificationResolutions = async (uid) => {
    ...
    const notifications = await Notification.find({
      ...
      softDeletedAt: null,
    }).select("_id entityId metadata resolutionStatus resolvedAt createdAt").lean();
    if (!notifications.length) return resolutionsByNotificationId;

    const assignmentIds = uniqueValidIds(notifications.map((notification) => notification.entityId));
    const assignments = assignmentIds.length
      ? await KpiAssignment.find({ _id: { $in: assignmentIds } }).select("kpis updatedAt").lean()
      : [];
    ...
    const writes = notifications.map((notification) => {
      ...
      const fallbackMonthKey = Number.isNaN(createdAt.getTime())
        ? ""
        : `${createdAt.getFullYear()}-${String(createdAt.getMonth() + 1).padStart(2, "0")}`;
      const monthKey = String(notification?.metadata?.monthKey || fallbackMonthKey);
