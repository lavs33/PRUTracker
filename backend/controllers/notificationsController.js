...
function createNotificationsController({
  ...
  const syncOrphanEndorsementResolutions = async (uid) => {
    ...
    const agentIds = uniqueValidIds(notifications.map((notification) => notification?.metadata?.agentId));
    const [longLeaves, retirements] = await Promise.all([
      longLeaveIds.length || agentIds.length ? LongLeave.find({ $or: [
        ...(longLeaveIds.length ? [{ _id: { $in: longLeaveIds } }] : []),
        ...(agentIds.length ? [{ agentId: { $in: agentIds } }] : []),
      ] }).select("agentId affectedProspects affectedPolicyholders createdAt updatedAt").lean() : [],
      retirementIds.length || agentIds.length ? Retirement.find({ $or: [
        ...(retirementIds.length ? [{ _id: { $in: retirementIds } }] : []),
        ...(agentIds.length ? [{ agentId: { $in: agentIds } }] : []),
      ] }).select("agentId affectedProspects affectedPolicyholders createdAt updatedAt").lean() : [],
    ]);
