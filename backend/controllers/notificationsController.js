...
function isTaskNotification(notification) {
  return TASK_NOTIFICATION_TYPES.includes(String(notification?.type || "").toUpperCase());
}

function taskNotificationDisplayPriority(notification) {
  const type = String(notification?.type || "").toUpperCase();
  if (type === "TASK_ADDED") return 0;
  if (type === "TASK_DUE_TODAY") return 1;
  if (type === "TASK_MISSED") return 2;
  return 99;
}
...
function sortNotificationsForDisplay(notifications) {
  return [...notifications].sort((a, b) => {
    const aIsUnreadTask = isTaskNotification(a) && a?.status === "Unread";
    const bIsUnreadTask = isTaskNotification(b) && b?.status === "Unread";
    if (aIsUnreadTask && bIsUnreadTask) {
      const aTime = notificationTime(a);
      const bTime = notificationTime(b);
      const aEntityId = String(a?.entityId || "");
      const bEntityId = String(b?.entityId || "");
      const shouldUseTaskPriority =
        (aEntityId && aEntityId === bEntityId && Math.abs(aTime - bTime) <= 5000) ||
        aTime === bTime;
      if (shouldUseTaskPriority) {
        const priorityCompare = taskNotificationDisplayPriority(a) - taskNotificationDisplayPriority(b);
